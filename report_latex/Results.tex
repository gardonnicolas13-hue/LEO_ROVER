...
\subsection{Cumulative path is not comparable across sampling rates}
\label{sec:cadencetrap}

The table above lists MINS at $22.5$\,m where the wheels give $15.1$\,m, and
that juxtaposition invites a conclusion it does not support. Cumulative path
integrates every increment between consecutive samples, including the ones
produced by noise rather than by motion. For a series of $N$ samples carrying
independent per-sample position noise of scale $\sigma$, the expected
spurious contribution to path length grows as
\begin{equation}
\mathbb{E}\bigl[\ell_{\text{noise}}\bigr] \;\propto\; N\sigma ,
\label{eq:pathnoise}
\end{equation}
linearly in the sample count. Two series describing the \emph{same}
trajectory at different rates therefore report different path lengths, and
the faster one reports more.

That is exactly the situation here: MINS publishes at ${\approx}86$\,Hz and
\code{/firmware/wheel\_odom} at ${\approx}20$\,Hz, a ratio of $4.3$. The
comparison is not between two estimates of a distance; it is between a
distance measured with $4.3$ times more opportunities to accumulate noise.

The test is direct --- decimate MINS to the wheel rate and recompute.
Across six recordings in which both series are healthy:
...
\noindent At matched rates the median ratio is $0.98$: MINS and wheel
odometry agree on distance travelled to within about two per cent, across
six independent recordings. The apparent seven-per-cent excess at full rate
--- and the larger figures visible in individual runs --- is an artefact of
Eq.~(\ref{eq:pathnoise}), not an estimation error.
...
against an independent odometric reference. Since the two references are
physically unrelated --- one inertial-visual, one from wheel encoders --- an
agreement to two per cent is meaningful in a way that either alone is not.
...
(\S\ref{sec:wheelyaw}). Nor is the wheel reference itself exact: the
effective radius carries a calibrated $r_{\mathrm{eff}} = 62.2$\,mm against
a stock $62.5$\,mm, and mecanum rollers scrub. The two per cent should be
read as ``no gross scale error detected'', not as a measured accuracy.
\end{trapbox}
