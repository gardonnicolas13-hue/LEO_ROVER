...
\subsection{Everything measured in this chapter predates the fixes}
\label{sec:datationpreuves}

The measurements above are dated, and placing them against the patch
history changes what the chapter can claim. Both VIO estimators received
divergence fixes during the campaign; neither has been recorded since.

\begin{center}
\begin{tikzpicture}[font=\small, >={Latex[length=2mm]}, xscale=1.30]
  \draw[->, thick] (0,0) -- (7.4,0);
  \foreach \x/\d in {0.5/{20 Aug}, 1.9/{21 Aug}, 3.5/{24 Aug}, 5.4/{28 Aug}, 6.9/{today}}
    { \draw (\x,-0.10) -- (\x,0.10); \node[font=\scriptsize, below=2pt] at (\x,-0.10) {\d}; }
  \node[font=\scriptsize, align=center, fill=blue!10, draw, rounded corners=2pt,
        inner sep=2.5pt] at (1.9,0.85) {recordings\\with covariance};
  \draw[->, draw=black!55] (1.9,0.52) -- (1.9,0.12);
  \node[font=\scriptsize, align=center, fill=blue!10, draw, rounded corners=2pt,
        inner sep=2.5pt] at (3.5,-0.95) {recordings\\pose only};
  \draw[->, draw=black!55] (3.5,-0.62) -- (3.5,-0.14);
  \node[font=\scriptsize, align=center, fill=alertred!12, draw=alertred!70,
        rounded corners=2pt, inner sep=2.5pt] at (3.5,0.85) {sqrtVINS\\ZUPT patch};
  \draw[->, draw=alertred!70] (3.5,0.52) -- (3.5,0.12);
  \node[font=\scriptsize, align=center, fill=alertred!12, draw=alertred!70,
        rounded corners=2pt, inner sep=2.5pt] at (5.4,0.85) {openVINS\\reset patch};
  \draw[->, draw=alertred!70] (5.4,0.52) -- (5.4,0.12);
  \draw[decorate, decoration={brace, amplitude=4pt}, draw=alertred!80, thick]
    (6.85,-0.30) -- (5.5,-0.30);
  \node[font=\scriptsize, text=alertred!85, align=center] at (6.2,-0.85)
    {no recording\\at all};
\end{tikzpicture}
\end{center}

\noindent The \code{02\_zupt\_guards} patch of 2026-08-24 modifies
\code{ov\_srvins/src/update/UpdaterZeroVelocity.cpp} --- sqrtVINS. The
\code{01\_reset\_covariance} patch of 2026-08-28 modifies
\code{ov\_msckf/src/core/VioManager.cpp} --- openVINS. Searching the data
directories for anything recorded after that second date returns nothing.

Three consequences for how this chapter should be read.

\textbf{The covariance audit is entirely pre-patch.} The recordings
carrying $\mathbf{P}$ are from 2026-08-21, before either fix. Every
statement in \S\ref{sec:nees} through \S\ref{sec:covsaturee} describes
filters as they were that evening. The MINS results are unaffected --- MINS
received neither patch --- but the openVINS and sqrtVINS numbers are
historical by construction.

\textbf{openVINS was never measured post-patch either.} The 24 August
recordings sit after the sqrtVINS fix but before the openVINS one, and
they carry no covariance. On those recordings openVINS reports $10{,}244$\,m
of path and a $3374$\,m excursion against MINS's $91.9$\,m and $17.9$\,m
over the same 415 seconds --- worse than the 21 August figures, which is
consistent with its own fix not yet existing rather than with anything the
sqrtVINS patch did.

\textbf{No patch in this campaign has a measured effect.} Both are
documented, both have a written rationale, both were verified against the
symptom that motivated them at the time of writing. Neither has a recording
taken afterwards under comparable conditions.

\begin{trapbox}{A curiosity in the 24 August exports, unresolved}
\code{testbag\_20260824\_223708} and \code{valide\_20260824\_223810} carry
different bytes --- their checksums differ and their position columns are
not equal --- yet they agree to five significant figures on every aggregate
computed here: $20{,}766$ samples, $415.3$\,s, $91.88$\,m of MINS path,
$17.887$\,m of extent, and an identical median MINS--openVINS separation of
$230.055$\,m.

Two independent runs do not do that. The likely explanation is one source
bag exported twice through slightly different paths, but the exports carry
no provenance field and nothing in the logbook records the session, so this
is stated as an open question rather than resolved. Both were treated as a
single recording in the figures above, which is the conservative choice if
they are duplicates and a minor loss of sample size if they are not.
\end{trapbox}

\noindent The remedy is inexpensive and specific: one stationary recording
and one drive, exported with the covariance columns, on the current build.
That would date the covariance audit to the software actually running,
close the before-and-after on both patches, and cost about twenty minutes
of robot time. It is recorded as item~50.

\subsection{A recording taken on the current build}
\label{sec:mesureactuelle}

Section~\ref{sec:datationpreuves} closed by noting that no estimator
recording exists after either divergence patch, and that the remedy costs
about twenty minutes. That recording was made on 2026-09-09: 242 seconds,
$20{,}597$ MINS samples with both covariance blocks, wheel velocity
confirmed at $0.000$\,m/s throughout, on the software actually running
today.

It closes half of item~50 and explains why the other half stayed open.

\subsection{MINS, then and now}

Applying the identical procedure --- same NEES definition, same
$\chi^2(3)$ reference, same first-tenth discard:

\begin{center}
\begin{tabular}{@{}lrrr@{}}
\toprule
State block & $k$ on 2026-08-21 & $k$ on 2026-09-09 & change \\
\midrule
position          & $0.011$ & $\mathbf{0.158}$ & $14\times$ closer \\
linear velocity   & $0.131$ & $0.173$ & unchanged \\
angular velocity  & $1.260$ & $\mathbf{1.195}$ & unchanged, still calibrated \\
\bottomrule
\end{tabular}
\end{center}

\noindent Two results, of different kinds.

\textbf{The angular block is calibrated on today's build.} $k = 1.195$
against $1.260$ three weeks ago. The single well-calibrated covariance
found in this report is not an artefact of one evening's recording; it
survives every change made since, and it can be relied on.

\textbf{The position block improved by a factor of fourteen} --- from a
$\sigma$ inflated ninety-fold to one inflated six-fold. The reported
$3\sigma$ fell from $1.13$--$1.30$\,m to $0.534$\,m, while the pose itself
did not change materially: position extent over the recording was
$44.5$\,mm, inside the $4$--$46$\,mm range of the August runs. The estimate
is as good as it was; the uncertainty attached to it is much less wrong.

\begin{trapbox}{A plausible cause, and why it is not asserted}
The obvious candidate is the extrinsic freeze of 2026-09-07
(\S\ref{sec:extunobs}). Setting \code{do\_calib\_ext: false} removes the
camera--IMU extrinsic from the estimated state; its uncertainty no longer
enters the filter, and no longer propagates into the position block through
the cross-covariances. A large drop in reported position variance is
exactly what that change should produce, and the direction and rough
magnitude match.

It is not established. Several things changed between the two recordings,
the two sessions differ in every uncontrolled respect, and a single pair of
measurements cannot separate one cause from the others. The clean test is
available and cheap --- revert \code{do\_calib\_ext} for one stationary
recording, re-run this procedure, and compare --- and it is the natural
companion to the wheel-updater experiment already proposed in
\S\ref{sec:neesangulaire}. Both are recorded as experiments to run, not as
conclusions reached.
\end{trapbox}

\subsection{Why the two VIO filters recorded nothing}

The same recording captured zero samples from openVINS and zero from
sqrtVINS, on a session where both processes were running and their topics
were advertised. That is not a failure of the recording: neither filter
initialises on a stationary platform. Both wait for excitation before
declaring a state, so a stationary run --- the very condition that makes
the NEES truth exact --- is also the condition under which they publish
nothing at all.

This is a genuine methodological obstacle rather than an oversight, and it
sharpens item~50 rather than closing it. Dating the VIO covariance audit
needs a recording that begins with enough motion to initialise both filters
and then holds still long enough for the stationary test, in that order,
without an intervening restart. That is a different and more demanding
manoeuvre than the one performed here, and it is the reason the openVINS and
sqrtVINS figures in this chapter remain dated 2026-08-21.
