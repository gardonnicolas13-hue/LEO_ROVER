...
\end{lstlisting}
  ...
  closely than that.
\end{enumerate}
\begin{trapbox}{Figure pending: this test has not yet been run}
The procedure above is complete and ready to run, but has not been executed
on this platform as of this writing: no figure exists yet to insert here.
Run it once, then place the resulting \code{test1\_mins\_rect.png} at the
end of this list.
\end{trapbox}
\end{enumerate}
...
\end{enumerate}
\begin{trapbox}{Figure pending: this test has not yet been run}
Same status as the MINS run above: the procedure is ready, Test 2 has not
yet been recorded.
\end{trapbox}
...
\end{enumerate}
\begin{trapbox}{Figure pending: neither test has been run yet}
Both the agreement comparison and the dimensional comparison belong here
once Test 1 and Test 2 exist. Nothing in this subsection should be filled
in from expectation or from a different pair of runs: the whole point of
an absolute reference is that it is measured, not assumed.
\end{trapbox}
