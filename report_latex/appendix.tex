...
tape-measure seed, refined online (\code{do\_calib\_ext: true} in both
\code{config\_camera.yaml} and the equivalent openVINS setting), which is why
MINS still runs and initialises correctly without this step: it is a
precision improvement, not a blocker. Completing it is the single
highest-value calibration task remaining on this platform.
\end{trapbox}
...
\code{do\_calib\_ext} / \code{do\_calib\_dt} & false / false & true / true \\
...
image path, but only 300 features and an adaptive clone rate.
