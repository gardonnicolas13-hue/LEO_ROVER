...
together, which is why this split is worth knowing by heart. This table is
node-level; \S\ref{ap:repro-inventory} carries the same split at
\emph{file} level, for every configuration file, script and launch file in
the project.}
\label{tab:appwho}
\begin{tabular}{@{}lll@{}}
\toprule
...
\section{Master file inventory: what lives where, and on which machine}
\label{ap:repro-inventory}

The single most expensive confusion on this project is that \code{rosnode
list} and \code{rostopic list} present two physically separate computers as
one flat system. A node name tells you nothing about which machine runs it,
a topic name tells you nothing about where it is published, and a file path
that exists on one machine may not exist on the other at all. Every failure
in \S\ref{ap:repro-trouble} that cost more than an hour began with an
assumption about \emph{where} something was.

This section is the inventory that answers that question once. Every entry
carries a machine marker:

\begin{itemize}[leftmargin=1.4em]
\item \textbf{[PC]} --- the operator workstation (Ubuntu 20.04, \code{eno1}
  or the USB Wi-Fi dongle). Paths are relative to the repository root,
  \code{\textasciitilde/TOUT/}, unless written absolute.
\item \textbf{[ROBOT]} --- the Raspberry Pi 4 aboard the rover. Paths are
  absolute, exactly as you see them over \code{ssh pi@\$ROBOT\_HOST}. These
  files are \emph{not} in the repository: they live on the robot's SD card
  and are lost on a reflash (\S\ref{ap:repro-robot}).
\end{itemize}

\noindent Everything below was enumerated from the filesystem on
2026-09-11, not written from memory. Where a file's runtime location differs
from where its source is stored, both are given.

\subsection{Files that exist only on the robot}
\label{ap:repro-inv-robot}

These are the reason a fresh SD card does not give you a working rover. None
of them is in git; all three project-specific changes are described with
their rationale in \S\ref{ap:repro-robot}.

\begin{longtable}{@{}L{5.6cm}cL{6.4cm}@{}}
\toprule
Path & Machine & What it is \\
\midrule
\endfirsthead
\toprule
Path & Machine & What it is \\
\midrule
\endhead
\code{/etc/ros/robot.launch} & [ROBOT] &
  The robot's own bring-up launch. Holds the RealSense arguments, including
  the \code{enable\_gyro}/\code{enable\_accel} defaults that leave the D455's
  IMU switched off (item 45, \S\ref{ap:repro-camreset}) \\
\addlinespace
\code{/etc/ros/setup.bash} & [ROBOT] &
  Pins \code{ROS\_IP}. This is the file behind the nastiest network failure
  in this report: it can advertise a dead WireGuard address while the master
  answers normally and every topic exists (\S\ref{ap:repro-trouble}) \\
\addlinespace
\code{/etc/systemd/system/leo.service} \newline plus its \code{LimitRTPRIO}
  drop-in & [ROBOT] &
  Starts \code{rosmon}, which starts the ROS master and every robot-side
  node. The drop-in is Change~1 (\S\ref{ap:repro-rtfix}); without \emph{both}
  it and the \code{chrt} call, the serial node will not start at all \\
\addlinespace
\code{/opt/ros/noetic/share/leo\_bringup/} \newline \code{launch/leo\_bringup.launch}
  & [ROBOT] &
  Stock Husarion bring-up, not ours. Replaced wholesale by a reflash \\
\addlinespace
\code{/dev/ttyAMA0} & [ROBOT] &
  The UART to the CORE2/LeoCore board. A GPIO serial line, \textbf{not} USB
  --- the distinction took a day to establish and is the whole subject of
  \S\ref{ap:repro-rtfix} \\
\addlinespace
\code{hostapd} / \code{dnsmasq} configuration & [ROBOT] &
  Serve the rover's own access point, SSID \code{LeoRover-e138} at
  \code{10.0.0.1}. This is the fallback path when the campus network refuses
  the robot, and the only address that works regardless
  (\S\ref{ap:repro-trouble}) \\
\bottomrule
\end{longtable}

\subsection{The nodes this project wrote}
\label{ap:repro-inv-nodes}

All sources live in the repository on the workstation. Read the
\emph{Runs on} column carefully: one of them is stored here but executes on
the robot, because the hardware it talks to is the robot's.

\begin{longtable}{@{}L{5.4cm}cL{6.6cm}@{}}
\toprule
Source file (in repo, [PC]) & Runs on & Role \\
\midrule
\endfirsthead
\toprule
Source file (in repo, [PC]) & Runs on & Role \\
\midrule
\endhead
\code{catkin\_ws/src/leo\_navigation/} \newline \code{scripts/imu\_sanitizer.py}
  & [PC] &
  Despikes and rescales the raw IMU; publishes \code{/imu/data\_clean}, which
  is what MINS consumes. \S\ref{ap:repro-sanitizer} \\
\addlinespace
\code{.../scripts/wheel\_remap.py} & [PC] &
  Wheel odometry remapping. \S\ref{ap:repro-wheelremap} \\
\addlinespace
\code{.../scripts/pose\_selector.py} & [PC] &
  The estimator switch; publishes \code{/robot\_pose\_fused}, the one topic
  the rest of the system reads. \S\ref{ap:repro-poseselector} \\
\addlinespace
\code{.../scripts/carolus\_vicon\_bridge.py} & [PC] &
  The only absolute reference into the filter.
  \S\ref{ap:repro-carolusbridge} \\
\addlinespace
\code{.../scripts/carolus\_tf\_bridge.py} & [PC] &
  TF plumbing for the Carolus/AprilTag frame \\
\addlinespace
\code{.../scripts/vins\_rotation\_guard.py} & [PC] &
  Written, built, \textbf{deliberately not launched}.
  \S\ref{ap:repro-guardfile} \\
\addlinespace
\code{.../scripts/pi\_health\_monitor.py} & [PC] &
  Thermal and load telemetry from the Pi, read over the network \\
\addlinespace
\code{.../scripts/goto\_pose.py} & [PC] &
  Pose-regulation motion control (\S\ref{ap:posecontrol}) \\
\addlinespace
\code{.../scripts/fill\_mins\_camchain.py} & [PC] &
  Calibration helper: writes Kalibr output into the MINS camera config \\
\addlinespace
\code{.../src/minimu9\_node.cpp} \newline
  \emph{branch} \code{feature/minimu9-driver} & \textbf{[ROBOT]} &
  I$^2$C driver for the replacement IMU. \textbf{Must run on the robot} ---
  the I$^2$C bus belongs to the Pi --- which is why it is deliberately absent
  from \code{navigation\_supervision.launch}, a workstation launch file.
  Not on the main branch; see the handoff note in
  \code{docs/HANDOFF\_NEXT\_STUDENT.md} \\
\addlinespace
\code{leo\_backend.py} (repository root) & [PC] &
  The cockpit's brain: FSM, telemetry, vision subprocess.
  \S\ref{ap:repro-backend} \\
\bottomrule
\end{longtable}

\subsection{Launch files, and what each one starts where}
\label{ap:repro-inv-launch}

All five live in \code{catkin\_ws/src/leo\_navigation/launch/} on the
workstation, and all five are run \emph{from} the workstation. They start
workstation nodes only: the robot's nodes are started by \code{leo.service}
on the robot itself, independently, and are already running before you
launch anything here.

\begin{longtable}{@{}L{4.6cm}L{9.0cm}@{}}
\toprule
File ([PC]) & What it brings up \\
\midrule
\endfirsthead
\toprule
File ([PC]) & What it brings up \\
...
\endhead
\code{navigation\_master.launch} &
  The top-level entry point. Includes the three below. This is the one
  \code{tools/restart\_stack.sh} calls \\
\addlinespace
\code{mins.launch} &
  MINS. Note the configuration path is passed through \code{args="..."} and
  \emph{not} \code{<param name="config\_path">}: on ROS1 the node reads
  \code{argv[1]} only, and passing it the other way produces
  ``unable to open the configuration file'' (item 42) \\
\addlinespace
\code{navigation\_supervision.launch} &
  \code{imu\_sanitizer}, \code{wheel\_remap}, \code{pose\_selector}, the
  image \code{republish} nodes. Also carries the \code{imu\_source} argument
  (\code{core2} $\leftrightarrow$ \code{minimu9}) that switches the sanitizer's
  input topic and accelerometer scale \emph{together} \\
\addlinespace
\code{pose\_selector.launch} &
  The selector alone, for restarting it without the rest --- required after
  any estimator reset (item 48) \\
\addlinespace
\code{tf\_static\_extrinsics.launch} &
  The static transform tree \\
\bottomrule
\end{longtable}

\subsection{Where each estimator actually reads its configuration}
\label{ap:repro-inv-config}

All estimator configuration is on the workstation: the estimators themselves
run there (\S\ref{ap:mins-embedded} explains why MINS is not on the Pi).

\begin{longtable}{@{}L{5.0cm}L{8.6cm}@{}}
\toprule
Estimator ([PC]) & Configuration path \\
\midrule
\endfirsthead
\toprule
Estimator ([PC]) & Configuration path \\
...
\endhead
MINS &
  \code{LEO\_Rover\_Navigation\_System/} \newline
  \code{MINS-master/mins/config/leo/} \newline
  Ten files: \code{config.yaml} is the entry point and names the other nine
  (\code{config\_imu}, \code{config\_camera}, \code{config\_wheel},
  \code{config\_estimator}, \code{config\_init}, \code{config\_system},
  \code{config\_vicon}, \code{config\_lidar}, \code{config\_gps}) \\
\addlinespace
MINS, camera-only profile &
  \code{.../mins/config/leo\_vio/} --- the same platform with the wheels
  removed, used to isolate what the wheels contribute \\
\addlinespace
MINS, Pi profile &
  \code{.../mins/config/leo\_pi/} --- mono, downsampled, calibration off.
  Kept for reproducibility of the embedded-feasibility arithmetic; never
  adopted (item 43) \\
\addlinespace
openVINS &
  \code{catkin\_ws/src/open\_vins/config/leo/} \newline
  \code{estimator\_config.yaml} plus \code{kalibr\_imucam\_chain.yaml} and
  \code{kalibr\_imu\_chain.yaml} \\
\bottomrule
\end{longtable}

\begin{trapbox}{\code{\$(find mins)} does not resolve to the path you are reading}
\code{mins.launch} loads
\code{\$(find mins)/config/\$(arg config)/config.yaml}, which resolves
through the ROS package path, not through the directory you happen to be
editing. On this workstation \code{mins\_ws/src/MINS} is a
\textbf{symbolic link} to
\code{/home/lab272/TOUT/LEO\_Rover\_Navigation\_System/MINS-master}
(verified 2026-09-11). There is therefore exactly \emph{one} copy of the
MINS configuration on disk, reachable by two different-looking paths.

This matters in both directions. Editing either path edits the same file, so
a change you think you made in a scratch copy is live. And if a future
student replaces that symlink with a real second checkout, the two paths
become independent, the launch file will keep loading the one you are
\emph{not} editing, and the filter will silently run yesterday's calibration.
Check with \code{readlink -f mins\_ws/src/MINS} before trusting either.
\end{trapbox}

\subsection{Tooling, by the question it answers}
\label{ap:repro-inv-tools}

\code{tools/} holds roughly sixty scripts [PC]; every one carries a header
comment explaining why it exists, and that header is the authority, not this
table. What follows is the subset a new arrival needs in the first week,
grouped by the question being asked. Several of these \emph{operate on} the
robot over SSH while \emph{running from} the workstation --- that is the
normal pattern here, and the \emph{Acts on} column says which.

\begin{longtable}{@{}L{4.8cm}cL{7.2cm}@{}}
\toprule
Script (in \code{tools/}, [PC]) & Acts on & Question it answers \\
\midrule
\endfirsthead
\toprule
Script (in \code{tools/}, [PC]) & Acts on & Question it answers \\
\midrule
\endhead
\code{robot\_env.sh} & [PC] &
  Where is the robot? Single source of truth for \code{ROS\_MASTER\_URI} and
  \code{ROS\_IP}, overridable through \code{\textasciitilde/.leo\_network}.
  Sourced by every other script \\
\addlinespace
\code{preflight\_mins.py} & both &
  Is the whole chain healthy enough to run an experiment? GO/NO-GO in one
  command. Run it \emph{before} any test that costs robot time \\
\addlinespace
\code{restart\_stack.sh} & [PC] &
  Restart the workstation navigation stack. Mandatory ordering after any
  estimator reset: sanitizer $\rightarrow$ MINS $\rightarrow$
  \code{pose\_selector} (item 48) \\
\addlinespace
\code{record\_trajectories.sh} & [PC] &
  Capture an experiment properly. Writes a rosbag to disk, independent of the
  web cockpit --- which is \emph{not} reliable for a run that counts \\
\addlinespace
\code{compare\_tests.m}, \newline \code{plot\_trajectories.m} & [PC] &
  What did the run show? MATLAB comparison, accepts \code{.mat} or CSV \\
\addlinespace
\code{diag\_reseau\_robot.sh} & both &
  Why can I not reach the robot? \\
\addlinespace
\code{recuperer\_robot\_hotspot.sh} & both &
  The robot is powered but unreachable: switches the workstation onto the
  rover's own access point, diagnoses over SSH, and restores the campus
  Wi-Fi afterwards \\
\addlinespace
\code{check\_uart\_overruns.sh}, \newline \code{check\_serial\_health.sh}
  & \textbf{[ROBOT]} &
  Is the serial link losing bytes? Reads the Pi's own UART counters over SSH
  (\S\ref{ap:repro-rtfix}) \\
\addlinespace
\code{verif\_minimu9.py} & \textbf{[ROBOT]} &
  Is the replacement IMU wired and sane? Control and pre-calibration of the
  MinIMU-9, to be run once the sensor is physically installed \\
\addlinespace
\code{launch\_sqrtvins\_pi.sh}, \newline \code{sqrtvins\_pi\_ctl.sh}
  & \textbf{[ROBOT]} &
  $\sqrt{\text{VINS}}$ runs \emph{on the Pi}, on demand, and not as part of
  the workstation stack (\S\ref{sec:sqrtvins}) \\
\addlinespace
\code{leo\_watchdog.sh} & [PC] &
  The self-healing cron watchdog. Suspend it with
  \code{touch /tmp/leo\_maintenance} before any manual work, and
  \textbf{remember to remove that file afterwards} \\
\addlinespace
\code{update\_report.sh} & [PC] &
  Rebuild and publish this document. Note it prepends the TeX~Live~2026
  installation in \code{\textasciitilde/texlive/2026/} to \code{PATH}: the
  system \code{pdflatex} is a Debian TeX~Live~2019 that lacks packages this
  report uses, so a build run without that \code{PATH} fails on
  \code{IEEEtrantools.sty} \\
\bottomrule
\end{longtable}

\subsection{Cockpit, data, and documentation}
\label{ap:repro-inv-web}

\begin{longtable}{@{}L{4.4cm}cL{7.6cm}@{}}
\toprule
Path & Machine & What it is \\
\midrule
\endfirsthead
...
\midrule
\endhead
\code{web/ops.html} & [PC] &
  The control cockpit: driving, AUTO, pose source, export \\
\addlinespace
\code{web/trajectory.html} & [PC] &
  Live map, batched Canvas rendering \\
\addlinespace
\code{web/demo.html} & [PC] &
  Read-only by construction --- no publisher, no command binding. For
  demonstrations \\
\addlinespace
\code{web/logbook.html} + \code{auto\_entries.json} & [PC] &
  Mission log, written automatically by the backend \\
\addlinespace
\code{web/serve.py} & [PC] &
  Static server. Its filter mirrors \S10 of the \code{.gitignore}: what git
  refuses to archive, the server refuses to publish --- added after an audit
  found \code{exports/} browsable on the public internet \\
\addlinespace
\code{web/start\_web.sh} & [PC] &
  Starts \code{serve.py}, \code{web\_video\_server}, \code{rosbridge} and the
  Cloudflare tunnel \\
\addlinespace
\code{data/trajectories/} & [PC] &
  Exported runs (CSV, PNG) and the rosbags. Not versioned: the raw
  recordings exceed what a git repository should carry \\
\addlinespace
\code{report\_latex/} & [PC] &
  This document's sources, and \code{main.pdf} \\
\addlinespace
\code{JOURNAL\_DE\_BORD.md} & [PC] &
  Dated version history of the whole campaign. The first place to look when
  you need to know \emph{when} something changed \\
\addlinespace
\code{docs/HANDOFF\_NEXT\_STUDENT.md} & [PC] &
  Prioritised list of what to work on next, written at handover \\
\addlinespace
\code{archives/} & [PC] &
  Out of repository. Timestamped backups, received documents, deliveries,
  figures. Nothing was deleted into it and nothing referenced was moved
  there \\
\bottomrule
\end{longtable}

\begin{trapbox}{A file in this repository is not necessarily code that runs
here}
Three categories coexist in the same tree, and telling them apart saves a
great deal of time:
\begin{itemize}[leftmargin=1.4em]
\item Code that runs on the \textbf{workstation} --- the great majority.
\item Code that is stored here but \textbf{executes on the robot}:
  \code{minimu9\_node.cpp}, the $\sqrt{\text{VINS}}$ launchers, and every
  script in the \emph{Acts on: ROBOT} rows above. Copy or deploy them to the
  Pi; running them here does nothing useful, and in the I$^2$C case will
  simply find no bus.
\item Upstream code that is \textbf{not ours at all} --- openVINS, MINS,
  Kalibr, realsense-ros (\S\ref{ap:repro-repos}). Patches we made to these
  are kept separately in \code{patches/}, \code{patches\_openvins/} and
  \code{patches\_mins/} precisely so that the distinction survives a
  re-clone.
\end{itemize}
\end{trapbox}
...
Four upstream stacks are used, each in its own catkin workspace so their
build flags and dependencies stay isolated (Kalibr's workspace,
\code{kalibr\_ws}, is separate from the three runtime workspaces of
\S\ref{ap:repro-build} because it is a calibration-time tool, not something
that runs alongside the estimators):
...
\section{Building the operator website, step by step}
\label{ap:repro-website}

The cockpit is described elsewhere in this report by \emph{topic}: its
architecture and data flow in \S\ref{sec:multipage}, its public API in
\S\ref{sec:web}, its transport and ports in \S\ref{ap:repro-webtransport}, the
Cloudflare tunnel in \S\ref{ap:repro-cloudflare}. \code{web/README\_WEB.md} in the
repository covers the same ground for a maintainer.

None of those is a \emph{build order}. This section is: fourteen steps from an
empty directory to a public, self-healing cockpit, each one naming the exact
files it creates or touches, so that a successor can follow the same path --- or
find the one file responsible for the part that is misbehaving.

\begin{trapbox}{The entire website runs on the workstation}
Not one line of the cockpit runs on the robot. Every file named in this
section is \textbf{[PC]} in the sense of \S\ref{ap:repro-inventory}. The robot
publishes ROS topics and nothing else; it does not serve HTTP, does not hold
the pages, and is not reachable from a browser. The robot's own
\code{rosbridge} is in fact deliberately \emph{disabled}
(\S\ref{ap:repro-phantom}) so that it cannot compete with the
workstation's.

This matters when something breaks: a cockpit fault is almost never a robot
fault, and restarting the rover to fix a blank page is a reflex worth
unlearning.
\end{trapbox}

\subsection{Step 1 --- Choose an architecture that needs no server code}
\label{ap:repro-web-arch}

The decision that shapes everything else: the browser talks to ROS
\emph{directly}, over a WebSocket, using the \code{rosbridge} protocol v2.0.
There is no application server, no REST API, no database, and no
robotics code written for the web at all. A page opens a socket, subscribes to
topics, publishes to topics, and that is the whole contract.

The consequence worth stating plainly is that \textbf{the pages are static
files}. They can be copied, opened from disk, and served by anything. What
makes them a cockpit is \code{vendor/roslib.min.js} plus the address of a
running \code{rosbridge}.

\subsection{Step 2 --- Stand up the transport}
\label{ap:repro-web-transport}
...
\midrule
\endhead
\code{web/start\_web.sh} &
  Starts and supervises every web-side service. Read its header before
  changing anything: it is idempotent by design, and the reason is a real
  failure (below) \\
\addlinespace
\code{tools/robot\_env.sh} &
  Sourced by \code{start\_web.sh}. Single source of truth for
  \code{ROS\_MASTER\_URI} and \code{ROS\_IP}; changing networks is one line in
  \code{\textasciitilde/.leo\_network}, not five files \\
\bottomrule
\end{longtable}

\noindent \code{rosbridge} is launched on port \textbf{9090} for direct LAN
use. The parameter \code{\_websocket\_\allowbreak external\_port:=9090} is passed
explicitly and must stay: \code{autobahn} validates the HTTP \code{Host}
header, and \code{rospy} will otherwise re-read a stale
\code{\textasciitilde websocket\_\allowbreak external\_port} left on the parameter server
by an earlier launch --- a ghost that cost two days of a broken localhost
mode.

\begin{trapbox}{Why \code{start\_web.sh} restarts nothing it does not have to}
The watchdog (\S\ref{ap:repro-watchdog}) re-runs this script whenever
\emph{one} port is missing. An earlier version started from scratch every
time, so a \code{9443} that failed to bind --- a flapping Wi-Fi is enough ---
caused the \emph{healthy} 9090 \code{rosbridge} to be killed once a minute,
and the operator's cockpit disconnected in a loop. Each component is now
restarted only if genuinely dead, and a component still starting (process up,
port not yet bound) is left alone: \code{rosbridge} can take over a minute to
bind under load.
\end{trapbox}

\subsection{Step 3 --- The first page}
\label{ap:repro-web-firstpage}
...
\midrule
\endhead
\code{web/vendor/} \newline \code{roslib.min.js} &
  The ROS client, \emph{vendored} rather than loaded from a CDN. This is
  deliberate: the cockpit must work on a lab network with no internet \\
\addlinespace
\code{web/ops.html} &
  The control page: driving, AUTO mode, pose source, export \\
\addlinespace
\code{web/app.js} &
  All cockpit logic, shared by the pages that need it. Its header block is the
  authoritative statement of the ROS contract \\
\bottomrule
\end{longtable}

\subsection{Step 4 --- Define the ROS contract, once}
\label{ap:repro-web-contract}

Four topics carry everything, and they are produced and consumed by
\code{leo\_backend.py} (\S\ref{ap:repro-backend}) on the workstation:

\begin{longtable}{@{}L{5.0cm}L{8.6cm}@{}}
\toprule
Topic & Direction and payload \\
\midrule
\endfirsthead
\toprule
Topic & Direction and payload \\
\midrule
\endhead
\code{/mission/command} &
  browser $\rightarrow$ backend. \code{std\_msgs/String} carrying JSON:
  \code{set\_mode}, \code{stop}, \code{reset}, \code{clear\_map},
  \code{park}, \code{set\_params}, \code{manual}, \code{set\_view} \\
\addlinespace
\code{/mission/telemetry} &
  backend $\rightarrow$ browser, ${\sim}10$\,Hz. One JSON object, roughly
  sixty fields (\S\ref{sec:telemetry-schema}) \\
\addlinespace
\code{/mission/log} &
  backend $\rightarrow$ browser. Human-readable mission events \\
\addlinespace
\code{/imu/data\_clean} &
  subscribed directly for the artificial horizon --- the one case where a
  page reads a sensor topic rather than the backend's summary \\
\bottomrule
\end{longtable}

\subsection{Step 5 --- Video, and why it is not on this script any more}
\label{ap:repro-web-video}

\code{web\_video\_server} serves MJPEG on port \textbf{8080}, and the cockpit
displays \code{/mission/image\_annotated} --- the backend's annotated frame,
not the raw camera.

It is started by \code{navigation\_supervision.launch}, \emph{not} by
\code{start\_web.sh}. The note in the script explains why: it belongs with the
navigation stack that produces its image, and when \code{start\_web.sh} freed
port 8080 on every web healing, it killed the supervised instance each time.

\subsection{Step 6 --- Replace \code{http.server} with something that can be trusted}
\label{ap:repro-web-serve}

\code{web/serve.py} [PC] is the static server on port \textbf{8000}. It exists
for three defects that \code{python3 -m http.server} has and that only became
visible once the site was public:

\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Stale pages.} Documents that change on every update
  (\code{.html}, \code{.json}, \code{.pdf}, \code{.js}, \code{.css}) are sent
  with \code{Cache-Control: no-cache, must-revalidate}. Note the \code{.pdf}:
  it was forgotten in the first fix, and Cloudflare --- seeing no
  \code{Cache-Control} --- applied its own four-hour default and publicly
  served a 300-page report while the disk held 309. A forced browser reload
  could not help: the stale copy was at the network edge, not in the client.
\item \textbf{Directory listings.} \code{SimpleHTTPRequestHandler} serves the
  whole directory, listings included. The audit of 2026-08-18 confirmed live
  that \code{/exports/} answered 200 with a browsable index of 104 files,
  17\,MB of experimental data; \code{models/} likewise.
\item \textbf{Backup files served next to the real ones.} This project's own
  convention produces names like \code{ops.html.\allowbreak bak-\allowbreak avant-v4-161908} ---
  complete, obsolete cockpits, published beside the current one. Thirteen were
  found. The refusal patterns in \code{serve.py} are those of \S10 of the
  \code{.gitignore}, deliberately: what git refuses to archive, the server
  refuses to publish.
\end{enumerate}

\subsection{Step 7 --- A design system, self-hosted}
\label{ap:repro-web-design}
...
\midrule
\endhead
\code{web/mission.css} &
  The design system --- ``Deep Space Control''. Colour tokens, the
  glassmorphic panels, the bento grid \\
\addlinespace
\code{web/fonts/fonts.css} \newline \code{web/fonts/inter-*.woff2} &
  Inter, self-hosted in four weights. No Google Fonts request: the cockpit
  must render identically with no internet \\
\addlinespace
\code{web/vendor/} \newline \code{tailwind.min.js} &
  Utility classes, vendored for the same reason \\
\addlinespace
\code{web/vendor/} \newline \code{lucide-1.25.0.umd.js} &
  Icons \\
\addlinespace
\code{web/vendor/} \newline \code{gsap.min.js}, \newline \code{ScrollTrigger.min.js} &
  Animation on the presentation pages \\
\addlinespace
\code{web/vendor/} \newline \code{chart-4.5.1.umd.js} &
  Plots (PID curves, telemetry history) \\
\addlinespace
\code{web/vendor/NASA\_logo.svg}, \newline \code{fit\_seal.svg} &
  Institutional marks \\
\bottomrule
\end{longtable}

\subsection{Step 8 --- The fourteen pages}
\label{ap:repro-web-pages}

\begin{longtable}{@{}L{4.7cm}L{8.9cm}@{}}
\toprule
Page (\code{web/}) & What it is for \\
\midrule
\endfirsthead
\toprule
Page (\code{web/}) & What it is for \\
...
\code{ops.html} & \textbf{The cockpit.} Driving, AUTO, pose source, export \\
\code{trajectory.html} & Live 2D map, batched Canvas rendering \\
\code{logbook.html} & Mission journal; also carries the report download card \\
\code{demo.html} & \textbf{Read-only by construction} --- no publisher, no
  command binding. For demonstrations, so that no one can drive by accident \\
\code{pid.html} & The three control loops, live \\
\code{system.html} & Host and stack health \\
\code{audit.html} & Audit views \\
\code{navigation\_modes.html} & Explains the navigation modes \\
\code{robot\_description.html} & The robot, with the 3D model \\
\code{tf\_validator.html} & Inspect and \emph{edit} frames --- the only page
  that writes (Step 12) \\
\code{mins\_tuning.html} & MINS parameter exploration \\
\code{hector\_response.html} & A written answer to a supervisor's question,
  kept as a page \\
\code{preview-design.html} & Design bench for the system of Step 7 \\
\bottomrule
\end{longtable}

\subsection{Step 9 --- The 3D view}
\label{ap:repro-web-3d}
...
\midrule
\endhead
\code{web/3d\_engine.js} & The renderer used by the description and
  trajectory pages \\
\code{web/vendor/three.min.js} & three.js \\
\code{web/vendor/} \newline \code{OrbitControls.js} & Camera control \\
\code{web/vendor/STLLoader.js} & Mesh loading \\
\code{web/models/} \newline \code{leo\_rover\_v1.9\_v5.stl} \newline
\code{web/models/leo\_rover\_map.stl} & The rover and the room \\
\code{tools/decimate\_stl.py} & Reduces mesh size before shipping it to a
  browser --- a CAD export is not a web asset \\
\bottomrule
\end{longtable}

\subsection{Step 10 --- Make it bilingual}
\label{ap:repro-web-i18n}

\code{web/i18n.js} [PC] holds every string in English and French and exposes
\code{I18N.t()}, \code{I18N.set()} and \code{I18N.apply()}; the DOM is marked
up with \code{data-i18n} attributes and re-scanned on change. The chosen
language persists in \code{localStorage}.

Two findings are recorded rather than hidden. The key counts match exactly
(616 EN, 616 FR), but \textbf{75 keys are dead}, and 37 of those are
\code{log\_*} --- which reveals that the mission log is \emph{never}
translated, because the backend emits raw French text on
\code{/mission/log}. Translating the cockpit did not translate the thing an
operator actually reads during a mission.

\subsection{Step 11 --- Pages that read data files}
\label{ap:repro-web-data}
...
\midrule
\endhead
\code{web/auto\_entries.json} &
  Mission events written automatically by \code{leo\_backend.py}; read by
  \code{logbook.html}. Also the evidence source for several results in this
  report (RTB pairs, \S\ref{sec:july23rtb}) \\
\code{web/logbook\_data.js} & The hand-written journal entries \\
\code{web/tf\_frames.json} & Frames shown --- and edited --- by
  \code{tf\_validator.html} \\
\bottomrule
\end{longtable}

\subsection{Step 12 --- One write path, bound to the loopback on purpose}
\label{ap:repro-web-write}

\code{tf\_validator.html} can \emph{save} an edited frame. That write is served
by \code{tools/tf\_save\_server.py} [PC] on \textbf{127.0.0.1:8010}, and the
binding is the security design:

port 8000 is published to the internet by the Cloudflare tunnel, so a write
route placed there would be open to everyone. Filtering by client IP would
achieve nothing, because \code{cloudflared} connects from the loopback itself
and every request would appear local. Binding the write server to
\code{127.0.0.1} instead means saving works from the workstation and fails
from outside --- which is exactly the intent.

\begin{trapbox}{The one place a rotation matrix can be overwritten from a browser}
This is the page behind the forced-identity incident of 2026-08-20: a
guard-rail was overridden twice and an identity matrix stayed on disk until
the next day, invalidating every openVINS feature measurement taken in
between. The lesson is not to remove the page but to date what it changed:
after any edit here, check \code{web/tf\_frames.json} against what the
estimator is actually loading.
\end{trapbox}

\subsection{Step 13 --- Publish it, with TLS that survives a tunnel}
\label{ap:repro-web-public}
...
\midrule
\endhead
\code{cloudflared tunnel run} &
  Publishes the site at \code{cockpit.leo-rover-gardon.dev}. Started from
  \code{start\_web.sh} with an \textbf{absolute path}: bare
  \code{cloudflared} failed silently under the watchdog's minimal
  \code{PATH}, leaving the public tunnel dead with no visible alert \\
\addlinespace
\code{tools/} \newline \code{rosbridge\_websocket\_tunnel.py} &
  The TLS \code{rosbridge} on port \textbf{9443}, launched with
  \code{\_websocket\_external\_port:=443}. Without that value, handshakes
  arriving from \code{cloudflared} carry a public \code{Host} with no port
  --- implicitly 443 --- and \code{autobahn} rejects every one of them \\
\addlinespace
\code{\textasciitilde/.leo\_certs/rosbridge\_cert.pem}, \newline
\code{rosbridge\_key.pem} &
  The certificate pair. Outside the repository, deliberately \\
\addlinespace
\code{/tmp/leo\_tunnel\_off} &
  A flag, not a file to edit by hand. Created by the cockpit's
  \emph{Tunnel public} button; \code{start\_web.sh} checks it and refuses to
  reopen an exposure the operator has just closed --- otherwise the watchdog
  would reopen it within the minute \\
\bottomrule
\end{longtable}

\subsection{Step 14 --- Publish the report onto the site}
\label{ap:repro-web-report}

\code{tools/update\_report.sh} [PC] compiles this document and deploys it to
the single public location: the download card of
\code{logbook.html}, at
\code{web/reports/} \newline \code{LEO\_Rover\_Mission\_Control\_}\newline\code{Report\_Nicolas\_Gardon.pdf}.

It also rewrites the \code{?v=} stamp on that link across every page that
carries it. The stamp is not decoration: without it Cloudflare serves a cached
copy, and the symptom is a report that is visibly several versions old with no
error anywhere. \code{serve.py}'s \code{no-cache} on \code{.pdf} (Step 6) is
the second line of defence; the stamp is the first.

\subsection{Verifying the site after any change}
\label{ap:repro-web-verify}

Three checks, in increasing cost, all documented in
\code{web/README\_WEB.md}:

\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Structure} --- the checker in \code{README\_WEB.md} validates
  every page's structure, links and assets \emph{on the served site}, not on
  disk. The distinction matters: \code{serve.py} refuses paths that exist
  perfectly well in the directory.
\item \textbf{i18n} --- list keys used in HTML but absent from
  \code{i18n.js}. A missing key renders as its raw identifier, which is
  unmistakable once seen and easy to miss while editing.
\item \textbf{Through the tunnel} --- load the public URL, not just
  \code{localhost:8000}. Several defects in this section existed only at the
  edge: the four-hour PDF cache, the \code{Host} header rejection on 9443,
  the browsable \code{exports/}. None was reproducible locally.
\end{enumerate}
...
\subsection{MINS, start to finish: download, build, install, calibrate, test}
\label{ap:repro-mins-guide}
MINS (multi-sensor inertial navigation system) is the tightly-coupled filter
on this platform: camera, IMU and wheel odometry fused in one estimator,
publishing its pose on \code{/mins/imu/odom}. Because its own TF broadcasts
would otherwise fight openVINS over the same frames, they are isolated onto a
dedicated \code{/tf\_mins} topic (subscribe there explicitly in RViz, not on
\code{/tf}).

This subsection is a single, self-contained path from an empty machine to a
MINS you can trust, and does not assume the rest of this appendix has been
read first: download and build the estimator, install and understand the ten
configuration files that make it work on \emph{this} robot rather than a
generic one, calibrate the two measurements those files depend on, then
validate that a fresh install is not just running but running correctly.
Steps that apply to the whole platform and not just MINS (ROS, system
libraries, network) are covered fully in \S\ref{ap:repro-build} and only
summarised here; every \S reference below is for extra depth, not a
prerequisite for the step it is attached to.

\paragraph{Download and build.}
\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Prerequisites.} Ubuntu~20.04 with ROS~Noetic, and the C++
  libraries both estimators need (\S\ref{ap:repro-build}, Steps~0--2):
\begin{lstlisting}[language=bash]
sudo apt install libeigen3-dev libboost-all-dev libopencv-dev \
                 libceres-dev libsuitesparse-dev
\end{lstlisting}
  \textbf{One MINS-specific caveat, from upstream's own README:} MINS is
  tested against Eigen~$3.3.7$ or lower; a newer system Eigen can fail to
  compile the bundled \code{libpointmatcher} third-party library (used for
  the LiDAR sensor, disabled on this platform but still compiled). Ubuntu
  20.04's own \code{libeigen3-dev} is $3.3.7$, so this only bites if you have
  manually upgraded it.
\item \textbf{Clone MINS} into its own workspace, kept separate from
  openVINS because the two disagree on build flags (\S\ref{ap:repro-build}):
\begin{lstlisting}[language=bash]
mkdir -p ~/TOUT/mins_ws/src
cd ~/TOUT/mins_ws/src
git clone https://github.com/rpng/MINS.git
\end{lstlisting}
\item \textbf{Resolve dependencies and build:}
\begin{lstlisting}[language=bash]
cd ~/TOUT/mins_ws
...
\end{lstlisting}
  \textbf{Expected:} a progress table ending with a green summary,
  \code{[build] Summary: All ... packages succeeded!}, matching
  \S\ref{ap:repro-build}'s Step~3.
\item \textbf{Do not source this workspace back-to-back with
  \code{catkin\_ws}.} \code{devel/setup.bash} replaces several environment
  variables instead of extending them, so sourcing both in sequence breaks
  one workspace's or the other's \code{ROS\_PACKAGE\_PATH}
  (\S\ref{ap:repro-sourcetrap}, the single most confusing failure mode in
  this build). \textbf{Never launch anything MINS-side with a plain
  \code{roslaunch}}; always go through the wrapper:
\begin{lstlisting}[language=bash]
~/TOUT/catkin_ws/src/leo_navigation/launch_mins.sh navigation_master.launch
\end{lstlisting}
  or, equivalently and more simply, \code{bash tools/restart\_stack.sh},
  which calls the same wrapper for you.
\end{enumerate}

\paragraph{Install and configure.}
\textbf{The step a fresh clone is missing.} MINS ships with example
configuration profiles (\code{kaist}, \code{tum\_vi}, \code{euroc\_mav},
\code{simulation}, \dots) but no profile for this robot: there is no
\code{mins/config/leo/} directory in the upstream repository. Without one,
\code{mins.launch}'s default argument (\code{config:=leo}) points at a file
that does not exist, and MINS reports that it cannot open a configuration
file. The \code{leo} profile is \textbf{this project's own work}, so it is
versioned in \emph{this} repository, not MINS's, at
\code{LEO\_Rover\_Navigation\_System/MINS-master/mins/config/leo/}. Install it
into the workspace just built:
\begin{lstlisting}[language=bash]
cp -r ~/TOUT/LEO_Rover_Navigation_System/MINS-master/mins/config/leo \
      ~/TOUT/mins_ws/src/MINS/mins/config/leo
\end{lstlisting}
\textbf{Verify:} \code{ls ~/TOUT/mins\_ws/src/MINS/mins/config/leo/} lists ten
files. Every one is required: \code{config.yaml}'s only job is to include the
other nine by relative filename, and a missing file aborts the same way a
missing \code{leo/} directory does.

\begin{center}\small
\begin{longtable}{@{}L{2.9cm}L{9.9cm}@{}}
\toprule
File & What it holds, and what is not a stock/default value \\
\midrule
\endhead
\code{config.yaml} & Top-level include list only. Also documents, in its own
  header comment, the \textbf{enable ladder} this deployment follows: bring
  up camera+IMU first, then wheels, then the Carolus VICON slot, then
  LiDAR last, each stage validated before the next is switched on. An
  unvalidated extrinsic on one sensor makes the whole fused estimate diverge
  in a way that is hard to attribute back to that sensor. \\
\code{config\_imu.yaml} & Noise densities from an Allan-variance
  characterisation of \emph{this} IMU, not datasheet figures. The field that
  matters most here is \code{topic: "/imu/data\_clean"}: point this at the
  raw firmware topic (\code{/firmware/imu}) instead and MINS silently
  ingests every glitch, every frozen-sample run, and the accelerometer's
  $-10.6\%$ scale error that \code{imu\_sanitizer.py} exists to remove
  (\S\ref{ap:repro-ourcode}). \\
\code{config\_camera.yaml} & \code{cam0}/\code{cam1} topics are
  \code{/pc/camera/infra1|2/image\_rect\_raw}: the workstation's local
  republish of the robot's compressed stream, never the robot's raw topics
  directly (two raw $848\times480$ streams demand more bandwidth than the
  WiFi link can sustain, and have been measured saturating the AP to the
  point of dropping it). Intrinsics come from \code{kalibr\_calibrate\_cameras}
  and are frozen (\code{do\_calib\_int: false}); \code{T\_imu\_cam} is still a
  tape-measure seed, see \textbf{Calibrate} below. \code{max\_slam: 0} because
  this platform pivots on the spot often, which makes landmark depth
  unobservable and produced a measured $+0.58$\,rad/s phantom heading term
  from bad SLAM anchors on 2026-07-13. \\
\code{config\_wheel.yaml} & The most heavily retuned file; see the dedicated
  walkthrough below. \\
\code{config\_estimator.yaml} & \code{gravity\_mag: 9.790}, local gravity for
  Melbourne FL, not the generic $9.81$; re-measure only if this configuration
  is deployed somewhere else. Filter and clone-rate settings otherwise match
  MINS's own defaults. \\
\code{config\_init.yaml} & Static IMU+wheel initialisation thresholds. MINS
  initialises \textbf{standing still} in $4$--$12$\,s with these defaults;
  nothing platform-specific to change here. \\
\code{config\_system.yaml} & \code{verbosity: 1}, not MINS's own default of
  \code{2}: at \code{2}, the initialiser's diagnostic prints (waiting for
  data, gravity convergence, static/dynamic init attempts) are filtered
  silently, which looks exactly like a hung initialiser when it may be
  running normally. Also sets the output paths for the trajectory/timing
  logs. \\
\code{config\_gps.yaml} & \code{enabled: false}. Present only because MINS's
  loader expects the file to exist; this platform has no GNSS receiver.
  Leave it as shipped. \\
\code{config\_lidar.yaml} & \code{enabled: false}. Staged for a future
  integration (an RPLidar publishes \code{LaserScan}; MINS wants
  \code{PointCloud2}, and a conversion node does not exist yet). Leave
  disabled. \\
\code{config\_vicon.yaml} & \code{enabled: false} until
  \code{carolus\_vicon\_bridge.py} is verified publishing on
  \code{/mins/external\_ref/carolus}, MINS's slot for the Carolus beacon
  acting as an indoor GPS (\S\ref{ap:repro-ourcode}). Flip to \code{true}
  once that bridge is confirmed live; the topic is a private slot and
  nothing else may publish on it (rule established 2026-07-08). \\
\bottomrule
\caption{Every file \code{config.yaml} includes, and what is genuinely
platform-specific inside it.}
\label{tab:minsconfigfiles}
\end{longtable}
\end{center}

\noindent\textbf{\code{config\_wheel.yaml} in detail}, because it carries more
measured, platform-specific corrections than any other file in the tree:
\begin{itemize}[leftmargin=1.4em]
\item \code{type: "Wheel2DAng"}: a two-wheel differential model. This rover is
  four-wheel skid-steer, not differential-drive; \code{Wheel2DAng}
  approximates it from the front-left/front-right pair, and the online
  extrinsic refinement below absorbs most of the mismatch.
\item \code{intrinsics: [0.0622, 0.0622, 0.358]}: effective wheel radius
  (twice) and track width, in metres. \textbf{These are measured, not the
  firmware's stock values}, and they are specific to this set of wheels; see
  \textbf{Calibrate} below for how to re-measure them.
\item \code{T\_imu\_wheel}: a $180^\circ$ rotation about $z$, because this
  rover's wheel-odometry frame points opposite to the camera/IMU frame at the
  firmware level. Without it, every wheel measurement contradicts the
  inertial prediction and the $\chi^2$ gate silently rejects it, which looks
  exactly like ``MINS is ignoring the wheels.''
\item \code{chi2\_mult: 5}, not MINS's default of \code{15}: the
  slip-rejection gate for wheel-versus-IMU disagreement, tightened after
  measuring rigid wheels slipping in turns (below).
\item \code{noise\_w: 0.8} and \code{noise\_v: 0.2}, both retuned from MINS's
  defaults (\code{0.2} and \code{0.5} respectively): in a turn, this rover's
  wheels can imply a yaw rate an order of magnitude larger than the
  camera-measured truth ($3.8$\,rad/s of wheel-implied rotation for
  $0.001$\,rad/s actually measured), so angular trust is loosened; linear
  speed from the wheels is accurate and is trusted \emph{more} than the
  default, because it is what keeps the filter anchored through camera
  dropouts.
\item \code{topic: "/joint\_states\_mins"} with
  \code{sub\_topics: "wheel\_FL\_joint, wheel\_FR\_joint"}: MINS reads wheel
  velocities \textbf{by array index, not by name}
  (\code{ROSSubscriber::callback\_wheel} takes \code{velocity.at(0)} and
  \code{.at(1)}), and this rover's real \code{/joint\_states} does not order
  its four wheels front-left-first. Point this at raw \code{/joint\_states}
  and MINS silently reads the wrong wheel; \code{wheel\_remap.py} exists
  purely to fix the order (\S\ref{ap:repro-ourcode}), and \code{sub\_topics}
  must name-match that node's own \code{\textasciitilde front\_left\_name}
  and \code{\textasciitilde front\_right\_name} parameters. Omitting
  \code{sub\_topics} is a silent-drop trap in its own right: MINS's loader
  defaults it to generic joint names this rover does not use, the remapped
  topic keeps publishing (visible on \code{rostopic hz}), and MINS's internal
  wheel buffer simply never fills, which looks like an initialisation problem
  rather than a name mismatch.
\end{itemize}

\noindent\textbf{Wiring it up.} Two more pieces complete the installation,
covered fully in \S\ref{ap:repro-launcharch}:
\begin{itemize}[leftmargin=1.4em]
\item The config profile is selected by \code{mins.launch}'s \code{config}
  argument, passed to the estimator as \code{args="\$(find mins)/config/\$(arg
  config)/config.yaml"}. It must be \code{args}, never a
  \code{<param name="config\_path">}: the parameter read exists only in
  MINS's ROS2 build, and on ROS1 that block is compiled out entirely
  (\S\ref{ap:repro-argsparam}). Getting this wrong does not fail clearly:
  MINS reports it cannot open a file whose name was never typed.
\item MINS's config files are parsed with OpenCV's \code{FileStorage}, not a
  general YAML parser: a long inline comment placed after a value on the same
  line aborts the file silently, and the node then starts, subscribes to
  nothing, and respawns forever with no error naming the file
  (\S\ref{ap:repro-yamltrap}). If a comment is added or edited in any
  \code{config\_*.yaml}, keep it short or put it on its own line, and
  validate before restarting anything:
\begin{lstlisting}[language=bash]
cd ~/TOUT/mins_ws/src/MINS/mins/config/leo
python3 -c "import cv2; f=cv2.FileStorage('config_wheel.yaml', \
cv2.FILE_STORAGE_READ); print('readable:', f.isOpened()); f.release()"
\end{lstlisting}
\end{itemize}
\noindent Finally, three companion nodes must be running or the topics these
config files name never publish: \code{imu\_sanitizer.py} (feeds
\code{config\_imu.yaml}'s topic), \code{wheel\_remap.py} (feeds
\code{config\_wheel.yaml}'s), and, once Stage~3 is reached,
\code{carolus\_vicon\_bridge.py} (feeds \code{config\_vicon.yaml}'s). All
three start automatically as part of \code{navigation\_supervision.launch},
included by the same \code{navigation\_master.launch} that starts MINS itself
(\S\ref{ap:repro-launcharch}); nothing further needs to be launched by hand.

\paragraph{Calibrate.}
Three measurements feed the config files above; two are already done and
versioned, one is not.
\begin{enumerate}[leftmargin=1.4em]
\item \textbf{IMU sanitisation (already automatic).} \code{imu\_sanitizer.py}
  applies a fixed accelerometer scale correction ($\times 1.1186$, measured
  stationary against local gravity) and re-measures gyroscope bias at every
  startup (\S\ref{ap:repro-ourcode}). Nothing to run by hand: this happens
  every time the stack starts, before MINS ever sees a sample.
\item \textbf{Wheel radius and track (\code{config\_wheel.yaml}'s
  \code{intrinsics}).} Required again only if the wheels themselves change
  (different tyres, a different rover). Measured passively, with no commanded
  motion from the script itself:
\begin{lstlisting}[language=bash]
cd ~/TOUT
python3 tools/calib_roues_rigides.py
\end{lstlisting}
  Then, from the cockpit or a joystick, drive the rover \textbf{straight for
  about 3\,m} (estimates the radius), then \textbf{spin in place for about
  two full turns} (estimates the track), and press \code{Ctrl-C}. The script
  prints the left/right radius, the track, and an \code{intrinsics:} line
  ready to paste into \code{config\_wheel.yaml}.
\item \textbf{Camera--IMU extrinsic (\code{config\_camera.yaml}'s
  \code{T\_imu\_cam}).}
\begin{trapbox}{Honest status: not yet completed on this platform}
The procedure below is fully scripted and ready to run, but the excitation
motion it needs (physically lifting and tilting the rover in front of the
target) has \textbf{not} been performed as of this writing
(Table~\ref{tab:openitems}, item~2). The deployed extrinsic is a
tape-measure seed. \textbf{Updated 2026-09-07:} it is no longer refined
online. \code{do\_calib\_ext} is now \code{false} in \emph{both}
\code{config\_camera.yaml} and \code{config\_wheel.yaml}, and openVINS's
equivalent (\code{calib\_cam\_extrinsics}) is likewise \code{false}, because
online estimation of an \emph{unobservable} quantity was measured to random
walk at ${\approx}1.4$\,mm/min with the rover stationary --- eventually
pushing wheel and visual updates outside the $\chi^2$ gate and costing the
$X/Y$ anchor entirely (\S\ref{sec:openitems}, item~1 of the 2026-09-07
entries).

This inverts the priority stated in earlier revisions of this paragraph.
While the seed was refined online, completing the Kalibr calibration was a
precision improvement; now that nothing is refined, \textbf{the seed is the
final value}, its quality sets steady-state accuracy directly, and there is
no second net. Completing it is the single highest-value calibration task
remaining on this platform.
\end{trapbox}
  Kalibr lives in its own workspace, since its build requirements conflict
  with both estimators':
\begin{lstlisting}[language=bash]
mkdir -p ~/TOUT/kalibr_ws/src && cd ~/TOUT/kalibr_ws/src
git clone https://github.com/ethz-asl/kalibr.git
cd ~/TOUT/kalibr_ws && catkin build
source devel/setup.bash
\end{lstlisting}
  With Kalibr built, two scripts do the rest, in order:
\begin{lstlisting}[language=bash]
tools/calib_imucam_record.sh [duration_s]   # default 90s, records a bag
tools/calib_imucam_run.sh <bag>             # kalibr's two calib steps
\end{lstlisting}
  \textbf{The recording step is the one that fails if driven, not held.}
  Kalibr needs all three rotation axes excited in front of the checkerboard
  target; a rover rolled flat along the ground stays planar and roll/pitch
  are never observed, so the calibration fails silently. Pick the rig up and
  tilt it by hand instead. The run step needs no supervision: it executes
  \code{kalibr\_calibrate\_cameras} then \code{kalibr\_calibrate\_imu\_camera}
  back to back, $10$--$40$ CPU-heavy minutes depending on bag length.
  \textbf{Expected:} a \code{*-camchain-imucam.yaml} and a report PDF;
  \textbf{accept only if} the reprojection error is $<0.5$\,px and
  \code{timeshift\_cam\_imu} is a few milliseconds, not a fraction of a
  second. Then, specifically for MINS (the companion
  \code{calib\_imucam\_apply.sh} script installs a result into
  \emph{openVINS's} config only, by design, so the two estimators' opposite
  conventions are never mixed in one script):
\begin{lstlisting}[language=bash]
python3 catkin_ws/src/leo_navigation/scripts/fill_mins_camchain.py \
        <bag>-camchain-imucam.yaml \
        mins_ws/src/MINS/mins/config/leo/config_camera.yaml
\end{lstlisting}
  This inverts Kalibr's \code{T\_cam\_imu} into the \code{T\_imu\_cam}
  convention MINS expects and copies the time offset; a direct copy without
  inverting produces a plausible-looking, wrong geometry.
\end{enumerate}
\noindent \textbf{A fourth measurement, optional but valuable:} the
ground-truth floor protocol of \S\ref{ap:repro-calibrun} validates
\code{gyro\_scale} and cross-checks the wheel radius against a tape measure
and a 3-4-5 triangle rather than against another estimator. It is not
required for MINS to run, but it is the only procedure in this guide that
compares any of the above to physical reality rather than to another sensor.

\paragraph{Test.}
\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Launch.} From \code{\textasciitilde/TOUT}:
\begin{lstlisting}[language=bash]
bash tools/restart_stack.sh
\end{lstlisting}
  \textbf{Expected:} \code{stack relancée}, then
  \code{MINS initialisé après <N>s} (typically $8$--$12$\,s), then
  \code{TERMINE}. Unlike openVINS, MINS needs no jerk to start: it
  initialises standing still (\S\ref{ap:repro-launch}).
\item \textbf{Confirm the node registered:}
\begin{lstlisting}[language=bash]
source /opt/ros/noetic/setup.bash
rosnode list | grep mins_subscribe
\end{lstlisting}
\item \textbf{Confirm it is actually publishing, at a healthy rate.} A
  registered node can be completely silent; this is the real test:
\begin{lstlisting}[language=bash]
rostopic hz /mins/imu/odom          # expect ~80-90 Hz
\end{lstlisting}
  Nothing at all after $15$--$20$\,s usually means one of the config files
  above failed to parse (\S\ref{ap:repro-yamltrap}: check
  \code{logs/navigation\_master.log} for a respawn loop) or a companion node
  (\code{imu\_sanitizer.py}, \code{wheel\_remap.py}) is not running.
\item \textbf{Validate correctness, not just aliveness.} A silent estimator
  is an obvious failure; a wrong one that publishes cleanly at $85$\,Hz is
  not. Protocol T1.1 (\S\ref{ap:repro-t11}) is the cheapest test that tells
  the two apart: log \code{/robot\_pose\_fused} for five minutes with the
  rover completely still,
\begin{lstlisting}[language=bash]
cd ~/TOUT/tools
python3 log_pose_static.py 300 /tmp/t1_1_static_pose.csv
python3 analyze_pose_variance.py /tmp/t1_1_static_pose.csv
\end{lstlisting}
  \textbf{Pass:} linear drift below $1$\,mm/s on every axis. A fresh install
  that passes this has a correctly wired MINS. One that fails it almost
  always has a configuration or extrinsic problem, not a filter-gain problem
  (\S\ref{ap:repro-t11}).
\item \textbf{Validate under motion, once the above passes.} Drive a closed
  loop and compare MINS against openVINS on the same run, following
  \S\ref{ap:repro-comprun} end to end. That procedure also covers switching
  the cockpit's \emph{Pose Source} to MINS and recording both estimators to a
  rosbag, which the website's own buffers are not reliable enough to replace.
\item \textbf{Validate against a measured rectangle, for an absolute
  dimensional check.} Protocol T1.1 catches a wrong estimator standing
  still; \S\ref{ap:repro-comprun} catches disagreement \emph{between} MINS
  and openVINS while moving. Neither compares either one to a physical,
  tape-measured shape. This one does, using the cockpit's own \emph{Test 1
  / Test 2} recorder on the Trajectory page rather than a terminal command,
  so it is the version of this test an operator without shell access can
  still run.

  \textbf{How the recorder actually works, since it looks simpler than it
  is.} Clicking \emph{Test 1} or \emph{Test 2} does two things at once: it
  labels the run, and it starts a \code{rosbag record} process that
  \code{leo\_backend.py} launches \emph{detached} from itself, writing
  straight to disk. That detachment is the entire point: the cockpit's own
  in-memory trajectory buffer is lost on every backend restart, which on
  this platform is common enough (thermal throttling, the watchdog, a
  crashed estimator) that a run relying on it can silently lose data
  mid-drive with no visible symptom. A detached recording keeps writing
  regardless, and \emph{Export to MATLAB} later reads whichever is more
  authoritative: the finished recording on disk if one exists for that test
  label, the in-memory buffer only as a fallback if no robust recording was
  ever started for it. Both estimators are written to that same recording
  unconditionally, whether MINS or openVINS is the one currently shown on
  screen: the Test~1/Test~2 label only decides the filename prefix the
  eventual export uses, never what gets captured. This is exactly the
  mechanism validated on 2026-07-29 with a real pair of drives around the
  lab (\S\ref{sec:july27twotest}): read that section first if the numbers
  below look implausible on a first attempt, since it shows what a clean
  pair of exports and a trustworthy verdict actually look like on this
  platform, drift and all.
\begin{enumerate}[leftmargin=1.2em]
\item \textbf{Mark a rectangle on the floor.} Tape one of known dimensions,
  measured with a tape measure, not by eye; $2.00\,\text{m} \times
  1.50\,\text{m}$ fits most lab spaces, but use whatever the room allows.
  Mark the starting corner and the direction of travel: you will start and
  finish there, facing the same way, so the loop closes.
\item \textbf{On the Trajectory page, click \emph{Test 1}.} This both
  labels the run and starts a rosbag recording detached from the browser
  (\S\ref{ap:repro-comprun}: it survives a backend restart, unlike the
  page's own buffer). MINS and openVINS are captured together regardless
  of which one is on screen, but by convention Test 1 is the lap whose
  \emph{MINS} trace gets trusted below.
\item \textbf{Drive the rectangle once}, corner to corner, back to the
  marked start point and heading. Slowly and smoothly; a sharp turn at
  each corner is not required, since the analysis below reads the
  trajectory's overall extent, not its individual corners.
\item \textbf{Click \emph{Export to MATLAB}.} This writes
  \code{web/exports/test1\_mins.csv}, \code{test1\_vins.csv} and
  \code{test1.mat}, and, if MATLAB is available on the workstation,
  launches \code{plot\_trajectories('test1.mat')} automatically: a useful
  MINS-versus-openVINS sanity check on this one lap, but not yet the
  rectangle comparison below.
\item \textbf{Run the rectangle comparison, for MINS:}
\begin{lstlisting}[language=bash]
cd ~/TOUT/tools
matlab -batch "plot_rectangle_test('../web/exports/test1', 2.00, 1.50, 'mins')"
\end{lstlisting}
  \textbf{Expected:} a printed block with the measured dimensions, the
  trajectory's bounding box, the percentage error on each side, and the
  loop-closure gap, plus a saved \code{test1\_mins\_rect.png} overlaying
  the trajectory on the ideal rectangle. For context, not a pass/fail
  threshold: this platform's own wheel odometry alone over-reports
  straight-line distance by $39\%$ uncorrected (\S\ref{ap:repro-wheels});
  a correctly wired estimator should track the measured rectangle far more
  closely than that.
\end{enumerate}
\begin{casestudy}{Run 2026-08-05: closed, against the taped rectangle below}
$6.405\,\text{m}\times3.455\,\text{m}$, taped and tape-measured. Test 1,
MINS trusted. Figure~\ref{fig:aug05mins} and \S\ref{sec:aug05result} give
the full result and its interpretation; in short, bounding box
$6.176\times3.919$\,m ($-3.6\,\%$ length, $+13.4\,\%$ width), loop closure
$0.449$\,m over a $23.35$\,m path ($1.9\,\%$). The asymmetry between the two
axis errors is not noise: \S\ref{sec:aug05calib} ties it to a measured
turning asymmetry on this platform, not to a uniform scale error, which is
the reading the raw bounding-box numbers alone cannot give.
\end{casestudy}
\end{enumerate}

\subsection{The same test, for openVINS}
\label{ap:repro-rect-vins}
openVINS has no dedicated ``start to finish'' walkthrough in this appendix
the way MINS now does (\S\ref{ap:repro-mins-guide}); its build and launch
are covered in \S\ref{ap:repro-build} and \S\ref{ap:repro-launch}. What
follows is only the test methodology, mirrored from the MINS protocol
above using the cockpit's \emph{Test 2} slot, because that is specifically
what was asked for.

\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Wake openVINS before starting Test 2}, with a jerk
  (\S\ref{ap:repro-launch}): it publishes nothing at all until it has felt
  one, and Test 2's whole purpose below is to be the lap whose
  \emph{openVINS} trace is trusted.
\item \textbf{On the Trajectory page, click \emph{Test 2}.} If Test 1 is
  still recording, the cockpit asks to finalise and save it first: confirm,
  and it does so before starting Test 2.
\item \textbf{Drive the rectangle a second time}, corner to corner, back to
  the same marked start point and heading. This is a genuinely independent
  lap, not a re-analysis of Test 1's data: driving it twice is the point,
  since \S\ref{ap:repro-rect-compare} below compares the two laps to each
  other, not two estimators reading the same one.
\item \textbf{Click \emph{Export to MATLAB}} again. This writes
  \code{web/exports/test2\_mins.csv}, \code{test2\_vins.csv} and
  \code{test2.mat}, overwriting nothing from Test 1: the files are named by
  test label, not by timestamp.
\item \textbf{Run the rectangle comparison, for openVINS:}
\begin{lstlisting}[language=bash]
cd ~/TOUT/tools
matlab -batch "plot_rectangle_test('../web/exports/test2', 2.00, 1.50, 'vins')"
\end{lstlisting}
  \textbf{Expected:} the same style of report as MINS's (measured
  dimensions, achieved bounding box, percentage error per side, loop-closure
  gap), saved to \code{test2\_vins\_rect.png}. Use the \emph{same}
  \code{length\_m}/\code{width\_m} arguments as the MINS run: both laps are
  being measured against the identical physical rectangle, which is what
  makes \S\ref{ap:repro-rect-compare} below meaningful.
\end{enumerate}
\begin{trapbox}{Run 2026-08-05: executed, and openVINS did not close the
loop}
Test 2 was recorded against the identical rectangle, in the same session, in
the same rosbag pipeline as the MINS run above. Unlike that run, this one is
not a validation: openVINS returned a $675.28\times96.01$\,m bounding box and
a $2760.55$\,m path against a $19.72$\,m measured perimeter. The full
diagnosis --- a rewritten pose-fusion guard, a diagnosed IMU tilt, a thermal
envelope this session sat inside rather than beside, and the algebraic proof
that no single scale factor can rescue this trajectory --- is
\S\ref{sec:aug05guard}--\S\ref{sec:aug05result}, not repeated here. What
belongs here is the honest status: the procedure above is correct and was
followed exactly; its output on this platform, this day, is not a usable
openVINS figure. \S\ref{ap:repro-hitrun} and a re-run once the platform is
cooled and lit remain the way to get one.
\end{trapbox}

\subsection{MINS versus openVINS, against the same measured rectangle}
\label{ap:repro-rect-compare}
Two independent laps now exist, each measured against the same physical
rectangle. Two different comparisons are worth running on them: one
already exists in this project for exactly this two-lap situation, the
other is the dimensional check Hector asked for specifically.

\paragraph{The existing tool: agreement between the two laps.}
\S\ref{ap:repro-comprun} already introduces \code{compare\_tests.m} in
passing, for two independent drives labelled Test~1 and Test~2; here it is
in full, on this pair:
\begin{lstlisting}[language=bash]
cd ~/TOUT/tools
matlab -batch "compare_tests('../web/exports/test1', '../web/exports/test2')"
\end{lstlisting}
By design, it reads Test 1's \emph{MINS} trace and Test 2's \emph{openVINS}
trace specifically (not both traces of each), and reports trajectory
shape, path length, loop-closure error and final-versus-initial heading:
the right metrics for two separate roulages, since there is no shared
timestamp to align them point-by-point the way \S\ref{ap:repro-comprun}'s
Kabsch recalage does for a single simultaneous drive.

\paragraph{The new comparison: both against the tape measure.}
\begin{enumerate}[leftmargin=1.4em]
\item \textbf{Put both rectangle results side by side.} From the two
  console outputs of the subsections above (or the two \code{\_rect.png}
  annotations), tabulate: the measured length and width (identical for
  both, it is the same physical rectangle), each estimator's achieved
  length and width, each estimator's percentage error on each side, and
  each estimator's loop-closure gap.
\item \textbf{Read the percentage errors before the closure gap.} A small
  closure gap with a large dimensional error is possible: an estimator
  that consistently under-scales the whole trajectory can still return
  close to its own starting point, because the scale error cancels out
  over a closed loop. The dimensional comparison is what catches that;
  \S\ref{sec:july29align} makes the same point about scale hiding inside
  an alignment-based metric.
\item \textbf{Whichever estimator is closer on both dimensions} is the one
  with the more accurate metric scale on this platform, on this floor, on
  these two laps. This is not a general verdict from a single pair of
  drives, particularly since Test 1 and Test 2 are two different physical
  laps, not the same motion measured twice: repeat in both directions
  around the rectangle before trusting the comparison, the way
  \S\ref{ap:repro-calibrun} insists on three passes for the ground-truth
  floor protocol before applying anything.
\end{enumerate}
\begin{casestudy}{Run 2026-08-05: both tests exist, and the comparison is not
a fair one}
\begin{center}
\begin{tabular}{lrr}
\toprule
...
Heading drift  & $5.1^\circ$ & $5.9^\circ$ \\
\bottomrule
\end{tabular}
\end{center}
Both figures came from the same \code{compare\_tests.m} and
\code{plot\_rectangle\_test.m} calls this appendix specifies, against the
identical physical rectangle, in the same session. Read plainly, they are
not close: openVINS's dimensional error runs three orders of magnitude past
MINS's. \textbf{Do not report this as ``MINS outperforms openVINS by
$100\times$.''} That framing implies two comparable algorithmic results,
one merely worse than the other, and \S\ref{sec:aug05result} shows it is not
that: MINS completed a valid acquisition, openVINS's acquisition failed for
reasons independently diagnosed and platform-specific --- a thermal envelope
this session sat inside (\S\ref{sec:aug05thermal}), IMU gaps at $80\,\%$ of
the automated abort threshold, and an uncorrected mount tilt
(\S\ref{sec:aug05tilt}) doubly integrating into exactly the kind of drift
openVINS has no wheel odometry to resist. The near-identical heading drift
between the two ($5.1^\circ$ vs $5.9^\circ$) is the tell: an estimator whose
orientation tracking were fundamentally broken would show it here too, and
neither does. What this comparison honestly establishes is narrower and
still useful: MINS is validated end to end on this platform, against a
physical reference, today; openVINS is not yet, and \S\ref{tab:openitems}
records what a fair re-attempt requires --- a cooled Pi, a lit floor, and
ideally the still-outstanding Kalibr camera--IMU extrinsic
(\S\ref{ap:repro-calib}).
\end{casestudy}
...
(\code{do\_calib\_int: false}). \textbf{Updated 2026-09-07:} extrinsic
refinement is now off as well --- openVINS carries
\code{calib\_cam\_extrinsics: false} and \code{calib\_cam\_timeoffset: false},
so nothing about the camera--IMU geometry is estimated online any more. Only
MINS still refines the camera time offset (\code{do\_calib\_dt: true}); its
wheel branch has both frozen (\code{do\_calib\_ext: false},
\code{do\_calib\_dt: false}).

\paragraph{MINS.} A per-sensor tree of \textbf{ten} files, selected by the
\code{config} launch arg (\code{mins.launch}:
\code{<arg name="config" default="leo"/>}), under
\code{mins\_ws/src/MINS/mins/config/leo/}. MINS initialises \emph{standing
still} (static IMU+wheel init, 4--12\,s, \S\ref{sec:ovinit}), which is why it
is the default served source. Its transform output is isolated to
\code{/tf\_mins} (\code{mins.launch}, \S\ref{sec:chain}); the Carolus global
fix enters through the VICON slot on \code{/mins/external\_ref/carolus}.
\textbf{For the full file-by-file listing, what in each file is
platform-specific, and a complete download-to-test walkthrough, see
\S\ref{ap:repro-mins-guide}.}
...
\item \textbf{Camera--IMU extrinsics + time offset.} With the intrinsic
  ...
  translation \emph{were} refined online (\code{do\_calib\_ext},
  \code{do\_calib\_dt}) until 2026-09-07; extrinsic refinement is now
  disabled, so only $t_d$ is still estimated, and the translation seed stands
  as installed. The $90^\circ$ rotation must in any case be seeded correctly:
  it is outside the filter's recoverable region, and always was.
\item \textbf{Install.} Copy the Kalibr outputs into each estimator's config:
  ...
  characterisation of the \emph{body} IMU carried on the CORE2 control board
  (or the manufacturer figures inflated for margin). That is the only
  inertial source any estimator consumes, along the chain
  \code{/firmware/imu} $\rightarrow$ \code{/imu/data\_raw} $\rightarrow$
  \code{/imu/data\_clean} (and \code{/imu/data\_vins} for the two VINS
  branches).

  \emph{It is not the D455's own IMU.} That distinction matters because the
  two are not interchangeable: they sit in different frames, and the
  camera--IMU extrinsic $^{I}\mathbf{T}_{C}$ calibrated here is the transform
  to the \emph{body} IMU. The D455's motion module is physically present ---
  it enumerates as a USB HID interface and the RealSense driver reports
  \code{Motion Module was found} --- but it is disabled in the driver, and
  the streams it would publish on \code{/camera/imu} have never been used by
  this project (Table~\ref{tab:openitems}).
...
\begin{trapbox}{What that public tunnel was also publishing (audit, 2026-08-18)}
\code{web/serve.py} subclassed \code{SimpleHTTPRequestHandler} and overrode
only the cache headers --- so it served the \emph{whole} directory, directory
indexes included, and that directory is on the public internet. Verified
live, not assumed:
\code{https://cockpit.leo-rover-gardon.dev/exports/} answered \code{200} with
a browsable index of $104$ files ($17$\,MB of experiment data: \code{.mat},
trajectory CSVs, figures), as did \code{models/} and \code{reports/}. Eight
\code{.bak-cachefix-*.html} pages from July 29 --- complete, stale copies of
the cockpit --- were being served next to the real ones, plus thirteen more
backups whose names do \emph{not} begin with a dot
(\code{ops.html.bak-avant-v4-161908}, \code{app.js.bak-casse-203841}), which
is why a naive ``hide dotfiles'' rule is not enough.

The fix keeps the deny list aligned with section 10 of \code{.gitignore}, on
the principle that \textbf{what git refuses to archive, the server refuses to
publish}: any path segment starting with a dot, or carrying \code{.bak},
\code{.orig}, \code{.rej} or \code{.swp}, returns $404$; directory listing is
disabled outright. \code{send\_head()} still serves \code{index.html}, so
pages are untouched, and individual files under \code{exports/} remain
reachable by direct URL --- only enumeration disappears. Sixteen cases were
unit-tested before deployment and re-checked through the public tunnel
afterwards.
\end{trapbox}

\noindent \textbf{Opening and closing that exposure from the cockpit.} The
\emph{Public tunnel} control in Operations $\rightarrow$ Commands starts and
stops \code{cloudflared} through the \code{tunnel} action of
\code{leo\_backend.py}. Killing the process is \emph{not} enough on its own:
\code{leo\_watchdog.sh} sees it dead within the minute and replays
\code{web/start\_web.sh}, which brings it straight back --- the operator would
conclude the button was broken. A closure therefore has to be \emph{declared},
not merely executed, so the backend drops \code{/tmp/leo\_tunnel\_off} and both
scripts consult it before restarting anything. The flag deliberately lives in
\code{/tmp}: a reboot clears it and restores public access, which is the less
surprising behaviour of the two (nobody wants a site silently dark because of
a click three weeks earlier). The telemetry carries \code{up} and
\code{disabled} separately, because ``not up'' alone cannot distinguish a
deliberate closure from a crash the watchdog is already repairing --- the
cockpit shows three states accordingly. Closing from a page itself loaded
through the tunnel is allowed, but warns in as many words that this page is
about to disconnect and that reopening will require the local address.
Verified end to end: closed (public \code{530}, local \code{200}), still closed
after $95$\,s and a full watchdog tick, then reopened (public \code{200}).

\section{The handoff package: helping another lab reproduce this setup}
\label{ap:handoff-package}

This section documents \code{leo\_rover\_pour\_connor.zip}, a self-contained
archive built to answer a specific, recurring request: \emph{another team has
the same hardware, their estimator will not stay stable, and they need a
known-good reference to compare against}. It was first assembled in August 2026
for a colleague at Florida Tech whose MINS instance diverged within seconds of
startup, and the same package answers the same question for anyone who follows.

\noindent \textbf{When to send it.} Use it when someone reports an estimator
that goes unstable \emph{immediately} rather than drifting slowly. That symptom
is almost never a defect in MINS or openVINS; it is a configuration or
calibration mismatch, and the fastest resolution is a line-by-line
\code{diff} against configuration files that are known to run. Prose describing
the settings is far slower and far less reliable than the files themselves.

\subsection{What is inside, and why each piece is there}

\begin{lstlisting}[language=bash]
leo_rover_pour_connor.zip
  README.txt                     <- read this first, English
  config_MINS_leo/               <- the 7 live MINS config files
    config.yaml
    config_imu.yaml
    config_camera.yaml
    ...
    config_system.yaml
  documentation/
    main.pdf                              <- this report
    patches_sqrtVINS_README.md
    patches_openVINS_README.md
    rpng_sqrtvins_zupt_bug_report.md
    robot_sqrtVINS_config_README.md
\end{lstlisting}

\noindent \textbf{\code{config\_MINS\_leo/}} holds the seven MINS configuration
files exactly as they run on this rover, copied from
\code{LEO\_Rover\_Navigation\_System/MINS-master/mins/config/leo/}. These are the
core of the package. The recipient should not adopt them wholesale --- their
extrinsics and noise densities belong to \emph{this} unit --- but should diff
them against their own and examine every difference deliberately.

\noindent \textbf{\code{documentation/}} holds the reasoning behind the values.
\code{main.pdf} is this report, whose calibration sections explain the
camera--IMU extrinsic convention and the inertial calibration limits. The two
\code{patches\_*} files document defects found in the estimators themselves,
with measured before-and-after numbers; they matter because a recipient chasing
a divergence may be chasing a known upstream bug rather than their own
configuration. \code{rpng\_sqrtvins\_zupt\_bug\_report.md} is the ZUPT bug
report filed upstream. \code{robot\_sqrtVINS\_config\_README.md} explains, value
by value, why each embedded parameter is set as it is --- useful as a model of
the reasoning even for a reader who never runs sqrtVINS.

\noindent \textbf{Everything in the archive is written in English}, including
the comments inside the YAML files. This is deliberate: the working language of
this repository is French, and an archive that leaks French comments into
another lab's configuration is a package that will be misread. If you rebuild
the archive, preserve that property.

\subsection{The five causes worth checking first}

The package is most useful when paired with a diagnostic order. Every case of
immediate instability diagnosed on this platform so far traced back to one of
the following. They are listed in the order worth testing --- cheapest and most
frequently implicated first --- not in order of theoretical interest:

\begin{enumerate}
  \item \textbf{Camera--IMU extrinsic, direction and frame convention.}
  Kalibr emits \code{T\_cam\_imu} (points in the IMU frame mapped into the
  camera frame); MINS expects the \emph{inverse}. Loading one where the other
  is expected raises no error --- it feeds the filter a wrong rotation from the
  first frame, and divergence follows within seconds. A single axis flip, or an
  optical-frame versus body-frame mismatch, produces the same signature. This
  is the most common cause by a wide margin.

  \item \textbf{Static initialization window.} MINS estimates its initial
  gravity direction and gyroscope bias from a stationary period at startup.
  If the rover is handled, bumped, or still settling during that window, the
  alignment locks in tilted, gravity leaks into the horizontal axes, and the
  doubly integrated residual produces an \emph{accelerating} drift. Measured on
  this rover after an initialization disturbed by hardware work in progress:
  772\,m/min of vertical drift with the robot standing still.
  A clean restart with the platform untouched brought the same estimator to
  0.02\,m/min horizontal and 0.02\,m/min
  vertical --- a factor of roughly $3\times10^{4}$, from one variable.

  \item \textbf{Time alignment between camera and IMU.} Tens of milliseconds
  are enough to break the tight coupling. Check that a time offset is set
  explicitly, or that online estimation of it is enabled, particularly when the
  two sensors are served by separate drivers.

  \item \textbf{IMU noise densities and random walks.} Values taken from a
  datasheet rather than an Allan-variance characterization of the specific unit
  leave the filter overconfident: small residuals are amplified into large
  corrections instead of being absorbed. The failure does not look like a typo;
  it looks like a broken estimator.

  \item \textbf{Publication rates and dropped frames.} A degraded IMU rate or
  intermittently dropped images produces the same early instability and costs
  nothing to rule out with \code{rostopic hz} on both streams.
\end{enumerate}

\begin{trapbox}{Check the cheap cause before the expensive one}
Cause 2 --- an initialization that was not static --- costs a single restart to
test. Cause 1 requires reading and re-deriving a transform. Test in that order,
not in order of intellectual interest. On the evening of 2026-08-31, roughly
forty minutes were spent verifying inputs that all turned out to be healthy
(IMU magnitude within 0.7\,\% of local gravity, wheel joint states at
19.5\,Hz with no gap above 0.10\,s, camera images with a pixel standard
deviation near 31, and every MINS configuration file byte-identical to the
committed version) before the initialization window was even considered.
\end{trapbox}

\subsection{Rebuilding the archive}

The archive is not tracked in Git; it is regenerated on demand so that it always
carries the configuration currently in service. To rebuild it, stage the seven
MINS configuration files and the five documents listed above, add an English
\code{README.txt} describing each directory, and compress the result. Two checks
before sending:

\begin{lstlisting}[language=bash]
# 1. every YAML must still parse under OpenCV FileStorage,
#    which is stricter than a standard YAML parser
python3 -c "import cv2,sys; fs=cv2.FileStorage(sys.argv[1],0); \
            print(fs.isOpened())" <file>

# 2. no French must remain in any text file
LC_ALL=C grep -rlP "[\\x80-\\xFF]" <staging directory>
\end{lstlisting}

\noindent The second check exists because the first version of this archive
shipped with three README files and two YAML comment blocks still in French,
which would have forced the recipient to translate the very material meant to
save them time.

\subsection{What the package deliberately does not contain}

No source code, no build artefacts, and no workspace. The recipient is expected
to have a working MINS build already --- their problem is that it runs and
diverges, not that it does not run. Including a workspace would add gigabytes,
would embed absolute paths from this machine, and would invite the recipient to
replace a build that is probably fine. The archive is a \emph{reference to
compare against}, not an installation.

\noindent It also contains no bag file. If a diff of the configuration files
does not resolve the problem, the correct next step is to ask the recipient for
a short recording of \emph{their} system --- ten to fifteen seconds covering the
static initialization window and the first motion --- which localizes the fault
far faster than exchanging descriptions of it.



\section{Running MINS on the robot: a feasibility audit, and why the answer is no}
\label{ap:mins-embedded}

The Lab holds two complementary configurations: a local MINS running on the
robot's own computer, and the LEO configuration documented here, where MINS
runs on a ground workstation. This section records the measurements behind
that split, so the decision is auditable rather than assumed --- and so that a
future student with faster embedded hardware can re-run the arithmetic instead
of re-deriving it.

\subsection{What was compared}

A colleague supplied a MINS tree (referred to here as \code{MINS\_MC}) with the
suggestion that disabling dynamic cloning would let MINS run natively on the
Raspberry Pi. Inspecting that tree gave three findings worth recording.

\noindent \textbf{First, the tree is an unmodified upstream clone} of
\code{rpng/mins}, not a tuned configuration. Its only substantive change is
\code{mins/launch/subscribe.launch}, converted from rosbag playback to live
topic subscription with remaps --- a genuinely useful piece of work, and the
part worth adopting.

\noindent \textbf{Second, the advice has a real provenance.} The upstream
\code{Robomaster\_S1} profile ships with exactly \code{clone\_freq: 30} and
\code{dynamic\_cloning: false}. The suggestion was not invented; it was read
off a reference profile for a comparable small ground robot.

\noindent \textbf{Third, and this is the trap:} that profile and ours are tuned
in opposite directions on different axes.

\begin{center}
\begin{tabular}{lll}
\toprule
Parameter & \code{Robomaster\_S1} & our \code{leo} \\
\midrule
cameras & mono, \code{use\_stereo: false} & stereo, two cameras \\
\code{downsample} & true & false \\
\code{do\_calib\_ext} / \code{do\_calib\_dt} & false / false & false / true$^{\dagger}$ \\
resolution & $640\times360$ & $640\times480$ \\
\code{n\_pts} & 1500 & 300 \\
grid & $15\times15$ (225 cells) & $10\times8$ (80) \\
\code{clone\_freq} & 30 & 20 \\
\code{dynamic\_cloning} & false & true \\
\bottomrule
\end{tabular}
\end{center}

\noindent $^{\dagger}$ \textbf{Changed 2026-09-07.} This row read
\code{true / true} in earlier revisions. \code{do\_calib\_ext} was set to
\code{false} after the extrinsic was measured random-walking at
${\approx}1.4$\,mm/min with the rover stationary --- the quantity is
unobservable without motion, and the drift eventually pushed updates outside
the $\chi^2$ gate. Note the honest consequence for the argument below: on
\emph{this} row the two profiles now agree rather than oppose, and the
contrast that follows rests on the remaining rows.

The RoboMaster profile saves heavily on the image path --- mono, downsampled,
no online calibration --- and spends that budget on tracking: 1500 features,
225 grid cells, a fixed 30\,Hz clone rate. Ours does the reverse: an expensive
image path, but only 300 features and an adaptive clone rate. Since
2026-09-07 the two profiles also share the frozen-extrinsic choice, though
they reached it for different reasons: RoboMaster as a compute saving, ours
as a stability fix.

\begin{trapbox}{Transplanting two parameters out of a balanced profile}
Lifting \code{clone\_freq: 30} and \code{dynamic\_cloning: false} into the
\code{leo} profile \emph{without} also adopting mono and \code{downsample}
combines the expensive half of both profiles. The settings are not wrong; they
are load-bearing for a configuration whose other choices compensate for them.
Parameters copied between profiles must be copied as a set, or not at all.
\end{trapbox}

\subsection{The measurement}

\noindent \textbf{Cost on the workstation.} With the robot stationary and
dynamic cloning settled at its floor of 9\,Hz --- a best case, not a typical
one --- MINS consumes \textbf{85\,\% of one core} of an Intel i7-8700. Its own
timing breakdown reads CAM 18\,ms, IMU 2\,ms, wheels 0\,ms: \textbf{the camera
path is 90\,\% of the cost}, which is where any optimisation must act.

\begin{trapbox}{The 5.4$\times$ margin that is not a margin}
MINS prints \code{Total: 20ms / 111ms (5.4X)}. That figure covers only its
update path. Total process CPU is the 85\,\% measured externally --- image
callbacks, ROS deserialisation and the KLT front end are not counted in the
20\,ms. Reading 5.4$\times$ as available headroom overstates the budget by
roughly a factor of four, and would turn a no-go into an apparent go.
\end{trapbox}

\noindent \textbf{Relative performance.} Rather than assume a ratio, both
machines ran the same single-threaded floating-point benchmark: \textbf{436
versus 108} operations per second, so the Cortex-A72 is \textbf{4.05$\times$}
slower per core than the i7-8700. This benchmark likely flatters the Pi, since
its BLAS kernels are well optimised for NEON while MINS also runs OpenCV KLT
and Eigen on small matrices, where the x86 advantage is wider. A planning range
of 4--6$\times$ is therefore appropriate.

\noindent \textbf{The arithmetic.} At 4$\times$, MINS needs roughly
\textbf{3.4 of the Pi's 4 cores}; at 6$\times$, more than the machine has. Of
those four cores, about \textbf{2.3 are free} with the camera driver running.

\noindent \textbf{What settles it is thermal, not arithmetic.} The Pi already
idles at 70.6\,$^\circ$C with the throttle flag armed. Sustained load
drives it to the 80--85\,$^\circ$C threshold, where the clock drops
to 600\,MHz --- a further $\approx2.5\times$ loss, arriving exactly when the
margin is thinnest. This platform has already been observed becoming
unreachable under load. Placing the state estimator on the wrong side of that
failure mode is not a tuning question.

\subsection{Verdict, and what would change it}

\textbf{MINS stays on the ground station.} A \code{leo\_pi} profile exists
(\code{mins/config/leo\_pi/}) applying the three levers that matter --- mono,
\code{downsample: true}, online calibration disabled --- which are the same
three the upstream RoboMaster profile pulls. It may well run. It is
nonetheless not adopted, for two reasons stated plainly: the saving is
\emph{estimated, not measured}, and going mono removes stereo from a platform
where \textbf{the camera is the only sensor constraining $z$} (§\ref{ap:handoff-package}
records what happens when that constraint is lost).

The decision would be revisited on different hardware --- a Jetson-class
board --- not by tuning harder on this one.

\subsection{ROS 1, tf2, and the source changes that were already needed}

Two platform decisions are settled and should not be revisited.

\noindent \textbf{ROS 1 (Noetic) is structural, not preference.} Both machines
run Noetic exclusively. The binding constraint is the motor and sensor link:
the CORE2 board speaks to the Pi over \code{rosserial}, which exists only for
ROS 1. The ROS 2 equivalent is micro-ROS, which requires reflashing the
microcontroller firmware --- a hardware change, not a configuration one.
Everything downstream (the \code{rosmon} supervision, the rosbridge cockpit,
the three estimator workspaces) sits on that foundation.

\noindent \textbf{The ROS 1 source changes are the tf1 to tf2 migration, and
they are already done here.} Upstream MINS still uses the legacy
\code{tf::TransformBroadcaster} on its ROS 1 path. Four files carry it, and
all four were migrated to \code{tf2\_ros} in this tree on 2026-07-22:

\begin{center}
\begin{tabular}{lc}
\toprule
File (ROS 1 path) & legacy \code{tf} occurrences upstream \\
\midrule
\code{mins/src/core/ROSPublisher.cpp} & 12 \\
\code{mins/src/sim/SimVisualizer.cpp} & 5 \\
\code{mins/src/core/ROSPublisher.h} & 1 \\
\code{mins/src/sim/SimVisualizer.h} & 1 \\
\bottomrule
\end{tabular}
\end{center}

\noindent The \code{ROS2*} counterparts already used \code{tf2} upstream and
were unaffected. Anyone asking which files need source changes to run MINS
under ROS 1 is asking about these four.

\subsection{Which camera: RGB or the IR stereo pair}

For pose estimation on this platform the IR stereo pair is the correct choice,
for one theoretical and one hardware reason.

\noindent The theoretical reason is scale: a calibrated stereo pair observes
metric scale directly, where a monocular stream depends on the IMU and wheels
to recover it. Both openVINS and sqrtVINS support stereo --- \code{use\_stereo:
true}, \code{max\_cameras: 2}, sharing the same \code{TrackKLT} front end --- so
neither is limited to monocular, and both already run stereo here.

\noindent The hardware reason is specific to this unit and decides the matter
regardless: \textbf{the D455's colour stream fails RGB8 format negotiation on
this camera}, root-caused earlier in the project. The \code{infra1} mono8 feed
is the confirmed reliable stream and is what every calibration recording in
this work used. The cockpit displays greyscale IR for the same reason. The
colour stream is additionally disabled by default because enabling it costs the
camera nodelet 133\,\% of a core against 85\,\%.

\noindent An RGB-versus-IR accuracy comparison on this rover would therefore be
measuring a defective stream against a working one, not two valid options.
Repeating it on a unit with a healthy colour pipeline would be a fair test; on
this one it is not.
...
\subsection{``The robot is on but completely invisible'': four faults stacked}
\label{ap:repro-invisible}
An outage on August 18 cost several hours and is worth reproducing in full,
because the robot was healthy throughout: uptime $4$\,days $22$\,h, battery
$12.3$\,V, $60.8\,^\circ$C with \code{throttled=0x0}, and
\code{/firmware/wheel\_states} publishing at $15.8$\,Hz \emph{locally}. Four
independent faults were stacked, and each one masked the next.

\begin{enumerate}[leftmargin=1.4em]
\item \textbf{The rover falls back to its own access point.} When
  \code{wlan\_int} fails to join the campus network it stays \code{UP} with
  \emph{no address}, and the robot is then reachable only on \code{br0},
  \code{10.0.0.1}, behind SSID \code{LeoRover-e138}. An ICMP sweep of all
  $8192$ addresses of the campus \code{/19}, then a TCP sweep for ports $22$
  and $11311$, found nothing at all --- and mDNS cannot help, because that
  guest network blocks client-to-client multicast. \textbf{The test that
  settles it in five seconds is \code{nmcli dev wifi list} on the
  workstation}: if \code{LeoRover-e138} is there at full strength, stop
  scanning. Note that \code{10.0.0.1} --- already the default in
  \code{tools/robot\_env.sh} --- is in fact the \emph{most} robust address of
  the three, since \code{br0} is up regardless of which network the robot did
  or did not manage to join.
\item \textbf{The saved Wi-Fi profile was pinned to a replaced dongle.}
  \code{nmcli connection up LeoRover-e138} failed with a misleading message
  naming \code{eno1}, because \code{connection.interface-name} still held the
  name of the previous USB dongle. Cleared once and for all with
  \code{nmcli connection modify LeoRover-e138 connection.interface-name ""}.
\item \textbf{\code{/etc/ros/setup.bash} on the robot pins
  \code{ROS\_IP="10.200.0.2"}}, the WireGuard address. This is the nastiest
  of the four: once the rover is reachable over its access point the master
  answers, \code{rostopic list} works and the topics \emph{exist}, yet every
  robot-side node advertises a dead address, so no TCPROS connection can be
  made and everything looks mute. Confirm with \code{lookupNode}: an URI of
  the form \code{http://10.200.0.2:...} is the signature. The fix that avoids
  restarting the robot's entire stack is a persistent route on the
  workstation,
  \code{nmcli connection modify LeoRover-e138 +ipv4.routes "10.200.0.2/32 10.0.0.1"};
  the robot accepts packets addressed to its own \code{wg0} address arriving
  on \code{br0} ($0\,\%$ loss measured).
\item \textbf{Long-lived nodes keep publishing but never re-subscribe.}
  After the network transition \code{serial\_node} still had every
  \emph{outbound} connection alive (wheels, IMU, odometry) and \textbf{no
  inbound \code{/cmd\_vel}} --- the operator's report was precisely ``the
  commands no longer work'' while telemetry looked healthy. A brand-new
  publisher did not attract it either. \code{rosbridge\_server} failed the
  same way one layer up: the cockpit showed \code{ONLINE}, video ran (MJPEG
  over plain HTTP, independent of rosbridge), and every telemetry-driven
  field stayed blank --- \code{MISSION 00:00} against a backend counting
  $462$\,s, and, most visibly, \emph{no button selected} in either the pose
  source or the wheel selector. Both were cured by a targeted restart, the
  same \code{rosmon} stop/start the watchdog already performs for the camera.
\end{enumerate}

\begin{trapbox}{Two measurement traps met while diagnosing this}
\code{rostopic hz} buffers its output when redirected or piped: it prints
nothing while data is flowing perfectly. Always confirm with
\code{rostopic echo -n1} (add \code{--noarr} for images) before declaring a
topic dead --- this alone produced a false ``camera mute'' diagnosis here.

And \code{pkill -f "rosbridge\_websocket"} \textbf{kills its own shell}: the
pattern appears in that shell's own command line. Use explicit PIDs.
\end{trapbox}

\noindent \textbf{The decisive end-to-end test}, when several unrelated
cockpit fields are blank at once, is to open the WebSocket yourself rather
than to read JavaScript: connect to \code{ws://localhost:9090} and send
\begin{lstlisting}[language=bash]
{"op":"subscribe","topic":"/mission/telemetry","type":"std_msgs/String"}
\end{lstlisting}
then wait. A
\code{101 Switching Protocols} handshake \emph{followed by silence} means the
transport is at fault, not the rendering. When several unrelated fields fail
together it is never the view layer.

\subsection{Nothing rotated any log, on either machine}
\label{ap:repro-logrotate}
Found by the audit of August 18 and worth stating plainly, because it had
been quietly true since the beginning: \textbf{no log in this project had any
rotation}, neither on the workstation nor on the robot, and nothing capped
the systemd journal either.

On the robot that was close to costing the stack. The $30$\,GB card was at
$97\,\%$, \textbf{$934$\,MB free}: $2.8$\,GB of uncapped systemd journal,
$3.9$\,GB of ROS logs across four finished sessions, and a single
\code{rosmon.log} at $783$\,MB. And a full disk is not a slow degradation
here --- it kills \code{leo.service}, which was still at \code{Restart=no},
so the robot would have stayed powered and mute exactly as it did on July 21.
Both halves are now fixed: \code{SystemMaxUse=200M} in
\code{journald.conf}, a \code{cap-ros-logs} systemd timer truncating ROS logs
hourly, and a \code{20-restart.conf} drop-in giving the unit
\code{Restart=on-failure} with \code{StartLimitBurst=5} over $5$ minutes so a
config that fails instantly cannot become a restart storm. Result:
$97\,\% \rightarrow 79\,\%$, $934$\,MB $\rightarrow 6.1$\,GB free. All four
files are versioned under \code{tools/robot/}, since a card reflash would
otherwise lose them without trace.

\begin{trapbox}{Two robot-side facts that break the obvious fix}
\code{cron} \textbf{does not exist on this Pi}: \code{/usr/bin/crontab} is
absent and \code{cron.service} is inactive, so a crontab entry fails
silently --- the first attempt did exactly that. systemd timers, on the other
hand, already run there (\code{man-db}, \code{apt-daily}), which is why the
cap is a timer.

And these logs must be \textbf{truncated, never deleted}: \code{rosmon.log}
is held open continuously by \code{rosmon}, so unlinking it would not return
the space while the process lives. Its descriptor carries \code{O\_APPEND}
(flags \code{02402001}, checked before touching anything), which is what
makes \code{truncate -s 0} safe: the writer resumes at the end of the emptied
file, with no sparse hole and no phantom space.
\end{trapbox}

\noindent On the workstation the same gap was real but far less urgent
($231$\,GB free): \code{navigation\_master.log} had reached $621$\,MB in five
days and takes back $\sim\!84$\,MB in a few hours. The guard lives at the top
of \code{leo\_watchdog.sh}, which cron already runs every minute --- placed
\emph{before} the early exit taken when the robot is unreachable, because
that is precisely when those logs grow fastest. It fired on its first pass,
reclaiming $230$\,MB from \code{carolus\_detect.log}.
...
\item \textbf{Clock offset between the two machines}, if the robot has been
  running on its own access point. Measured August 18: the robot ran
  $+0.16$\,s ahead of the workstation and \code{NTPSynchronized} was
  \code{no} --- on the fallback AP it reaches no time server, and this Pi has
  no RTC. The drift was measured over $16.7$\,min at $+12.3$\,ms
  ($\sigma=4.4$\,ms, so significant), i.e.\ $\approx\!+1$\,s per day. Nothing
  was broken by it at that magnitude --- TF resolved $20/20$ across the
  PC/robot boundary and no extrapolation warning appears in any log --- but
  it accumulates for as long as the robot stays off the campus network, and
  it clears itself the moment \code{wlan\_int} rejoins it. Check it without
  SSH round-trip noise by reading the age of a robot-stamped message from the
  workstation:
\begin{lstlisting}[language=bash]
rostopic echo -n1 /firmware/wheel_states   # comparer stamp et date du PC
\end{lstlisting}
\end{enumerate}
...
\section{Pose-regulation motion control: Carolus and AprilTag, to an
arbitrary (X, Y, Z, $\theta$)}
\label{ap:posecontrol}
Every autonomous behaviour documented elsewhere in this report
(\S\ref{sec:pid}, \S\ref{sec:apriltagfallback}) \emph{centres} on a target,
it drives the target's pixel offset toward zero, with no notion of a
specific commanded $(X,Y,Z,\theta)$ to stop at. This appendix specifies,
derives, and implements the missing capability the supervisor requested:
given an arbitrary desired ground-frame pose, drive there from an arbitrary
start, using either Carolus or AprilTag as the position sensor, through the
same ground frame Section~1 already established
(\S\ref{sec:quatuniversal}, \code{beacon\_link}).

\subsection{Why a three-phase controller, not a single smooth law}
\label{ap:posecontrolchoice}
A single, continuously-blended control law that drives $(x,y,\theta)$ to a
goal simultaneously exists in the literature (polar-coordinate feedback,
Aicardi et al.\ 1995) and was evaluated for this appendix. It was not
chosen, for a reason worth stating rather than hiding: its stability proof
requires a specific coupling between the heading-error gain and the
final-orientation-error gain that this report could not independently
re-derive with full confidence in the time available, and \textbf{shipping
an unverified stability argument to a rover with a live, unresolved UART
fault (\S\ref{sec:aug11mins}) is a worse mistake than shipping a simpler
controller with a plain per-phase proof.} The controller specified below
uses three sequential phases, each individually a single-degree-of-freedom
proportional regulation whose convergence is a one-line argument, and
mirrors the exact pattern already implemented and field-validated on this
lab's LIMO platform (\code{LimoUltimateMission}'s
\code{TURN\_1}$\to$\code{MOVE\_L}$\to$\code{TURN\_2} state machine,
\S\ref{sec:limodoc}), rather than introducing an unvalidated technique where
a validated one already exists in the same lab.

\subsection{Geometry and control law}
\label{ap:posecontrolmath}
Let $(x,y,\theta)$ be the current pose in the ground frame
(\code{beacon\_link}) and $(x_g,y_g,\theta_g)$ the commanded goal ($Z$ is
not independently controlled, this platform's motion is planar, so a
requested $Z$ is only meaningful as a height at which the target was sighted,
not a controllable degree of freedom, and is not carried into the control
law below). Define
\begin{equation}
\Delta x = x_g - x,\quad \Delta y = y_g - y,\quad
\rho = \sqrt{\Delta x^2+\Delta y^2},\quad
\gamma = \operatorname{atan2}(\Delta y,\Delta x).
\label{eq:posegeom}
\end{equation}
\textbf{Phase 1, TURN\_1 (rotate to face the goal).} Error
$e_1=\operatorname{wrap}(\gamma-\theta)\in(-\pi,\pi]$, control
$\omega = \operatorname{clamp}(k_\theta e_1,\,\pm\omega_{\max})$, this
project's own \code{PID} class (\S\ref{sec:pidmath}) instantiated with
$K_i{=}0$ exactly as the existing \code{\_pid\_lock\_align} loop is. A
single scalar error driven by proportional feedback toward zero is
Lyapunov-trivial: $V=\tfrac12 e_1^2$, $\dot V = e_1\dot e_1 = -k_\theta e_1^2
\le 0$ for $k_\theta>0$.
\textbf{Exit:} $|e_1|<\varepsilon_\theta$.

\textbf{Phase 2, MOVE\_L (drive to the goal, correcting bearing en route).}
$v = v_{\text{cruise}}$ (constant, this platform's obstacle governor,
\S\ref{sec:obstacleloop}, remains active underneath and unmodified: this
phase adds a goal, it does not remove the existing safety layer), with a
small proportional bearing correction re-evaluating
Eq.~\ref{eq:posegeom} every control tick,
$\omega = \operatorname{clamp}(k_\phi\,\operatorname{wrap}(\gamma(t)-\theta(t)),\,\pm\omega_{\text{corr}})$,
$\omega_{\text{corr}}\ll\omega_{\max}$ so the robot corrects heading without
re-entering a full stop-and-turn. \textbf{Exit:} $\rho<\varepsilon_\rho$.

\textbf{Phase 3, TURN\_2 (rotate to the commanded final heading).} Error
$e_3 = \operatorname{wrap}(\theta_g-\theta)$, identical control and
convergence argument as Phase~1. \textbf{Exit:} $|e_3|<\varepsilon_\theta$,
mission complete.

\subsection{Sensor front end: Carolus and AprilTag feed the same state
machine}
\label{ap:posecontrolsensor}
The pose $(x,y,\theta)$ Eq.~\ref{eq:posegeom} consumes is, deliberately,
sensor-agnostic: it is whichever of \code{/odom\_in\_beacon}
(\code{carolus\_tf\_bridge.py}, Carolus, \S\ref{sec:aug12readiness}) or an
equivalent AprilTag-derived ground-frame pose is freshest, following the
same strict-priority pattern already verified for centring
(\S\ref{sec:apriltagfallback}) rather than a second, separate arbitration
scheme. AprilTag's own detection message already carries a full 6-DOF pose
(\code{d.pose.pose.pose} in \code{\_on\_tag\_detections}, currently
reprojected to a 2D pixel centre and a scalar distance only, for the
centring use case); this appendix's implementation reads the same message's
full pose and passes it through the identical optical-to-body rotation
already derived and verified (Eq.~\ref{eq:rob}), rather than a second,
independently-tuned conversion.

\begin{lstlisting}[language=Python]
# catkin_ws/src/leo_navigation/scripts/goto_pose.py -- NEW, this session.
# Three-phase pose regulation (SS D.15.2), sensor-agnostic front end (SS D.15.3).
# NOT wired into any .launch file and NOT added to leo_backend.py's FSM --
# see the trapbox below for why.

class GotoPose:
    def __init__(self, goal_x, goal_y, goal_theta):
        self.goal = (goal_x, goal_y, goal_theta)
        self.phase = "TURN_1"
        self.pid_turn = PID(Kp=K_THETA, Ki=0.0, Kd=0.08,
    ...
    def update(self, x, y, theta, now):
        ...
        if self.phase == "TURN_1":
            e = wrap(gamma - theta)
            omega = self.pid_turn.update(e, now)
            if abs(e) < EPS_THETA:
                self.phase = "MOVE_L"; self.pid_turn.reset()
            return (0.0, omega)
        ...
        if self.phase == "MOVE_L":
            ...
            if rho < EPS_RHO:
                self.phase = "TURN_2"; self.pid_turn.reset()
                return (0.0, 0.0)
            return (V_CRUISE, omega)  # obstacle governor (S:obstacleloop) still applies
...
\begin{trapbox}{Written this session, deliberately not deployed}
\code{goto\_pose.py} is new, real code, not pseudocode, and every equation
in \S\ref{ap:posecontrolmath} is verified by hand (each phase's Lyapunov
argument is one line, checked directly, not cited without derivation). It
is \textbf{not} added to \code{leo\_backend.py}'s FSM, not referenced from
any \code{.launch} file, and has not been run against the real robot, for
one concrete reason: \S\ref{sec:aug11mins} documents a live, unresolved
UART checksum storm on this exact platform, found the same night this
controller was written, capable of silently dropping
\code{/firmware/wheel\_states}. Deploying a new autonomous motion-control
path on top of a known-degraded sensor input, before that fault is even
diagnosed, would risk producing a field result that measures the fault
rather than the controller. The three-phase design (\S\ref{ap:posecontrolchoice})
and the reused, already-verified building blocks (the \code{PID} class,
the optical-to-body rotation, the strict-priority sensor front end) mean
integration, once the UART fault is resolved, is wiring \code{GotoPose}
into the existing FSM as one more state alongside \code{LOCK\_BEACON} and
\code{GOTO\_BEACON}, not new development.
\end{trapbox}
