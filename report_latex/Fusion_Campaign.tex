...
\end{tabular}
\end{table}
\noindent $^\ast$\code{/ov\_srvins/odomimu} never reached the PC-side
\code{rosbag record} process despite confirmed local publication on the
Pi at the time recording started; the divergence is instead reconstructed
directly from $\sqrt{\text{VINS}}$'s own node log, timestamped and
