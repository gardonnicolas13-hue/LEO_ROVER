...
moderately dynamic lap on this hardware.'' Two candidate directions,
neither attempted tonight: report Defect 2 upstream (RPNG's own issue
tracker) with the exact reproduction above, since a $10^{17}$-scale
$\chi^2$ acceptance is a defect in the library, not a tuning choice;
and re-run the same lap with $\sqrt{\text{VINS}}$'s disparity-only ZUPT
path (\code{zupt\_chi2\_multipler: 0}, untested tonight, distinct from
the IMU-based path Defect 2 isolates) as a middle ground between ``no
velocity anchoring at all'' and ``the exact setting that diverges.''
...
41 & \textbf{$\sqrt{\text{VINS}}$ on the Pi: installed and running, diverges
    on a real driven lap} (\S\ref{sec:sqrtvins}). Builds and runs at
    $30$--$70\,\mathrm{Hz}$ on live sensor data, two real defects found
    and fixed (an undocumented config schema, and \code{try\_zupt}
    causing a $10^{17}$-scale $\chi^2$ gate failure --- deployed with it
    off). On one recorded lab lap: MINS $1.02$\,m loop closure, PC
    openVINS $541.7$\,m, $\sqrt{\text{VINS}}$ unbounded ($43\,742$\,m
    reported distance for a $\sim\!20$\,m lap) &
    report the $\chi^2$ defect upstream (RPNG) with the reproduction in
    \S\ref{sec:sqrtvins}; re-run with \code{zupt\_chi2\_multipler: 0}
    (disparity-only ZUPT, untested) instead of fully disabled \\
\end{longtable}
\end{center}
