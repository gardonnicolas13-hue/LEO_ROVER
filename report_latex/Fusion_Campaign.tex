...
40 & \textbf{A live UART checksum storm --- root mechanism caught directly
    ...
    $\sim\!40$ bytes to overflow the FIFO in one occurrence &
    check whether the xHCI controller's DMA mask forces unnecessary
    \code{swiotlb} bouncing; audit the SD-card driver's \code{udelay}
    busy-waits inside its IRQ handler; check
    \code{/proc/irq/*/smp\_affinity} for moving the MSI-based xHCI/MMC
    interrupts off CPU0, away from IRQ~$14$'s fixed line, without touching
    the UART side at all --- all three candidate explanations remaining
    after tonight are now measurement-driven, not argued \\
