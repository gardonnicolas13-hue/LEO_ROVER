...
confirmed) before moving on. The one candidate not attempted tonight,
the SD-card driver's \code{udelay} busy-waits, remains open.
...
40 & \textbf{A live UART checksum storm --- root mechanism caught directly
    ...
    test window &
    audit the SD-card driver's \code{udelay} busy-waits inside its IRQ
    handler --- the one candidate not yet attempted; re-test IRQ~$27$'s
    move with a matched-load protocol (\S\ref{sec:aug13ab}-style, not a
    single before/after sample) to separate a real effect from the
    drift observed tonight \\
