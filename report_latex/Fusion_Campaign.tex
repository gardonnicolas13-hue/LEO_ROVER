...
41 & \textbf{$\sqrt{\text{VINS}}$ on the Pi: installed and running, diverges
    on a real driven lap, disparity-only ZUPT also checked and also fails}
    (\S\ref{sec:sqrtvins}). Builds and runs at $30$--$70\,\mathrm{Hz}$ on
    live sensor data, two real defects found and fixed (an undocumented
    config schema, and \code{try\_zupt} causing a $10^{17}$-scale
    $\chi^2$ gate failure on the IMU path --- deployed with it off). On
    one recorded lab lap: MINS $1.02$\,m loop closure, PC openVINS
    $541.7$\,m, $\sqrt{\text{VINS}}$ unbounded ($43\,742$\,m reported
    distance for a $\sim\!20$\,m lap). The hoped-for middle ground,
    disparity-only ZUPT (\code{zupt\_chi2\_multipler: 0}), was tested
    at rest the same evening and is worse: \code{chi2 inf} within under
    a second, not merely a large finite value &
    the $\chi^2$ defect write-up (both ZUPT sub-modes) is drafted,
    ready to submit to RPNG's issue tracker, not yet submitted; no
    remaining ZUPT configuration is worth testing further on this
    build --- next step, if pursued, is upstream, not local \\
\end{longtable}
\end{center}
