...
accessible knob. What remains, if this is pursued further, is a kernel
patch (out of scope here) or accepting the mechanism as a platform
limit and mitigating elsewhere: MMC's affinity move, re-tested properly
against a matched load next time, is the one candidate this evening
left genuinely open rather than closed.
...
40 & \textbf{A live UART checksum storm --- root mechanism caught directly
    ...
    test window. Third candidate also closed: \code{bcm2835\_mmc} exposes
    only two module parameters, both debug-logging flags, no timing
    control &
    ...
    its boot device); re-test IRQ~$27$'s move with a matched-load
    protocol (\S\ref{sec:aug13ab}-style, not a single before/after
    sample) --- the one candidate this evening left genuinely open \\
