...
40 & \textbf{A live UART checksum storm, thermal explanation now ruled
    insufficient} (\S\ref{sec:aug11mins}, \S\ref{sec:aug13ab}). Discovered
    mid-verification: sustained \code{wrong checksum} errors, same
    signature as Eq.~\ref{eq:missprob} (July 30), ROS graph wiring itself
    confirmed correct (unlike item 10/July 22). Three explanations tested,
    all narrowed by direct measurement rather than argued: motion
    excitation, refuted (storm persisted with the rover fully stationary);
    the kernel's RT budget starving \code{serial\_node}, refuted by a
    controlled A/B ($28,41,36,26$ against a $38$ baseline, within noise,
    setting reverted); temperature, ruled insufficient by item 38's
    matched-load A/B ($2.074\,\mathrm{s}^{-1}$ at zero throttling, two
    thirds of the July 30 rate). IRQ~$14$ (\code{uart-pl011}) is a
    non-threaded hard interrupt pinned to CPU0, not a schedulable thread
    --- the layer the first two hypotheses needed to act through, and
    where both were disproven &
    instrument interrupt latency directly on IRQ~$14$ (\code{ftrace}'s
    \code{irqsoff} tracer, confirmed available with no install needed;
    \code{cyclictest} would need \code{rt-tests} first), since three of
    this item's four candidate explanations are now eliminated or bounded
