...
The translation is a tape-measure seed. \textbf{Updated 2026-09-07:} it is
no longer refined online --- \code{do\_calib\_ext} is now \code{false}, so
centimetre-level translation error is \emph{no longer recoverable} either,
and stands exactly as installed. Ninety degrees of rotation was never
recoverable and still is not. The time offset between camera
...
% HAUTEUR PLAFONNEE (2026-09-08). La largeur seule ne suffit pas : ces PNG
% font 593,7 x 404,7 pt (rapport 0,682), donc a 0,82\linewidth (=455 pt de
% texte) chaque panneau montait a ~254 pt. Deux panneaux plus leurs sous-
% legendes — longues ici — et la legende d'ensemble depassaient les
% 652,7 pt de \textheight : LaTeX signalait « Float too large for page by »
% jusqu'a 98,76 pt sur la planche fig:trajresid.
% `keepaspectratio` fait respecter la PLUS CONTRAIGNANTE des deux bornes,
% donc c'est la hauteur qui decide ici (0,29\textheight ~ 189 pt, largeur
% resultante ~0,61\linewidth) sans jamais deformer l'image. Plafonner ici
% plutot que dans chaque figure corrige les huit panneaux d'un coup et
% empeche la reapparition du probleme si un panneau est ajoute.
\newcommand{\trajpanel}[2]{%
  \IfFileExists{figures/traj_p#1.png}{%
    \includegraphics[width=0.82\linewidth,height=0.29\textheight,%
                     keepaspectratio]{figures/traj_p#1.png}%
  }{%
...
\paragraph{Why the window separates the two cases, in closed form.} The
replay above is an empirical demonstration; the reason it works has a short
derivation. Model a discrete correction of size $\delta$ landing at time
$t_c$ inside an otherwise steady drive at speed $v_0$: the position is
$x(t) = v_0 t + \delta\,H(t-t_c)$, with $H$ the Heaviside step. For a window
of duration $T$ that contains $t_c$, Eq.~\ref{eq:guardwindow} reads
\begin{equation}
v_{\text{apparent}}(T) \;=\; \frac{\lvert x(t)-x(t-T)\rvert}{T}
\;=\; v_0 + \frac{\delta}{T} ,
\label{eq:guardstep}
\end{equation}
which \emph{decreases} monotonically in $T$ and tends to the true speed
$v_0$ as $T$ grows: the correction's contribution is amortised over an
increasingly long interval. A \emph{sustained} runaway of speed $v_{\text{bad}}$,
by contrast, has $x(t)=v_{\text{bad}}\,t$ and
\begin{equation}
v_{\text{apparent}}(T) \;=\; v_{\text{bad}} ,
\label{eq:guardramp}
\end{equation}
independent of $T$ entirely: there is no amortising it, because every
sub-interval of the runaway is itself the runaway. Substituting the worst
observed case ($v_0=0.13$\,m/s, $\delta=4.23$\,cm) into
Eq.~\ref{eq:guardstep} gives the crossing point where the correction alone
would still trip the $v_{\max}=1.5$\,m/s guard,
$T^\star = \delta/(v_{\max}-v_0) \approx 30.9$\,ms --- comfortably below the
deployed $T=100$\,ms, at which Eq.~\ref{eq:guardstep} evaluates to
$0.553$\,m/s, roughly a factor of three under threshold rather than the
bare $\delta/T=0.423$\,m/s a reader would get by dropping the $v_0$ term.
\begin{center}
\begin{tikzpicture}[font=\scriptsize]
  \def\kx{0.045} \def\ky{1.0}
  \draw[->, thick] (0,0) -- ({160*\kx},0) node[right]{$T$ (ms)};
  \draw[->, thick] (0,0) -- (0,{6.3*\ky}) node[above]{$v_{\text{apparent}}$ (m/s)};
  % seuil de la garde
  \draw[dashed, black!55] (0,{1.5*\ky}) -- ({160*\kx},{1.5*\ky});
  \node[left, black!55] at (0,{1.5*\ky}) {$1.5$};
  % divergence soutenue : plate, ne descend jamais sous le seuil
  \draw[very thick, alertred] (0,{6*\ky}) -- ({160*\kx},{6*\ky});
  \node[right, alertred] at ({160*\kx},{6*\ky}) {$v_{\text{bad}}$ soutenu};
  % correction discrete : Eq. guardstep, points precalcules
  \draw[very thick, accentblue, smooth]
    plot coordinates {({11*\kx},{3.975*\ky}) ({15*\kx},{2.95*\ky})
      ({20*\kx},{2.245*\ky}) ({25*\kx},{1.822*\ky}) ({31*\kx},{1.495*\ky})
      ({40*\kx},{1.188*\ky}) ({55*\kx},{0.899*\ky}) ({75*\kx},{0.694*\ky})
      ({100*\kx},{0.553*\ky}) ({130*\kx},{0.455*\ky}) ({150*\kx},{0.412*\ky})};
  \node[accentblue] at ({70*\kx},{2.6*\ky}) {$v_0+\delta/T$};
  % marqueurs aux deux bases de temps discutees dans le texte
  \fill ({11*\kx},{3.975*\ky}) circle (1.3pt);
  \node[above, accentblue] at ({11*\kx},{3.975*\ky}) {$11$ms};
  \fill ({100*\kx},{0.553*\ky}) circle (1.3pt);
  \node[above, accentblue] at ({100*\kx},{0.653*\ky}) {$100$ms};
  \draw[dotted] ({31*\kx},0) -- ({31*\kx},{1.5*\ky});
  \node[below] at ({31*\kx},0) {$T^\star{=}31$};
\end{tikzpicture}
\end{center}
The blue curve is Eq.~\ref{eq:guardstep} on the worst measured correction;
the flat red line is Eq.~\ref{eq:guardramp} on a sustained $6$\,m/s runaway.
Only the former crosses below threshold as $T$ grows, which is the entire
mechanism: nothing in the fix distinguishes ``discrete'' from ``sustained''
by pattern-matching, the window merely makes a one-off average out while
leaving a persistent one untouched.
...
and the check runs cleanly in both directions. Forward, a constant residual
$a$ that a filter fails to null integrates twice into
\begin{equation}
d(\Delta t) \;=\; \tfrac{1}{2}\,a\,\Delta t^2 ,
\label{eq:doubleint}
\end{equation}
so guessing $a=0.1$\,m/s$^2$ predicts $d(15) = \tfrac{1}{2}(0.1)(15)^2 =
11.3$\,m. Backward, inverting Eq.~\ref{eq:doubleint} on the openVINS drift
actually measured, $d=11.4$\,m over $\Delta t=15$\,s, gives
\begin{equation}
a \;=\; \frac{2d}{\Delta t^2} \;=\; \frac{2(11.4)}{15^2} \;=\; 0.1013\ \text{m/s}^2 ,
\label{eq:doubleintinv}
\end{equation}
recovering the guessed value to three figures: $0.1$\,m/s$^2$ was not a
round number chosen for the illustration, it is what the observed drift
itself implies.

That figure is worth contrasting with the raw specific force the tilt
alone would put on the $x$-axis, $g\sin(6.67^\circ) \approx 1.14$\,m/s$^2$
--- an order of magnitude larger than the $0.10$\,m/s$^2$ that actually
leaked into drift. The difference is not a discrepancy: a \emph{constant}
tilt is exactly the kind of slowly-varying error a filter's own attitude
and bias states can absorb, given any excitation to observe it against
(rotation, the camera, ZUPT resets), so only whatever fraction the
estimator fails to track shows up as uncorrected drift. That residual
fraction is $0.10/1.14 \approx 9\,\%$ of the raw tilt-induced force here ---
consistent with an estimator that is mostly, but not completely,
compensating for a bias it was never told about explicitly.
...
\begin{equation}
\text{scale} = \tfrac12(1.0059+1.0607)-1 = +3.3\,\%, \qquad
\text{asym.} = \tfrac12(1.0059-1.0607) = -2.7\,\% ,
\label{eq:asymval}
\end{equation}
so the asymmetry is not a rounding artefact, it is comparable in size to
...
\begin{center}
\begin{tikzpicture}[font=\scriptsize, scale=1.0]
  % Deux panneaux COTE A COTE plutot qu'un seul repere partage : la premiere
  % version superposait les deux rayons a 18 deg l'un de l'autre, ce qui
  % entassait les quatre etiquettes dans le meme coin. Separer physiquement
  % "gauche" et "droite" laisse la place de lire chaque angle seul.
  \begin{scope}
    \coordinate (OL) at (0,0);
    \draw[->, thin] (OL) -- (2.0,0);
    \draw[dashed, black!50] (OL) -- (0,2.0);
    \draw[dashed, black!50] (OL) ++(0.5,0) arc[start angle=0, end angle=90, radius=0.5];
    \draw[very thick, accentblue] (OL) -- ({1.9*cos(100)},{1.9*sin(100)});
    \draw[accentblue] (OL) ++(0.9,0) arc[start angle=0, end angle=100, radius=0.9];
    \node[align=center, below] at (1.0,-0.4)
      {\textbf{gauche}, $g_L=1.0059$\\dépasse le $90^\circ$ (gris) de peu};
  \end{scope}
  \begin{scope}[xshift=4.4cm]
    \coordinate (OR) at (0,0);
    \draw[->, thin] (OR) -- (2.0,0);
    \draw[dashed, black!50] (OR) -- (0,2.0);
    \draw[dashed, black!50] (OR) ++(0.5,0) arc[start angle=0, end angle=90, radius=0.5];
    \draw[very thick, alertred] (OR) -- ({1.9*cos(125)},{1.9*sin(125)});
    \draw[alertred] (OR) ++(0.9,0) arc[start angle=0, end angle=125, radius=0.9];
    \node[align=center, below] at (1.0,-0.4)
      {\textbf{droite}, $g_R=1.0607$\\dépasse le $90^\circ$ nettement plus};
  \end{scope}
\end{tikzpicture}
\end{center}
Both readings overshoot the same $90^\circ$ real target (grey, dashed, same
reference on both panels), but by visibly different amounts (angles
exaggerated here for legibility, not to numerical scale): a single
corrective gain that pulls the right-turn ray back to $90^\circ$ would
overshoot in the other direction on the left-turn ray, and vice versa,
which is the arithmetic of Eq.~\ref{eq:asymval} already showed.
...
\section{Field session, August 11: active cooling installed, item 11 recurs
a second time, and a live serial desync caught mid-session}
\label{sec:aug11}
...
closes item 38 (\S\ref{sec:july30thermal}): active cooling was installed and
the thermal envelope re-measured against the exact $86$--$89.1\,^\circ$C
boundary characterised on July 30. The architectural thread is that three
independent PC-side processes were found, on the same evening, holding a
ROS-graph state that predated the robot's current boot while still looking
alive to every presence check --- the identical defect item 11 already named
in a narrower form, recurring for the second time in exactly the pattern
item 10 established in \S\ref{sec:july22network}: not a new bug, a
prediction the registry had already made, confirmed by evidence collected
independently of it. A fourth, unrelated fault, a live UART checksum storm,
surfaced while re-verifying MINS for this very section and is reported
mid-diagnosis, unresolved, rather than held back for a tidier write-up.

\subsection{Active cooling: installation}
\label{sec:aug11fans}

Two 5\,V fans were wired directly to the Raspberry Pi's GPIO header, in
parallel, each with its own independent point-to-point connection rather
than a shared spliced junction: four wires, four pins, no solder joint
between them. Pins 2 and 4 are both unregulated taps off the Pi's main
5\,V rail (unlike the logic GPIO pins, which are current-limited and
unsuitable for a motor load), and pins 6 and 9 are both ground. Pin 8,
physically adjacent to pin 6 on the same row, was deliberately avoided: it
carries UART TXD, not ground, and is the one wiring mistake the layout makes
easy.
...
\begin{tikzpicture}[font=\small, >=Latex]
  \def\dy{0.85}
  \tikzset{pin/.style={circle, draw, minimum size=6mm, inner sep=0pt, font=\scriptsize}}
  % header, rows 1-5 (pins 1-10) of the 40-pin header. Left column = odd
  % pins (1,3,5,7,9), right column = even pins (2,4,6,8,10), row $k$ holds
  % pins $2k-1$ (left) and $2k$ (right), $k=1,\dots,5$.
  \node[pin, fill=black!4] (L1) at (0,0)          {1};
  \node[pin, fill=red!25, draw=alertred] (R1) at (0.85,0)          {2};
  \node[pin, fill=black!4] (L2) at (0,-1*\dy)      {3};
  \node[pin, fill=red!25, draw=alertred] (R2) at (0.85,-1*\dy)      {4};
  \node[pin, fill=black!4] (L3) at (0,-2*\dy)      {5};
  \node[pin, fill=black!25]              (R3) at (0.85,-2*\dy)      {6};
  \node[pin, fill=black!4] (L4) at (0,-3*\dy)      {7};
  \node[pin, fill=orange!35, draw=orange!70!black] (R4) at (0.85,-3*\dy) {8};
  \node[pin, fill=black!25] (L5) at (0,-4*\dy)      {9};
  \node[pin, fill=black!4]              (R5) at (0.85,-4*\dy)      {10};
  \node[font=\scriptsize, anchor=west, align=left] at (2.0,0)
        {\textcolor{alertred}{\textbf{+5V}} --- fan 1 red};
  \node[font=\scriptsize, anchor=west, align=left] at (2.0,-1*\dy)
        {\textcolor{alertred}{\textbf{+5V}} --- fan 2 red};
  \node[font=\scriptsize, anchor=west, align=left] at (2.0,-2*\dy)
        {\textbf{GND} --- fan 1 black};
  \node[font=\scriptsize, anchor=west, align=left] at (2.0,-3*\dy)
        {\textcolor{orange!70!black}{\textbf{not GND}} --- UART TXD, avoid};
  \node[font=\scriptsize, anchor=west, align=left] at (2.0,-4*\dy)
        {\textbf{GND} --- fan 2 black};
  \node[font=\scriptsize, anchor=east] at (-0.5,0) {top edge, pin 1};
\end{tikzpicture}
\caption[GPIO fan wiring]{The four direct connections used: pins 2 and 4
(+5\,V) to each fan's red lead, pins 6 and 9 (GND) to each fan's black
lead, independently, no shared junction. Pin 8 (right column, one row below
pin 6) carries UART TXD, not ground --- the adjacent trap the layout
invites; pin 9 (left column) is the one connection that changes side.}
\label{fig:fanwiring}
\end{figure}

The two fans' current draw was not measured against a datasheet, and is
recorded here as an open quantity rather than assumed benign: the indirect
evidence is that \code{vcgencmd get\_throttled} read \code{0x0} on every
check made after power-on, under both the light and heavy loads of
\S\ref{sec:aug11thermal}, and bit 0 of that register (under-voltage
\emph{now}) together with bit 16 (under-voltage \emph{since boot}) are both
part of that word and both clear throughout. That is evidence the added load
has not collapsed the 5\,V rail, not a substitute for a measured current
budget.

\subsection{Thermal verification against the July 30 envelope}
\label{sec:aug11thermal}

\S\ref{sec:july30thermal} established a boundary, not a single number: the
scheduling fix held the serial link at $86\,^\circ$C under full colour load
and lost it at $89.1\,^\circ$C, with $\sim3\,^\circ$C of usable margin and
``no headroom for ambient variation''. That boundary is the correct
reference for tonight's measurement, taken twice, at opposite ends of load.

\begin{casestudy}{The headline number: up to $30.6\,^\circ$C recovered, and
the clock never dropped}
Before active cooling, this exact rover collapsed at $89.1\,^\circ$C
(\S\ref{sec:july30thermal}): every firmware topic silent, ARM clock pinned at
its throttle floor of $600$\,MHz. With the two GPIO fans installed, the
coldest reading taken tonight was $58$--$59\,^\circ$C; the hottest, taken
under the heaviest simultaneous CPU load this rover has been driven under to
date (a $31$-node full-stack restart), was $72.5\,^\circ$C. Both are measured
against the same \code{vcgencmd} instrumentation used throughout this report,
not extrapolated:
\begin{center}
\large $89.1\,^\circ\text{C} \;\longrightarrow\; 58$--$59\,^\circ\text{C}$
\quad\textbf{(--$30.6\,^\circ$C, light load)} \\[5pt]
\large $89.1\,^\circ\text{C} \;\longrightarrow\; 72.5\,^\circ\text{C}$
\quad\textbf{(--$16.6\,^\circ$C, heaviest load ever recorded on this rover)}
\end{center}
The ARM clock read $1500$\,MHz, its undivided maximum, on \emph{every} check
made tonight, light load and heavy alike. Before cooling, this rover had
never once sustained a full colour-camera load at that clock speed for more
than a few minutes.
\end{casestudy}
...
\begin{tikzpicture}[font=\small]
  \def\base{50}      % axis starts at 50C, not 0C -- the band that matters
  \def\k{0.15}        % cm per degree
  \newcommand{\ytop}[1]{\k*(#1-\base)}
  % axis
  \draw[->] (-0.3,0) -- (-0.3,6.6) node[left, font=\scriptsize, align=center] {core\\temp.};
  \foreach \t in {50,60,70,80,90} {
    \draw[gray!40] (-0.3,{\k*(\t-\base)}) -- (7.2,{\k*(\t-\base)});
    \node[font=\scriptsize, anchor=east] at (-0.4,{\k*(\t-\base)}) {$\t\,^\circ$C};
  }
  % reference lines: envelope floor (86) and collapse ceiling (89.1)
  \draw[densely dashed, black!55] (-0.3,{\k*(86-\base)}) -- (7.2,{\k*(86-\base)})
        node[right, font=\scriptsize, black!55] {$86\,^\circ$C, July 30 stable floor};
  \draw[densely dashed, alertred] (-0.3,{\k*(89.1-\base)}) -- (7.2,{\k*(89.1-\base)})
        node[right, font=\scriptsize, alertred] {$89.1\,^\circ$C, July 30 collapse};
  % bars
  \fill[alertred!75] (0.6,0) rectangle (2.0,{\k*(89.1-\base)});
  \node[font=\scriptsize, align=center] at (1.3,-0.35) {July 30\\no cooling};
  \node[font=\scriptsize, anchor=south, align=center] at (1.3,{\k*(89.1-\base)+0.08}) {$89.1\,^\circ$C\\\scriptsize 600\,MHz};

  \fill[orange!75!black] (3.0,0) rectangle (4.4,{\k*(72.5-\base)});
  \node[font=\scriptsize, align=center] at (3.7,-0.35) {Aug 11\\heavy load};
  \node[font=\scriptsize, anchor=south, align=center] at (3.7,{\k*(72.5-\base)+0.08}) {$72.5\,^\circ$C\\\scriptsize 1500\,MHz};

  \fill[accentblue!75] (5.4,0) rectangle (6.8,{\k*(58.5-\base)});
  \node[font=\scriptsize, align=center] at (6.1,-0.35) {Aug 11\\light load};
  \node[font=\scriptsize, anchor=south, align=center] at (6.1,{\k*(58.5-\base)+0.08}) {$58$--$59\,^\circ$C\\\scriptsize 1500\,MHz};
\end{tikzpicture}
\caption[Thermal result, before and after cooling]{The same rover, same
instrumentation, before and after the two GPIO fans. Axis starts at
$50\,^\circ$C to make the band that matters legible; every bar is a real
reading, not an extrapolation. Both post-cooling bars sit below the July 30
stable floor, not just below its collapse ceiling.}
\label{fig:thermalbars}
\end{figure}

\begin{table}[htbp]
\centering
\footnotesize
\caption{Thermal state with active cooling, at two operating points, against
the July 30 no-cooling collapse (Table~\ref{tab:july30thermal}).}
\label{tab:aug11thermal}
\begin{tabular}{@{}p{2.4cm}p{2.3cm}p{3.6cm}p{3.6cm}@{}}
\toprule
Quantity & July 30, no cooling & Aug 11, light load & Aug 11, heavy load \\
\midrule
Core temperature         & $89.1\,^\circ$C   & $58$--$59\,^\circ$C & $72.5\,^\circ$C \\
\code{get\_throttled}    & \code{0xe0006}    & \code{0x0}           & \code{0x0} \\
ARM clock                & $600$\,MHz        & $1500$\,MHz          & $1500.3$\,MHz \\
Load average (4 cores)   & $8.85$            & not logged           & $6.84$ \\
Oversubscription $\lambda/4$ & $2.21\times$   & --                   & $1.71\times$ \\
Observed load             & $185\,\%$ colour nodelet alone & \code{realsense2\_camera\_manager} at $111\,\%$, $30/33$ nodes, $2$\,min window & full-stack restart: $31$ ROS nodes, MINS + openVINS + camera initialising together \\
\bottomrule
\end{tabular}
\end{table}

\noindent\textbf{Why the frequency term drops out, not just the number.}
\S\ref{sec:july30thermal} decomposed the UART service latency as
\begin{equation}
L \;=\; L_{\mathrm{preempt}} \;+\; \frac{f_{\max}}{f}\,C
\tag{\ref{eq:latencysplit} revisited}
\end{equation}
and identified the second term, inflated by throttling to
$f_{\max}/f = 1500/600 = 2.5$ (Eq.~\ref{eq:freqpenalty}), as the mechanism
the July 28 scheduling fix could not touch. With the clock now reading
$f = 1500$\,MHz on every check made tonight, under load as well as at rest,
\begin{equation}
\frac{f_{\max}}{f} \;=\; \frac{1500}{1500} \;=\; 1,
\label{eq:aug11nopenalty}
\end{equation}
which is not a smaller penalty than the $2.5\times$ figure that produced the
July 30 collapse: it is the term's identity element, no penalty at all. Every
scheduling-side margin the July 28 fix bought is now spent against $C$ alone,
never inflated by clock throttling, which is the specific mechanism the fans
were installed to remove, not merely a coincidental temperature drop.

\noindent\textbf{Margin, stated in the July 30 units.} Writing $M_{86}$ and
$M_{89.1}$ for the remaining margin to the envelope's stable floor and
collapse ceiling respectively,
\begin{align}
M_{86}(T)   &= 86.0 - T, &
M_{89.1}(T) &= 89.1 - T,
\label{eq:thermalmargin}
\end{align}
the light-load reading gives $M_{86} \approx 27$--$28\,^\circ$C and
$M_{89.1} \approx 30$--$31\,^\circ$C; the heavy-load reading, the highest
simultaneous CPU demand observed on this rover to date, gives
$M_{86} = 13.5\,^\circ$C and $M_{89.1} = 16.6\,^\circ$C. Even at that peak,
the margin to the point where the July 30 session lost every firmware topic
is more than five times the width of the envelope that was previously
found to separate a stable rover from one that answers no command at all.

\begin{trapbox}{The matched-load A/B item 38 actually asked for has not been run}
Item 38's required action was explicit: ``re-measure the envelope after
cooling; the correct A/B experiment is overrun rate at fixed load, not
temperature alone.'' Tonight's heavy-load figure is a different profile
(a $31$-node simultaneous stack restart) from July 30's ($185\,\%$ on a
single colour nodelet, sustained), so the two oversubscription figures in
Table~\ref{tab:aug11thermal} are informative but not the controlled
comparison the item calls for. The correct remaining experiment is to
restore the exact July 30 load, colour stream enabled, nothing else changed,
and compare the UART overrun counter (Eq.~\ref{eq:missprob}'s
$3.07\,\mathrm{s}^{-1}$ figure) against the same window with cooling
installed. Every reading above is real and was taken, not assumed; what has
not happened is holding the one variable item 38 asked to isolate.
\end{trapbox}

\subsection{A reboot-survival ghost, three times over: a formal staleness test}
\label{sec:aug11stale}

Restoring the robot's connectivity after tonight's reboot surfaced the same
failure shape three separate times, in three unrelated processes, before it
was recognised as one mechanism rather than three incidents. Each time, an
operator-visible symptom (missing telemetry, an unresponsive cockpit switch,
a manual-drive command that never arrived) was traced not to a crashed
process but to a live one: a PC-side ROS node whose OS process had never
exited, and whose ROS-graph registration, publisher handles and topic
subscriptions were therefore still those it held against the \emph{previous}
robot boot. Every presence-only liveness check, \code{pgrep}, \code{rosnode
list}, a bound TCP port, reports such a process as healthy, because by every
test those checks make it is.

\paragraph{A criterion that needs no tuning.} Let $t$ be the instant of a
check, $U(t)$ the robot's own uptime read at $t$ (\code{cat
/proc/uptime} over SSH), and $E_P(t)$ the elapsed time since a PC-side
process $P$ started (\code{ps -o etimes}). Any ROS-graph state $P$ holds was
necessarily established no earlier than the last time the robot's own
\code{rosmaster} came up, so
\begin{equation}
E_P(t) \;>\; U(t) \quad\Longrightarrow\quad P \text{ is stale relative to the
current robot session.}
\label{eq:staleness}
\end{equation}
The implication runs one way only, deliberately: $E_P(t) \le U(t)$ does not
certify $P$ healthy, it only fails to convict it by this test, since a
process can start fresh after a reboot and still fail for an unrelated
reason. Eq.~\ref{eq:staleness} is exactly the ad hoc comparison made three
times below, generalised into a check with no free parameter, in contrast to
item 11's original recommendation of an unspecified ``maximum grace
duration'' to tune.

\begin{table}[htbp]
\centering
\footnotesize
\caption{Three processes found stale by Eq.~\ref{eq:staleness} on the same
evening, none of them crashed.}
\label{tab:staleness}
\begin{tabular}{@{}p{2.5cm}p{2.3cm}p{1.5cm}p{6.4cm}@{}}
\toprule
Process & $E_P$ & $U$ (at check) & Resolution \\
\midrule
\code{leo\_backend.py} & $922$\,s & a few minutes &
  killed and relaunched; survived a duplicate-registration race
  (\code{shutdown request: [/leo\_backend] Reason: new node registered with
  same name} on the losing instance), surviving instance confirmed publishing
  \code{/mission/telemetry} at $9.985$\,Hz \\
\code{pose\_selector} & absent from \code{rosnode list} entirely, not merely stale & $\approx4$\,min &
  \code{tools/restart\_stack.sh} \\
\code{mins\_subscribe} & $1085$\,s & $\approx4$\,min &
  \code{tools/restart\_stack.sh}; ended \code{TIMEOUT init MINS}, item 40 \\
\code{rosbridge\_server} (\code{:9090}, LAN) & $11\,051$\,s ($\approx3$h$04$) & $28$\,min &
  \code{kill -9}, relaunched via \code{start\_web.sh}; verified with a real
  \code{HTTP/1.1 101 Switching Protocols} handshake afterward, not just a
  bound port \\
\bottomrule
\end{tabular}
\end{table}
...
\resizebox{\linewidth}{!}{%
\begin{tikzpicture}[font=\small, >=Latex]
  \draw[->] (0,0) -- (11.4,0) node[right, font=\scriptsize] {$t$};
  \draw[thick, densely dashed] (4.2,1.7) -- (4.2,-3.6);
  \node[anchor=south, font=\scriptsize, align=center] at (4.2,1.8) {robot\\reboot};

  % rosbridge_server: starts far left, long before reboot, survives it
  \draw[very thick, alertred] (0.2,0.9) -- (9.9,0.9);
  \node[font=\scriptsize, anchor=east] at (0.0,0.9) {\code{rosbridge\_server}};
  \draw[alertred, very thick] (9.75,0.75) -- (10.05,1.05);
  \draw[alertred, very thick] (9.75,1.05) -- (10.05,0.75);
  \node[font=\scriptsize, anchor=west, align=left] at (10.2,0.9)
       {$E_P\!=\!11\,051$s $>$ $U\!=\!28$min};

  % mins_subscribe: starts a bit before reboot
  \draw[very thick, alertred] (3.0,-0.3) -- (9.9,-0.3);
  \node[font=\scriptsize, anchor=east] at (0.0,-0.3) {\code{mins\_subscribe}};
  \draw[alertred, very thick] (9.75,-0.45) -- (10.05,-0.15);
  \draw[alertred, very thick] (9.75,-0.15) -- (10.05,-0.45);
  \node[font=\scriptsize, anchor=west, align=left] at (10.2,-0.3)
       {$E_P\!=\!1085$s $>$ $U\!\approx\!4$min};

  % leo_backend
  \draw[very thick, alertred] (3.5,-1.5) -- (9.9,-1.5);
  \node[font=\scriptsize, anchor=east] at (0.0,-1.5) {\code{leo\_backend.py}};
  \draw[alertred, very thick] (9.75,-1.65) -- (10.05,-1.35);
  \draw[alertred, very thick] (9.75,-1.35) -- (10.05,-1.65);
  \node[font=\scriptsize, anchor=west, align=left] at (10.2,-1.5)
       {$E_P\!=\!922$s $>$ $U\!\approx$ few min};

  % a fresh, non-stale process for contrast: firmware_message_converter, started after reboot
  \draw[very thick, accentblue] (4.5,-2.7) -- (9.9,-2.7);
  \node[font=\scriptsize, anchor=east] at (0.0,-2.7) {\code{firmware\_message\_converter}};
  \fill[accentblue] (9.9,-2.7) circle (2.2pt);
  \node[font=\scriptsize, anchor=west, align=left] at (10.2,-2.7)
       {$E_P\!=\!1963$s $<$ $U\!=\!3142$s: not stale};

  \node[font=\scriptsize, anchor=north west, align=left, text width=13.5cm] at (0,-3.3)
      {Checks all made at the same instant (right edge). Three lifelines
       cross the reboot; a fourth, drawn for contrast, starts after it and
       fails Eq.~\ref{eq:staleness}.};
\end{tikzpicture}%
}
\caption[Reboot-survival ghosts]{Elapsed time since process start
($E_P$, horizontal bars) against the robot's own uptime at the moment of
each check ($U$). A bar that starts before the reboot line and is still
being read afterward satisfies Eq.~\ref{eq:staleness}: it is provably
stale, independent of whether it still responds to a presence check.}
\label{fig:staleness}
\end{figure}

\noindent\textbf{Item 11, recurring for the second time.} Item 11 already
named this defect, narrower: ``a stuck rosbridge TLS process was left alone
for hours because idempotency has no time ceiling'' (\S\ref{sec:july21}),
found on the TLS bridge, port 9443. Tonight's \code{rosbridge\_server}
ghost is the identical defect on the sibling plain bridge, port 9090:
\code{start\_web.sh}'s own guard, \code{pgrep -f "\_\_name:=rosbridge\_server"},
matched the three-hour-old process and printed ``\code{en cours de
d\'{e}marrage --- patience}'' on every one-minute \code{leo\_watchdog.sh}
cron tick, exactly the unbounded grace period item 11 already flagged, on
the other bridge. Following the precedent set in \S\ref{sec:july22network}
when item 10 recurred, this is recorded as an update to item 11 rather than
a new item: the same blind spot, confirmed a second time, independently, on
the bridge item 11's own fix had not yet reached.

\subsection{A live misdiagnosis, corrected before it reached the code}
\label{sec:aug11rosbridge}

Restoring the manual-drive path surfaced a second rosbridge subtlety, and it
is recorded here including the wrong turn, in keeping with this report's
practice of stating a correction rather than silently replacing it
(cf.\ the July 30 colour-stream correction, \S\ref{sec:july30thermal}). The
cockpit is reachable two ways, shown in Figure~\ref{fig:rosbridgearch}: a LAN
client hits the plain bridge on port 9090 directly, while the public
Cloudflare domain proxies to a TLS bridge on port 9443. Both are the same
\code{rosbridge\_server} code; both are launched with a
\code{websocket\_external\_port} parameter that autobahn uses to validate
the incoming HTTP \code{Host} header's port before completing the WebSocket
handshake.
...
\begin{tikzpicture}[font=\small, >=Latex,
  box/.style={rectangle, draw, rounded corners=2pt, align=center,
              minimum height=0.9cm, fill=black!3, inner sep=4pt}]
  \node[box, minimum width=3.0cm] (lan) at (0,2.6) {browser, LAN\\\scriptsize\code{ws://host:9090}};
  \node[box, minimum width=3.0cm] (pub) at (0,0) {browser, public\\\scriptsize\code{wss://cockpit.../rosbridge}};
  \node[box, minimum width=3.2cm] (b9090) at (5.2,2.6) {\code{rosbridge\_server}\\\scriptsize\code{:9090}, \code{external\_port=9090}};
  \node[box, minimum width=2.6cm, fill=blue!6, draw=accentblue] (cf) at (5.2,0) {cloudflared\\\scriptsize \code{Host: 127.0.0.1:9443}};
  \node[box, minimum width=3.4cm, fill=blue!6, draw=accentblue] (b9443) at (12.0,0) {\code{rosbridge\_websocket\_tunnel}\\\scriptsize\code{:9443}, \code{external\_port=443}};
  \node[box, minimum width=2.6cm, minimum height=2.2cm] (graph) at (15.8,1.3) {ROS graph\\\code{leo\_backend}\\$\downarrow$\\\code{/cmd\_vel}\\$\downarrow$\\\code{serial\_node}};

  \draw[->] (lan) -- (b9090);
  \draw[->] (pub) -- (cf);
  \draw[->] (cf) -- node[above, font=\scriptsize] {\code{Host} rewritten} (b9443);
  \draw[->] (b9090) -- (graph.north west);
  \draw[->] (b9443) -- (graph.south west);
\end{tikzpicture}%
}
\caption[Dual rosbridge entry points]{Two independent bridges reach the same
ROS graph. \code{cloudflared} does not preserve the browser's public
\code{Host} header for this ingress rule; it rewrites it to the literal
origin address it dials (\code{127.0.0.1:9443}), which is why the TLS
bridge's \code{external\_port} is set to $443$ --- the public port implied
when a real browser's \code{Host} header carries no port at all --- and not
to $9443$.}
\label{fig:rosbridgearch}
\end{figure}

The live symptom, \code{failing WebSocket opening handshake ('port 9443 in
HTTP Host header ... does not match server listening port 443')}, appeared
in the tunnel's log at almost exactly one-minute intervals. The first
hypothesis was that this was real, tunnelled browser traffic being rejected,
and the corrective action taken was to relaunch the tunnel \emph{without}
\code{external\_port}. That was wrong, and \code{start\_web.sh}'s own
comment at the launch site says why: \code{cloudflared} forwards this
ingress rule's browser requests with \code{Host: cockpit.leo-rover-...}
(no port, $443$ implied), so removing the parameter would have broken real
traffic to fix a false signal. The true source of the one-minute-interval
log lines was \code{leo\_watchdog.sh}'s own liveness probe
(\S\ref{sec:july21}), which deliberately sends \code{Host: 127.0.0.1:\%d}
and, by its own long-standing comment, treats the resulting rejection as
proof of life (``\code{Un 400 suffit}''). The mistaken fix was reverted
before being left in place, and the true fault (\S\ref{sec:aug11stale}'s
\code{rosbridge\_server} ghost) was found afterward by the same
elapsed-time-versus-uptime comparison used throughout this section. A
realistic handshake, \code{Host} header without a port, TLS, against the
live port, was then used to confirm the fix directly rather than inferring
it from the absence of further log lines:
\begin{center}
\code{HTTP/1.1 101 Switching Protocols} \quad (both bridges, re-tested independently)
\end{center}

\subsection{An unresolved fault, caught mid-verification}
\label{sec:aug11mins}

\code{tools/restart\_stack.sh} completed with \code{TIMEOUT init MINS}
(item 40, below), and a first check afterward found \code{mins\_subscribe}
registered but \code{/mins/imu/odom} silent for a $10$\,s window while
\code{pose\_selector} continued reporting \code{pose\_source: "MINS"}, i.e.\
the estimator currently selected for pose fusion was not producing pose.
Re-checked directly while writing this section, the picture sharpened:
\code{/firmware/wheel\_states} was also silent for a $10$\,s window, yet
\code{rosnode info /firmware\_message\_converter} showed it correctly
subscribed to \code{/firmware/wheel\_states}, \code{/firmware/wheel\_odom}
and \code{/firmware/imu}, all three inbound from \code{/serial\_node}, unlike
the July 22 recurrence of item 10 where the subscription itself was missing.
This is a different failure shape from that one: the graph wiring is
correct and the data is simply not arriving. A single \code{wheel\_states}
message \emph{did} arrive on a longer window, carrying non-zero wheel
positions, evidence the robot has driven some distance tonight and that the
link is not fully dead, only degraded. \code{journalctl -u leo} for the
preceding five minutes was, at the moment of writing, filling with
\begin{center}
\code{/serial\_node: [SerialClient.run]: wrong checksum for topic id and msg}
\end{center}
in a sustained burst, the same signature Eq.~\ref{eq:missprob} quantified on
July 30 and the same one \S\ref{sec:july22network} traced to a UART desync.
Both of those earlier occurrences were tied to thermal throttling
starving \code{serial\_node} of scheduling time; tonight's core temperature
and ARM clock (Table~\ref{tab:aug11thermal}) rule that specific mechanism
out directly, since the clock has not left $1500$\,MHz. A second candidate,
not established here, is physical: the checksum storm was first observed
while the rover was actively being driven, and the UART link runs over a
board-to-board connector that a stationary rover's earlier characterisation
would not have exercised. This is recorded as a hypothesis, not a finding;
distinguishing it from a third, unidentified cause requires repeating the
July 30 protocol, overrun count against a fixed load, this time with motion
as the controlled variable instead of temperature.

\subsection*{What this session establishes}
Active cooling closes the mechanism, not just the number: with the ARM
clock pinned at $1500$\,MHz under every load tested so far, the frequency
term that made the July 28 scheduling fix fail above $86\,^\circ$C
(Eq.~\ref{eq:aug11nopenalty}) cannot recur, though the exact matched-load
overrun comparison item 38 specified remains to be run. Independently, three
processes surviving a robot reboot while presence checks called them healthy
is now a named, reusable defect with a tuning-free test (Eq.~\ref{eq:staleness}),
confirmed a second time on a bridge item 11 had not yet reached, in the same
pattern item 10 established a precedent for in \S\ref{sec:july22network}. A
diagnostic habit produced a genuine cost tonight, a real misdiagnosis
mid-session, and also caught it before it reached the running system, which
is the outcome the habit is for. What remains open, honestly: item 38's
controlled A/B, and a live checksum storm whose cause is a hypothesis, not
yet a finding.

\section{Field session, August 13: the matched-load A/B, finally isolated}
\label{sec:aug13ab}

Item 38 has carried the same open action since July 30: hold the load fixed
and compare the overrun rate, not the temperature. \S\ref{sec:aug11thermal}
came close but missed on its own terms, by its own admission --- the Aug 11
heavy-load figure was a $31$-node stack restart, a heavier and different
profile from July 30's sustained single-nodelet load, so the two
oversubscription figures it produced were suggestive, not the controlled
comparison the item specified. This section runs the comparison the item
actually asked for.

\subsection{Instrumentation gap, closed first}

Neither existing script measured the right quantity.
\code{tools/check\_serial\_health.sh} (item 19) counts \code{wrong checksum}
lines in \code{journalctl} --- a protocol-level symptom, one layer removed
from the event of interest. Every number this report's UART analysis is
actually built on, Eq.~\ref{eq:missprob}'s $3.07\,\mathrm{s}^{-1}$ and the
$137\,000 \to 676\,751$ progression of Table~\ref{tab:july30thermal}, comes
from the kernel's own FIFO overrun counter instead, and nothing in
\code{tools/} read it in a scripted, repeatable way: every past reading was
an ad hoc SSH one-liner, typed once and not saved. \code{tools/check\_uart\_overruns.sh}
closes that gap: it reads \code{/proc/tty/driver/ttyAMA}'s \code{oe:} field
twice, a chosen number of seconds apart, and reports the delta as a rate in
$\mathrm{s}^{-1}$ directly comparable to Eq.~\ref{eq:missprob}. Confirmed
live before use: \code{irq:14}, \code{uart:PL011}, the same interrupt
already implicated in \S\ref{sec:july30thermal}'s deadline model.

\subsection{Matching the load}

Checked live immediately before the measurement: \code{realsense2\_camera\_manager}
at $164.7\,\%$ CPU, \code{/camera/color/image\_raw/compressed} publishing at
$15.0$\,Hz (not the $4$\,Hz throttle applied for the Test1/Test2 protocol
elsewhere in this report --- reverted, whether deliberately or as a side
effect of a later restart, and confirmed via \code{rostopic hz} rather than
assumed), and both estimators registered and running simultaneously
(\code{mins\_subscribe}, \code{ov\_msckf}). That is the same qualitative
load July 30 described: ``the colour nodelet consumed $185\,\%$ of a
four-core budget already carrying two estimators' worth of publication
traffic.'' The CPU percentage itself is not expected to match: July 30's
$185\,\%$ was measured with the clock throttled to $600$\,MHz, and the same
code path against a $1500$\,MHz clock (Eq.~\ref{eq:aug11nopenalty}, no
frequency penalty) does less relative work for the same wall-clock output,
so a lower percentage at full clock is consistent with an equal or heavier
absolute load, not evidence of a lighter one. Core temperature, read before
and after both windows, held at exactly $73.0\,^\circ$C throughout with
\code{get\_throttled} at \code{0x0} and the clock at $1500.4$\,MHz on every
check --- stable across the full $\sim13$-minute span from the initial
process check to the last reading, which is circumstantial support that the
same background load persisted through the measurement, not a
per-second confirmation of it.

\begin{casestudy}{Two runs, one number: $2.07\,\mathrm{s}^{-1}$, not $3.07$}
Two independent $180$\,s windows, run back to back, no configuration change
between them:
\begin{center}
\begin{tabular}{@{}lccc@{}}
\toprule
Window & $\Delta$\code{oe} & Duration & Rate \\
\midrule
Run A & $383$ & $180.8$\,s & $2.118\,\mathrm{s}^{-1}$ \\
Run B & $367$ & $180.8$\,s & $2.030\,\mathrm{s}^{-1}$ \\
\midrule
Pooled & $750$ & $361.6$\,s & $\mathbf{2.074\,\mathrm{s}^{-1}}$ \\
\bottomrule
\end{tabular}
\end{center}
The two runs agree to within $4\,\%$ of each other, which is what makes the
pooled figure a measurement rather than a single noisy sample. Against the
July 30 reference,
\begin{equation}
\frac{2.074}{3.07} = 0.676, \qquad\text{a } 32.4\,\%\text{ reduction, and a }
67.6\,\%\text{ remainder.}
\label{eq:aug13reduction}
\end{equation}
Restating in the miss-probability form of Eq.~\ref{eq:missprob},
\begin{equation}
p_{\mathrm{Aug13}} = \frac{2.074}{781} = 2.66\times10^{-3}
\qquad\text{versus}\qquad
p_{\mathrm{Jul30}} = 3.9\times10^{-3}.
\label{eq:aug13missprob}
\end{equation}
\end{casestudy}

\subsection{What this actually settles}

Item 38, read literally, is closed: the matched-load A/B it specified has
been run, on the load it specified, and produced a number. What that number
says is not what a reader hoping to close the whole UART story would want.
Active cooling recovers roughly a third of the overrun rate at this load,
not the whole of it. Two out of every three overruns that occurred under
July 30's throttled, no-cooling condition are still occurring under a
condition with \emph{no} throttling at all, clock pinned at its maximum on
every single check taken across the session. Eq.~\ref{eq:aug11nopenalty}
already established that the frequency-penalty term in
Eq.~\ref{eq:latencysplit} is at its identity element under tonight's
conditions; a majority of the original overrun rate surviving that removal
means a majority of $L$ in that equation is coming from somewhere
Eq.~\ref{eq:freqpenalty} never touched --- $L_{\mathrm{preempt}}$ itself, or
a mechanism the decomposition does not model at all, such as the physical
connector hypothesis \S\ref{sec:aug11mins} raised and explicitly did not
establish. This result does not distinguish between those; it rules out
temperature as sufficient, which is a narrower and more useful claim than
ruling it out as relevant.

\begin{trapbox}{What this is not}
Two runs from one session, one physical robot, one evening. It is a real
controlled comparison --- same load, same instrumentation, same
$180$\,s window as the reference it is measured against --- not a large
sample, and it says nothing about whether the remaining $67.6\,\%$ is
constant, load-dependent, or itself thermal in a way this experiment did
not vary (ambient temperature, connector temperature specifically rather
than core temperature, time since boot). The next step this result points
to, not yet run, is interrupt-latency tracing on \code{irq 14} itself
(\code{ftrace}'s \code{irqsoff} tracer, confirmed available on this robot
without installing anything, against \code{cyclictest}, which is not
installed) to see whether the residual $2.07\,\mathrm{s}^{-1}$ traces to
$L_{\mathrm{preempt}}$ directly rather than staying a subtraction.
\end{trapbox}

\section{IRQ 14 starvation, caught directly: the MMC/USB chain that blocks
the UART}
\label{sec:aug13irqsoff}

The trapbox above named the next step; it was run the same evening.
\code{ftrace}'s \code{irqsoff} tracer records the single longest span in
which hardware interrupts were disabled, with a full function-call trace of
what the CPU was doing throughout. Two independent captures, buffer
widened to $2$\,MB/core first, tracer reset to \code{nop} and the buffer
size restored to its default afterward in both cases:

\begin{casestudy}{Both captures exceed the FIFO deadline by more than
double, and neither implicates the UART}
\textbf{Capture 1} ($90$\,s window, no load applied beyond the robot's own
idle state): \code{tracing\_max\_latency} $=2829\,\mu$s. The trace is a
single MSI interrupt, \code{xhci\_irq}, entering at $t{=}22\,\mu$s and not
releasing interrupts until $t{=}2829\,\mu$s: \emph{seventy-six} separate
\code{swiotlb\_bounce} calls, each preceded by
\code{dma\_direct\_sync\_single\_for\_cpu}/\code{dma\_direct\_unmap\_page},
draining a burst of completed USB transfer descriptors
(\code{handle\_tx\_event} $\to$ \code{finish\_td}) one at a time. Not one
line of this capture mentions \code{pl011} or \code{uart}.

\textbf{Capture 2} ($45$\,s window): $3064\,\mu$s, worse than capture 1.
This one brackets the UART directly. \code{pl011\_int} fires at
$t{=}10\,\mu$s (a normal service, draining its FIFO via
\code{pl011\_fifo\_to\_tty}); at $t{=}131\,\mu$s and again at
$t{=}183\,\mu$s, \code{bcm2835\_mmc\_irq} (the Pi's own SD-card
controller) runs, each invocation spinning on \code{\_\_udelay} inside the
handler; at $t{=}240\,\mu$s \code{xhci\_irq} begins the same
\code{swiotlb}-bound descriptor drain as capture 1 (this time $380$
\code{swiotlb} occurrences across the whole trace). \code{pl011\_int} does
not fire again until $t{=}2889\,\mu$s.
\end{casestudy}

\noindent\textbf{What capture 2's bracket means, in the deadline model's
own units.} The gap between the two \code{pl011\_int} services is
\begin{equation}
\Delta t_{\mathrm{pl011}} = 2889 - 10 = 2879\,\mu\text{s},
\label{eq:pl011gap}
\end{equation}
against $T_{\mathrm{dl}} = 1.28$\,ms (Eq.~\ref{eq:fifodeadline}), a factor
of $2.25\times$. At the byte arrival rate of Eq.~\ref{eq:fillrate}
($\tau_b = 40\,\mu$s), that gap spans
\begin{equation}
\frac{\Delta t_{\mathrm{pl011}}}{\tau_b} = \frac{2879}{40} \approx 72
\end{equation}
byte-arrival opportunities against a $32$-entry FIFO: not a near miss, a
gap wide enough for roughly $40$ bytes to have nowhere to go, in this one
occurrence alone. Both captures' worst-case latency exceeds
$T_{\mathrm{dl}}$ by a comparable margin ($2.21\times$ and $2.39\times$),
which is the first time this report has a single, named, directly-observed
mechanism wide enough to miss the deadline on its own, rather than an
inferred one.

\begin{trapbox}{What this does and does not establish}
Both captures name the same class of culprit --- \code{swiotlb}-bound
xHCI descriptor draining, in one case preceded by SD-card interrupt
handling that itself busy-waits with \code{\_\_udelay} --- and both are
consistent with the checksum storm's known properties: present with the
rover stationary (\S\ref{sec:aug11mins}), unaffected by
\code{serial\_node}'s scheduling priority or the kernel's RT budget
(item 40's ruled-out hypotheses), and only partially remedied by removing
thermal throttling (item 38, \S\ref{sec:aug13ab}) since a faster clock
drains the same $76$--$380$ \code{swiotlb} calls faster but does not
remove them. What two captures do not establish: that this is the
\emph{only} mechanism (a longer or repeated capture could surface a
different worst case), or that the task context shown in both traces'
header (\code{python3-43371}, \code{rt\_prio:25} --- almost certainly
\code{serial\_node} itself, the only userspace process at that priority)
means \code{serial\_node} is implicated in causing the delay, rather than
simply being whichever process the CPU happened to be running when a
hardware interrupt --- necessarily non-preemptible, regardless of which
process it interrupts --- arrived. Candidate next steps, none attempted
tonight: check whether the xHCI controller's DMA mask forces
\code{swiotlb} unnecessarily (a $64$-bit-capable mask would skip the bounce
buffer entirely); reduce the SD-card driver's use of busy-wait \code{udelay}
inside its IRQ handler if a kernel or DTS option controls it; check
\code{/proc/irq/*/smp\_affinity} for whether the xHCI and MMC interrupts
(MSI-based, unlike IRQ~$14$'s fixed GIC line) can be pinned away from
CPU0 entirely, which would remove the contention without touching the UART
side at all.
\end{trapbox}

\subsection{Two of three candidates checked, the same evening}

\paragraph{DMA mask: ruled out directly, not inferred.} The trapbox above
named the xHCI controller's DMA mask as a candidate. Checked directly
rather than assumed: \code{lspci -vv} reports the VL805 controller
(PCI $01{:}00.0$) with \code{dma\_mask\_bits} $=64$, and the platform's
own \code{dma-ranges} property (\code{/proc/device-tree/scb/dma-ranges},
decoded by hand against \code{\#address-cells}$=2$/\code{\#size-cells}$=2$)
maps a $16$\,GiB inbound window starting at physical address $0$ ---
double this robot's installed $7.6$\,GiB, so nothing on this board is
DMA-unreachable. This rules out address-range restriction as the reason
\S\ref{sec:aug13irqsoff}'s trace shows \code{swiotlb} bouncing; the cause
is elsewhere in the driver/allocator path, not identified tonight.

\paragraph{IRQ affinity: xHCI is hard-blocked by the kernel, MMC moved but the test was inconclusive.}
\code{/proc/interrupts} confirmed the exact contention pattern
\S\ref{sec:aug13irqsoff} inferred: IRQ~$14$ (UART), IRQ~$27$
(\code{mmc0}/\code{mmc1}) and IRQ~$43$ (\code{xhci\_hcd}) had each
delivered essentially $100\,\%$ of their interrupts to CPU0
($2.2$\,M, $18.6$\,M and $354$\,k respectively), despite an
\code{smp\_affinity\_list} of \code{0-3} on all three --- nothing was
actively rebalancing them. Writing a narrower mask to IRQ~$43$ failed
outright, twice, with \code{I/O error} on both the list and bitmask
interfaces: this specific Broadcom STB PCIe MSI controller does not
support runtime affinity changes for this interrupt at all, closing off
the most direct version of this mitigation. IRQ~$27$ (MMC) accepted the
same change. The before/after overrun-rate comparison this enabled was
inconclusive: three $30$\,s samples taken in sequence
($1.579\,\mathrm{s}^{-1}$ baseline, $2.632\,\mathrm{s}^{-1}$ with MMC
moved, $2.928$ and $2.961\,\mathrm{s}^{-1}$ after reverting to the
original affinity) drift upward across the whole window regardless of
which configuration was active, meaning a single sample before and
after cannot separate a real effect from ordinary time-varying load ---
exactly the discipline item 38's own matched-load protocol
(\S\ref{sec:aug13ab}) was built to enforce, and a single quick test here
did not. Configuration fully restored (\code{0-3} on all three,
confirmed) before moving on.

\paragraph{SD-card \code{udelay}: no runtime knob exists.} The third
candidate, checked last: \code{bcm2835\_mmc} (the driver bound to
\code{mmc0}, the eMMC2 controller carrying this Pi's SD card and root
filesystem, confirmed via \code{/sys/bus/mmc/devices/} and matching
\code{bcm2835\_mmc\_irq} in \S\ref{sec:aug13irqsoff}'s trace) exposes
exactly two module parameters, both read as $0$:
\code{/sys/module/bcm2835\_mmc/parameters/\{mmc\_debug,mmc\_debug2\}}
--- debug-logging verbosity flags by every naming convention in the
kernel, not timing controls. Nothing in this driver's exposed surface
lets the busy-wait be reduced or disabled without patching and
rebuilding the module itself, which was not attempted: this Pi's SD
card is also its boot device, and testing a rebuilt storage driver on
the only one this robot has, live, was judged the wrong trade for
tonight. All three candidates this item named are now checked, not
guessed at: one ruled out, one hard-blocked by the kernel, one with no
accessible knob.

\paragraph{MMC affinity, re-tested properly: still not confirmed, now for a
better reason.} The gap this section left open --- a single before/after
sample cannot separate a real effect from drift --- was closed the same
evening with an alternating protocol matching item 38's own discipline
(\S\ref{sec:aug13ab}): two $120$\,s windows with IRQ~$27$ on CPU$1$ and
two with it restored to \code{0-3}, in B--A--B--A order, configuration
confirmed restored after each switch.
\begin{center}
\begin{tabular}{@{}lcc@{}}
\toprule
& Round 1 & Round 2 \\
\midrule
B (CPU$1$) & $2.616\,\mathrm{s}^{-1}$ & $1.744\,\mathrm{s}^{-1}$ \\
A (\code{0-3}) & $2.450\,\mathrm{s}^{-1}$ & $2.558\,\mathrm{s}^{-1}$ \\
\bottomrule
\end{tabular}
\end{center}
Mean B $=2.180\,\mathrm{s}^{-1}$, mean A $=2.504\,\mathrm{s}^{-1}$, a
nominal $12.9\,\%$ reduction --- but B's own two samples span
$0.872\,\mathrm{s}^{-1}$ (standard deviation $0.617$), nearly three times
the $0.324\,\mathrm{s}^{-1}$ gap between the two means, while A's two
samples agree far more closely (spread $0.108$, standard deviation
$0.076$). A difference smaller than the noise within one of the two
groups it is drawn from is not a confirmed effect, whichever direction
it points. Four windows, honestly, is not enough power to settle this
either way; what this run adds over the earlier single-sample attempt
is knowing precisely how much power is missing; not that there is none.
What remains genuinely open is the same experiment run longer, or
repeated over more than one evening.

\section{sqrtVINS on the Raspberry Pi: installed, debugged, and diverged
--- a complete answer to a new request}
\label{sec:sqrtvins}

A new request arrived from Prof.\ Gutierrez the same evening: MINS is too
large to run in real time on the Raspberry Pi 4B, so it now runs on the
ground PC (already the case throughout this report, confirmed live:
\code{ps aux} on the PC shows \code{mins\_subscribe} at $84$--$85\,\%$
CPU, none of it on the robot). Carolus stays local to the Pi. A new
candidate, $\sqrt{\text{VINS}}$ (Peng et al., ICRA 2024 / TRO 2025,
\url{https://github.com/rpng/sqrtVINS}), was proposed as a lighter
alternative that could run locally on the Pi instead of shipping camera
frames to the PC: install it, test it, and compare its update rate and
accuracy against MINS on the lab's rectangular trajectory, with the
caveat --- stated in the request itself --- that $\sqrt{\text{VINS}}$
does not fuse wheel encoders and is therefore expected to be less
accurate than MINS on its own terms.

This section reports the complete result: installation succeeded, two
real defects were found and fixed the same evening, the system ran and
processed live sensor data in real time on the Pi, and then diverged
severely on an actual driven lap of the lab. All three are reported,
not just the parts that flatter the outcome.

\subsection{Installation}

$\sqrt{\text{VINS}}$ (package \code{ov\_srvins}) is an extension of
openVINS from the same lab (RPNG, University of Delaware), replacing the
conventional EKF/MSCKF covariance update with a Cholesky (LLT)
square-root filter and running in $32$-bit single precision
(\code{USE\_FLOAT=1}, confirmed directly in the compiler invocation ---
not merely claimed by the README). It ships \code{ov\_core}, \code{ov\_data}
and \code{ov\_eval} packages with the \emph{same names} as the openVINS
installation already running on this robot, so it was built in a fully
isolated workspace (\code{/home/pi/sqrtvins\_ws}) rather than the
existing \code{catkin\_ws}, to guarantee zero risk to the estimator this
report already relies on. Every system dependency
(\code{libceres-dev}, \code{libopencv-contrib-dev}, \code{libeigen3-dev})
was already present on the Pi. Build time: $104$ minutes
(\code{catkin build -j2}, deliberately capped to leave headroom on a
robot that was live throughout), zero errors.

\subsection{Two real defects, found by bisection, not by guessing}

\paragraph{Defect 1: an undocumented, incompatible config schema.} A
first config, built by porting this robot's proven openVINS values
(\S\ref{sec:july30thermal} and throughout this report) onto the classic
openVINS \code{estimator\_config.yaml} schema, crashed on every launch
before a single frame was processed:
\begin{center}
\code{terminate called after throwing an instance of}\\
\code{'boost::wrapexcept<boost::lock\_error>'}\\
\code{what():\ boost: mutex lock failed in pthread\_mutex\_lock: Invalid argument}
\end{center}
Bisected by rebuilding the config from the one file confirmed to run
without crashing (\code{config/euroc\_mav/estimator\_config.yaml}, the
package's own example) and re-adding this robot's tuned values one
field at a time, retesting after each addition. Every numeric value ---
calibration locks, measured gravity, KLT tuning, initialization
thresholds --- proved harmless in isolation and in combination. The
schema itself was the problem: \code{ov\_srvins} silently requires many
fields absent from classic openVINS configs (\code{init\_prior\_*},
\code{use\_imuavg}/\code{use\_rk4int} in place of
\code{integration:"rk4"}, and others), undocumented in the public
README, discovered only by diffing against the one config known to
work. A second, narrower trap surfaced while writing an explanatory
comment about this very fix: a code-review-style comment containing the
literal text \code{integration:"rk4"} (colon, no space, quoted value ---
resembling a real YAML assignment even though it was inside a \code{\#}
comment) reintroduced the identical crash. OpenCV's \code{FileStorage}
parser is not a full YAML parser; comments describing a bug can
apparently retrigger it. The deployed config avoids the pattern
entirely rather than relying on a fully characterised safe subset.

\begin{casestudy}{Defect 2: \code{try\_zupt: true} diverges catastrophically at rest, on this platform, with this filter}
This robot's proven openVINS value is \code{try\_zupt: true} --- on the
PC, it is documented elsewhere in this report as the correction that
took a divergence from $41\,824$\,m down to $0.002$\,m
(\S\ref{sec:july28zupt}). Porting it unchanged to $\sqrt{\text{VINS}}$ on the Pi, robot
stationary, produced the opposite: state velocity growing from
$|v_{IinG}| = 1780$ to $6015\,\text{m/s}$ across nine ZUPT updates in a
few seconds, each one logged as \emph{accepted} by the filter's own
$\chi^2$ consistency gate:
\begin{center}
\code{[ZUPT]: accepted |v\_IinG| = 6015.597 (chi2 165137575521026048.000 < 65.171)}
\end{center}
A $\chi^2$ residual of $1.65\times10^{17}$ against a threshold near
$65$ is not a near-miss gate failure, it is the gate reporting success
on a value seventeen orders of magnitude past it --- consistent with a
numerical representation problem rather than a modelling one, and the
one architectural fact that distinguishes this filter from every other
estimator in this report is single-precision arithmetic
(\code{USE\_FLOAT=1}, confirmed directly in the build above).
Confirmed by bisection to be caused by \code{try\_zupt} alone: identical
on every other setting to a configuration that ran stably for the full
length of every test conducted (up to $70\,\mathrm{Hz}$, repeatedly,
including a rebuild-from-scratch reproduction of the crash and its fix
the same evening). Deployed config: \code{try\_zupt: false}, the
opposite of the PC's proven setting, with the contrast documented
in-line in the config file itself.
\end{casestudy}

\subsection{Running, locally, on real sensor data}

With both defects fixed, $\sqrt{\text{VINS}}$ subscribed directly to
this robot's own camera and IMU topics
(\code{/camera/infra1|infra2/image\_rect\_raw}, no \code{/pc/camera/...}
prefix, no republish hop --- it runs on the same machine the D455
publishes from, so the WiFi bandwidth constraint that shapes
\S\ref{sec:july30thermal}'s republish architecture does not apply to
it at all) via a second, independently-named instance of this report's
own \code{imu\_sanitizer.py} (\code{imu\_sanitizer\_pi}, publishing
\code{/imu/data\_clean\_pi}, coexisting on the same ROS master as the
PC's instance without collision). Processing held between $30$ and
$70\,\mathrm{Hz}$ per frame with the robot stationary, correctly
reported ``no jerk detected, platform is stationary'' while waiting for
motion to initialize --- the identical behaviour already documented for
openVINS on this robot (\code{tools/record\_trajectories.sh}'s own
header note).

\begin{trapbox}{An unscheduled power interruption, mid-session, recovered cleanly}
The Pi lost power entirely partway through this investigation (confirmed:
operator checked physically, ``il était pas allumé''). Recovery required
exactly two commands (relaunch \code{imu\_sanitizer\_pi}, relaunch
\code{ov\_srvins}) since the build and config live on disk, not memory.
Separately, MINS on the PC survived the robot's reboot as a live process
but stopped appearing in the ROS graph --- the identical staleness class
already named in \S\ref{sec:aug11stale}
($E_P(t) > U(t)$, Eq.~\ref{eq:staleness}): the process outlived the robot
session that established its ROS-graph registrations. \code{tools/restart\_stack.sh},
already built for exactly this failure mode, resolved it in $8$\,s.
\end{trapbox}

\subsection{The driven test: three estimators, one lap, one clear result}

\begin{table}[htbp]
\centering
\small
\caption{Same lap of the lab, three estimators recorded simultaneously
in one bag ($415$\,s, $104701$ messages). Loop closure error = distance
between the first and last recorded position (a proxy for accumulated
drift, same methodology as \code{compare\_tests.m} elsewhere in this
report).}
\label{tab:sqrtvins_compare}
\begin{tabular}{@{}lccc@{}}
\toprule
Estimator & Messages & Position range (m) & Loop closure error \\
\midrule
MINS (PC, wheels+cam+IMU) & $32\,802$ & $x\in[0.0,11.8]$, $y\in[-0.2,4.7]$ & $\mathbf{1.02}\,\text{m}$ \\
openVINS (PC, cam+IMU) & $31\,889$ & $x\in[-168,3366]$, $y\in[-652,0]$ & $541.7\,\text{m}$ \\
$\sqrt{\text{VINS}}$ (Pi, cam+IMU) & (diverged before capture$^\ast$) & $x,y,z$ in the thousands & unbounded \\
\bottomrule
\end{tabular}
\end{table}
\noindent $^\ast$\textbf{Correction, 2026-08-18.} This footnote previously
read that \code{/ov\_srvins/odomimu} ``never reached the PC-side
\code{rosbag record} process despite confirmed local publication on the
Pi'' --- an explanation formed during the session and never checked
afterwards. Re-reading the bag settles it: \code{rosbag info} lists
\emph{five} topics, and that one is not among them. Not recorded-and-empty,
\textbf{never requested}. \code{tools/record\_trajectories.sh} simply did
not carry \code{/ov\_srvins/odomimu} in its topic list, so no network
explanation was ever needed. The line is added, with a preflight check that
treats its absence as normal rather than alarming, since \code{sqrtVINS}
runs on the Pi on demand and not as part of the PC stack. The divergence
figures quoted above remain reconstructed from
$\sqrt{\text{VINS}}$'s own node log, timestamped and un-editable after the
fact --- that part was never in doubt. This is the third time in this
campaign that an explanation given live proved wrong on re-reading; the
lesson is recorded in \S\ref{ap:repro-checklist}'s neighbourhood and holds
here too.

$\sqrt{\text{VINS}}$'s own log lets the divergence be dated precisely
against distance travelled, not just against a final number:
\begin{center}
\begin{tabular}{@{}lcccccc@{}}
\toprule
Frame \# & 1 & 700 & 900 & 1000 & 2000 & 6293 (last) \\
\midrule
Reported dist.\ (m) & $0.0$ & $19.3$ & $128.8$ & $275.4$ & $6093.6$ & $43\,742.6$ \\
\bottomrule
\end{tabular}
\end{center}
Up to frame $\sim\!700$ ($\approx 19$--$20$\,m), the reported distance is
consistent with a real lab lap. Past that point the growth rate itself
accelerates rather than staying linear --- a runaway, not a step
failure: once the estimate is wrong enough to corrupt the feature
geometry it depends on, each subsequent update makes the next one
worse, with nothing left to catch it. This is the specific failure mode
absent zero-velocity updates predicts, and Defect 2 above is exactly
why they were absent here.

\subsection{What this settles, and what it does not}

Every part of the original request now has a real answer, not a
promise of one. $\sqrt{\text{VINS}}$ \emph{does} install and run on a
Raspberry Pi 4B in real time (the README's claim is not merely
repeated here, it is independently confirmed at $30$--$70\,\mathrm{Hz}$
on this robot's own sensors). Its update rate, while numerically
healthy, was never the limiting factor. Its accuracy, on the one lap
this evening produced, was not just lower than MINS's --- expected,
stated in the request itself, and consistent with every other
camera+IMU-only estimator in this report --- it was categorically worse
than even vanilla openVINS on the \emph{identical} lap ($43\,000$\,m of
accumulated reported distance against $541$\,m of final displacement
error). The most defensible reading is not ``$\sqrt{\text{VINS}}$ is
unusable,'' it is ``the specific configuration deployed tonight,
running without zero-velocity updates because the alternative crashed
outright, was not enough to keep this filter bounded through a real,
moderately dynamic lap on this hardware.''

\begin{trapbox}{The disparity-only path was tried, the same evening: worse, not better}
Two candidate directions were named above as untried. The first, a
write-up of Defect 2 for RPNG's own issue tracker, is drafted and ready
to submit, now covering both ZUPT sub-modes below. The second, the
disparity-only path (\code{zupt\_chi2\_multipler: 0}) as a middle
ground between ``no velocity anchoring'' and ``the IMU-based setting
that diverges,'' was tested at rest the same evening --- and is not a
middle ground. The deployed, stable configuration already carried
\code{zupt\_chi2\_multipler: 0}; the entire change tested was
\code{try\_zupt: false} $\to$ \code{true}. Within roughly a dozen
updates, well under one second, the filter reached
\begin{center}
\code{[ZUPT]: accepted |v\_IinG| = 203734831702474752.000 (chi2 inf < 0.000)}
\end{center}
--- not merely a large finite $\chi^2$ as in Defect 2's IMU-path
capture, but literal floating-point infinity, against a printed
threshold that itself collapsed to \code{0.000}, consistent with the
consistency check's denominator vanishing rather than its numerator
merely overflowing. Reverted immediately, config restored and
confirmed. Neither ZUPT sub-mode is usable in this build; a real
driven test of the disparity-only path was not attempted, since a
mode that reaches \code{inf} in under a second standing still has
nothing left to test by moving.
\end{trapbox}

\subsection{A source patch: reading \code{UpdaterZeroVelocity.cpp} instead of just reporting it}
\label{sec:sqrtvins-patch}

The write-up mentioned above as ``drafted and ready to submit'' was
finished by actually reading the file it describes, rather than
stopping at the printed symptom. Two things stood out in
\code{ov\_srvins/src/update/UpdaterZeroVelocity.cpp}:

\begin{enumerate}
\item The $\chi^2$ consistency check runs \code{S.llt().solve(res)}
  without ever inspecting \code{.info()}. Eigen's documented
  behaviour, if the \code{LLT} decomposition of the innovation
  covariance $S$ fails (not positive-definite --- plausible under
  single precision as terms accumulate), is to return silently from
  \code{.solve()} with garbage rather than signalling failure.
  Nothing downstream checks for it.
\item In the disparity-only path, the reject condition is written
  \code{if (!disparity\_passed \&\& (chi2 > thresh || vel > max\_vel))
  reject;}. When \code{disparity\_passed} is true --- the common case
  at rest, since disparity stays small while stationary --- the
  entire $\chi^2$/velocity gate short-circuits out and the update is
  accepted \emph{unconditionally}. The printed
  \code{(chi2 inf < 0.000)} in \S\ref{sec:sqrtvins}'s log is cosmetic
  in this branch: nothing ever compares against it.
\end{enumerate}

\begin{casestudy}{A minimal, additive guard, tested against the real symptom it targets}
The patch adds an unconditional numerical-validity check ahead of the
existing reject block, changing no tuning value or threshold:
...
if (!chi2_numerically_valid) {
  ...
  last_zupt_count_ = 0;
  PRINT_WARNING(RED "[ZUPT]: rejected -- chi2 numerically invalid "
                    "(llt_success=%d, chi2=%.6e, disparity_passed=%d)\n" RESET, ...);
  return false;
}
\end{lstlisting}
Applied on the Pi's own \code{sqrtVINS} clone (\code{git diff}: one
file, $38$ insertions, $1$ deletion --- attached in full as
\code{docs/sqrtvins\_zupt\_numerical\_guard.patch}), rebuilt
(\code{catkin build ov\_srvins}, $5$\,min\,$29$\,s, ``All $2$ packages
succeeded''), and run live for $60$\,s against real stereo+IMU data,
robot stationary, disparity-only path, \code{try\_zupt: true}: $588$
updates correctly rejected as numerically invalid, $241$ correctly
accepted with sane $\chi^2$ values, zero acceptances of an
\code{inf}/garbage $\chi^2$. The specific bug this section opened
with --- silent acceptance of an astronomically-out-of-threshold
residual --- no longer reproduces, verified against the actual log,
not inferred from the absence of a crash.
\end{casestudy}

\begin{trapbox}{The patch is real; it does not close the case}
With every garbage ZUPT update now correctly rejected, the same
$60$\,s stationary run still diverges: \code{p\_IinG} grows smoothly
--- no longer via ZUPT chatter, since ZUPT is now provably not
contributing it --- from roughly $10^{15}$\,m to roughly
$3.4\times10^{17}$\,m over the window. That rules out the ZUPT
updater as \emph{the} divergence source and narrows it to a second,
independent numerical-stability issue elsewhere in the pipeline ---
propagation or the MSCKF update are the natural suspects, since ZUPT
is now demonstrably not the vector. That second issue has not been
diagnosed: it would mean reading \code{Propagator.cpp},
\code{UpdaterMSCKF.cpp} and \code{StateHelper.cpp} with the same
discipline applied here, a distinct, larger piece of work not yet
started. Per this investigation's own stated condition --- a driven
test only once the estimator is stable at rest --- it is not: no
driven test was run against the patched build, and none should be
claimed as validated until the second source is found. The patch is
offered to RPNG as a real, tested fix for the logic bug it targets,
not as a fix for the platform's divergence as a whole.
\end{trapbox}

\section{The Carolus quaternion, traced to source: why sixteen trials were
never going to converge on a general answer}
\label{sec:quaternion}
The supervisor's question, prompted by two independent deployments (LIMO,
\S\ref{sec:limodoc}; this platform) each finding a working sign convention by
exhaustive trial, was direct: is there a general solution, one that would also
work on a Robomaster with no prior Carolus deployment at all? The honest
answer is not a single quaternion, portable across mounts, that number cannot
exist, since three different physical camera mountings have three different
correct answers by construction. What this section delivers instead is
stronger: the exact mechanism, traced into Carolus's own source rather than
inferred from its behaviour, that made sixteen-trial search the only available
method until now, and a closed-form procedure that replaces the search with
one measured calibration motion, on any robot, Robomaster included.

\subsection{Two prior deployments, both solved by search}
\label{sec:limodoc}
Two lab-internal guides document independent Carolus integrations, both
predating this section. \emph{``How to use Carolus to make the LIMO follow
the beacon''} (T.~Brezins, LIMO rover, ROS Noetic) found, by exhaustive
sign search over $2^4=16$ combinations with the axis-letter permutation
already fixed, the working conversion
$X_{\text{robot}}=-Z_c,\,Y_{\text{robot}}=-X_c,\,Z_{\text{robot}}=-Y_c$ for
translation and \code{rotation.x,y,z,w}
$=(-o_z,\,o_x,\,-o_y,\,-o_w)$ for orientation, and states plainly: \emph{``if
you not use this Rover, you must find your own quaternion.''} A companion
guide for this Leo Rover (\S\ref{ap:repro-mins-guide}'s Carolus chain)
independently found the identical translation letters and signs, but a
\emph{different} orientation sign combination,
$(-o_z,\,o_x,\,o_y,\,o_w)$, reporting the same $2^4$-combination search and
the same warning. Two of four rotation signs differ between the two working
conventions despite an otherwise near-identical forward-facing colour-camera
mount, concrete, sourced evidence that no fourth robot can safely reuse
either number without its own verification.

\subsection{What is genuinely universal: the optical-to-body rotation}
\label{sec:quatuniversal}
Carolus (package \code{carolus\_astrobee}, derived from NASA's Astrobee
free-flyer code) solves a P4P problem in Ceres and publishes a
\code{geometry\_msgs/PoseStamped} on \code{/pose}. Its cost function, read
directly from \code{ceresP4P.hpp:120--150}, is the textbook calibrated pinhole
model, nothing platform-specific in it:
\begin{lstlisting}[language=C++]
ceres::AngleAxisRotatePoint(camera_R.data(), target_point, camera_point);
camera_point[0] += camera_T[0]; camera_point[1] += camera_T[1];
camera_point[2] += camera_T[2];
double xp = camera_point[0] / camera_point[2];   // pinhole: x/z
double yp = camera_point[1] / camera_point[2];   // pinhole: y/z
predicted_point[0] = xp*focalx_length_ + cx_;
predicted_point[1] = yp*focaly_length_ + cy_;
\end{lstlisting}
This is $X_{\text{cam}} = R\,X_{\text{world}} + T$ followed by division by the
\emph{third} coordinate, i.e.\ the standard OpenCV/optical convention: $X$
right, $Y$ down, $Z$ forward (out of the lens, increasing with depth). With
the known beacon points defined about their own origin, $X_{\text{world}}=0$
gives $X_{\text{cam}}=T$: the published translation is genuinely the
beacon's position in the camera's own optical axes, no hidden library
convention, verified by reading the optimiser, not assumed from the output.

ROS body-style frames (REP~103, the convention \code{base\_link} and every TF
in this project's tree already use) place $X$ forward, $Y$ left, $Z$ up. A
point with optical coordinates $(X_o,Y_o,Z_o)$ has body-style coordinates
obtained by matching what each axis physically means: forward is the optical
depth axis, left is minus optical right, up is minus optical down:
\begin{equation}
\begin{pmatrix}X_b\\Y_b\\Z_b\end{pmatrix}
= R_{ob}\begin{pmatrix}X_o\\Y_o\\Z_o\end{pmatrix}, \qquad
R_{ob} = \begin{pmatrix} 0&0&1\\ -1&0&0\\ 0&-1&0 \end{pmatrix}.
\label{eq:rob}
\end{equation}
$R_{ob}$ is orthogonal ($R_{ob}R_{ob}^\top=I$) with $\det R_{ob}=+1$: a proper
rotation, not a reflection, exactly as a fixed relabelling of one rigid
camera's own axes must be. \textbf{This matrix is universal.} It follows from
the two convention definitions alone (REP~103, REP~104/OpenCV) and depends on
neither the beacon, the solver, nor the robot. It is the entire content of
the ``letter correspondence'' both the LIMO and this platform's own
documentation found empirically ($X_{\text{robot}}=\pm Z_c$,
$Y_{\text{robot}}=\pm X_c$, $Z_{\text{robot}}=\pm Y_c$, \S\ref{sec:limodoc}):
Eq.~\ref{eq:rob} is the reason the \emph{letters} always matched across two
unrelated mounts, and derives in three lines what both predecessor documents
found by pattern-matching two independent trial-and-error searches against
each other.
...
\begin{tikzpicture}[font=\small, >=Latex, scale=1.0]
  ...
  \begin{scope}
    ...
    \node[font=\scriptsize] at (0,-0.7) {camera (optical)};
    \draw[->, accentblue] (0,0) -- (1.1,0) node[right, font=\scriptsize] {$X_o$ right};
    \draw[->, accentblue] (0,0) -- (0,-1.0) node[below, font=\scriptsize] {$Y_o$ down};
    \draw[->, alertred, very thick] (0,0) -- (0.75,0.75) node[above right, font=\scriptsize] {$Z_o$ forward (depth)};
  \end{scope}
  % body frame, right
  \begin{scope}[xshift=6.6cm]
    \draw[thick, rounded corners=2pt] (-0.5,-0.35) rectangle (0.5,0.35);
    \node[font=\scriptsize] at (0,-0.7) {robot body (REP-103)};
    \draw[->, alertred, very thick] (0,0) -- (1.1,0) node[right, font=\scriptsize] {$X_b$ forward};
    \draw[->, accentblue] (0,0) -- (-0.75,0.75) node[above left, font=\scriptsize] {$Y_b$ left};
    \draw[->, accentblue] (0,0) -- (0,1.1) node[above, font=\scriptsize] {$Z_b$ up};
  \end{scope}
  \draw[->, densely dashed] (1.4,0.1) -- (4.9,0.1) node[midway, above, font=\scriptsize] {$R_{ob}$, Eq.~\ref{eq:rob}};
\end{tikzpicture}
\caption[Optical to body frame]{The one universal piece: optical depth
($Z_o$, red) becomes body forward ($X_b$, red) on every camera, on every
robot, because both conventions are fixed definitions, not measurements.
Nothing about this diagram is platform-specific.}
\label{fig:opticalbody}
\end{figure}

\subsection{What is not universal, traced to the exact line that breaks it}
\label{sec:quatbug}
If Eq.~\ref{eq:rob} were the whole story, applying it to both translation and
rotation would settle the question on any robot in one derivation, no search
required. It does not, and the reason is a specific, quotable inconsistency
in \code{carolus\_astrobee.cpp} between how translation and rotation are
published, not a deeper property of optics.

Immediately after the solve, translation is published as the solver's raw
output, \code{PoseAstrobeeMsgs.pose.position = bestPose.t} verbatim
(\code{carolus\_astrobee.cpp:1180--1182}). Rotation is not:
\begin{lstlisting}[language=C++]
// TRANSPOSE TO GET THE CORRECT ROTATION MATRIX -- IF NOT TRANSPOSED,
// THE ROTATION MATRIX IS INVERTED
// DOUBLE CHECK THIS ITS 5AM
Eigen::Quaterniond q(bestPose.R.transpose());
\end{lstlisting}
(\code{carolus\_astrobee.cpp:1185--1187}, comments verbatim). This is not this
project's guess about upstream intent: it is the original authors' own,
timestamped uncertainty, left in the code. For a rigid transform
$X_{\text{cam}}=R\,X_{\text{world}}+T$, the self-consistent inverse (``where
is the camera, expressed in the world/beacon frame'') is
\begin{equation}
R_{\text{inv}} = R^\top, \qquad T_{\text{inv}} = -R^\top T.
\label{eq:properinv}
\end{equation}
\code{carolus\_astrobee.cpp} publishes $R_{\text{inv}}=R^\top$ but
$T_{\text{pub}}=T$, not $-R^\top T$. The published pair is a hybrid: an
inverted rotation paired with a non-inverted translation, which is not one
consistent SE(3) transform in either direction. \textbf{This is precisely why
no single textbook rotation, applied uniformly to both position and
orientation, was ever going to reconcile the empirical sign findings}: the
two components are not related by the same convention to begin with, so
searching for one $4\times4$ (or one quaternion) that fixes both at once is
searching for something that, given the source above, does not exist. Both
predecessor documents' sixteen-combination sweeps (\S\ref{sec:limodoc}) were
the correct response to this exact bug, not a shortcut around a cleaner
answer that was available and missed.

\subsection{The general method: one measured calibration, not sixteen trials}
\label{sec:quatgeneral}
The mismatch above is specific to this one upstream file. A Robomaster
integration would not inherit this exact bug, it might have none, or a
different one in whatever P4P/PnP code it uses. \textbf{A method that first
diagnoses this file would not be general.} The method that is general treats
Carolus (or any beacon detector) as an uncalibrated black box and solves for
whatever fixed correction it needs directly from measurement, which is
correct regardless of what is or is not consistent inside the box.

\paragraph{Setup.} Let $(R_c^{(i)}, t_c^{(i)})$ be Carolus's raw published
rotation and translation at calibration pose $i$, and let $(R_g^{(i)},
t_g^{(i)})$ be the same rover pose measured by ground truth (tape measure and
a marked heading, as already established for this platform's floor protocol,
\S\ref{ap:repro-calibrun}). The unknown is a single, fixed calibration
rotation $C$ such that
\begin{equation}
R_g^{(i)} = C\, R_c^{(i)}\, C^\top \quad\text{or}\quad R_g^{(i)} = C\, R_c^{(i)}
\label{eq:calibgeneral}
\end{equation}
for every $i$ (the first form if $C$ conjugates a rotation expressed in
Carolus's frame into the ground frame; the second if $C$ instead re-expresses
a rotation already acting on ground-frame coordinates, which of the two
applies is itself determined, not assumed, by the residual in step~3 below).

\paragraph{Procedure.}
\begin{enumerate}[leftmargin=1.4em]
\item \textbf{One reference pose is not enough for rotation}, only for
  translation's axis permutation and per-axis sign (three components, three
  independent equations, generically enough with one non-degenerate sighting
  once the beacon is not on an axis of the rover). Rotation about the
  optical axis itself is unobservable from a single sighting at a generic
  angle: take a \emph{second} pose related to the first by a known,
  commanded rotation (this platform already has a scripted primitive for
  exactly this, \code{90\textdegree\ G}/\code{90\textdegree\ D} on the
  Trajectory tab, \S\ref{ap:repro-calibrun}).
\item \textbf{Solve for $C$ in closed form.} With two or more
  $(R_c^{(i)}, R_g^{(i)})$ pairs, Eq.~\ref{eq:calibgeneral} is a Wahba's
  problem: minimise $\sum_i \lVert R_g^{(i)} - C R_c^{(i)} \rVert_F^2$ over
  the rotation group. The closed-form solution is the same SVD construction
  already implemented in this project's own Matlab tooling for trajectory
  alignment (\code{local\_kabsch2d}, \S\ref{ap:repro-comprun}, here in 3D
  rather than 2D and applied to orientations rather than positions):
  $H=\sum_i R_c^{(i)\top} R_g^{(i)}$, $H=U\Sigma V^\top$,
  $C = V\,\mathrm{diag}(1,1,\det(VU^\top))\,U^\top$. Two poses related by a
  rotation about a vertical axis already fully constrain $C$; a third,
  non-coplanar pose is a check, not a requirement.
\item \textbf{Translation calibration is then a linear solve, not a search.}
  With $C$ fixed, $t_g^{(i)} \approx C\,t_c^{(i)} + d$ for a fixed offset
  $d$ (the beacon-to-ground-origin displacement); two or more sightings give
  an over-determined linear system, solved by least squares.
\item \textbf{Validate, don't assume convergence.} Apply $(C,d)$ to a
  held-out sighting not used in the fit; residual position error should be
  within the beacon's own known measurement noise
  ($\sim$5\,cm, \code{config\_vicon.yaml}, \S\ref{ap:repro-mins-guide}), not
  simply small relative to the uncorrected error.
\end{enumerate}
This is the answer to the supervisor's actual question. It requires no prior
knowledge of Carolus's internal bug, no inspection of anyone's source code,
and generalises unchanged to Robomaster or any other platform: two measured
poses, one closed-form SVD, one linear solve, one held-out check. Sixteen (or
$2^4=16$, or $24\times16=384$, \S\ref{sec:limodoc}) blind trials are not a
step of this procedure at any point.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\small, >=Latex,
  box/.style={draw, rounded corners=2pt, align=center, fill=black!3,
              text width=3.0cm, inner sep=5pt}]
  \node[box] (p1) at (0,1.15)
       {\textbf{pose 1}\\\scriptsize $(R_c^{(1)}, t_c^{(1)})$ raw\\\scriptsize $(R_g^{(1)}, t_g^{(1)})$ measured};
  \node[box] (p2) at (0,-1.15)
       {\textbf{pose 2}\\\scriptsize known rotation from 1\\\scriptsize $(R_c^{(2)}, t_c^{(2)})$ raw\\\scriptsize $(R_g^{(2)}, t_g^{(2)})$ measured};
  \node[box, fill=blue!6, draw=accentblue, text width=3.6cm] (svd) at (5.2,0)
       {\textbf{closed-form SVD}\\\scriptsize (Wahba/Kabsch, Eq.~\ref{eq:calibgeneral})\\$\Downarrow$\\$C$ (rotation), $d$ (offset)};
  \node[box, text width=2.6cm] (check) at (9.7,0)
       {\textbf{held-out pose 3}\\\scriptsize residual $<$ 5\,cm\,?};
  \draw[->] (p1.east) -- ++(0.5,0) |- (svd.west);
  \draw[->] (p2.east) -- ++(0.5,0) |- (svd.west);
  \draw[->] (svd) -- (check);
\end{tikzpicture}
\caption[General quaternion calibration procedure]{The general method: two
measured poses fix $C$ in closed form (no search), a linear solve fixes the
translation offset, a third pose validates. Portable to any beacon detector
on any robot, since it never inspects the detector's internals.}
\label{fig:quatcalib}
\end{figure}

\begin{trapbox}{Not yet run on this platform, or on Robomaster}
Everything above is derived and, for the optical-to-body rotation
(Eq.~\ref{eq:rob}) and the source-code inconsistency (\S\ref{sec:quatbug}),
directly verified against \code{ceresP4P.hpp} and \code{carolus\_astrobee.cpp}.
The calibration procedure of \S\ref{sec:quatgeneral} has not yet been
executed, on this rover or any other: it requires two physical, tape-measured
poses and the \code{90\textdegree} scripted primitive already available on
the Trajectory tab, a field session this report does not yet contain. The
existing empirical sign convention already locked into
\code{carolus\_tf\_bridge.py}'s \code{\~{}quat\_signs} default
(\code{"-+++"}, \S\ref{sec:quaternion}) continues to be what this platform
actually runs until that calibration session replaces it with a measured,
closed-form value.
\end{trapbox}

\section{Relocalization theory: why an absolute fix corrects accumulated
drift, and by how much}
\label{sec:relocmath}
Item 5's VICON-slot integration (\S\ref{ap:repro-mins-guide},
\code{config\_vicon.yaml}) is implemented and unenabled
(\S\ref{sec:aug11mins} already established this precisely). What follows is
the theory the supervisor's requested straight-line experiment
(\S\ref{sec:relocexp} below) will measure against: not a description of what
should happen, but a derivation of what a correctly-tuned filter is
mathematically required to do, so the eventual field result can be judged
against a real prediction rather than a vague expectation of ``it should
snap back.''

\subsection{Drift as a growing covariance, not a growing error}
\label{sec:driftmodel}
Between absolute fixes, MINS propagates position purely by integrating IMU
and wheel measurements, each corrupted by process noise. For a filter
tracking position error as a random walk driven by noise intensity $q$
(the propagation step of any Kalman-family estimator, \code{config\_estimator.yaml}'s
own noise densities being exactly this $q$, already documented as measured
via Allan variance rather than datasheet, \S\ref{ap:repro-mins-guide}), the
estimator's own belief about its uncertainty evolves as
\begin{equation}
P(t) = P(t_0) + q\,(t - t_0),
\label{eq:covgrowth}
\end{equation}
variance growing \emph{linearly} in the time since the last correction, not
the position error itself, which is a single unknown realisation, not a
distribution. This distinction matters for reading the experiment correctly:
a single straight-line run's actual drift is one sample from a distribution
whose spread grows by Eq.~\ref{eq:covgrowth}, so one run confirms an order of
magnitude, not the growth law itself, which would need repeated trials at
matched duration to fit.

\subsection{The correction is a precision-weighted average, not a reset}
\label{sec:kalmanfix}
When a Carolus-derived fix $z = x + v$ arrives, $v\sim\mathcal N(0,R)$ (this
platform's own \code{config\_vicon.yaml}: \code{noise\_p}$=0.05$\,m, so
$R\approx(0.05)^2 = 2.5\times10^{-3}\,\text{m}^2$ per axis), the scalar
Kalman update gives the corrected estimate and its variance as
\begin{align}
K &= \frac{P}{P+R}, \label{eq:gain}\\
x^{+} &= x^{-} + K\,(z - x^{-}), \label{eq:xupdate}\\
P^{+} &= (1-K)\,P^{-} = \frac{P^{-} R}{P^{-}+R}
      = \left(\frac{1}{P^{-}} + \frac{1}{R}\right)^{-1}. \label{eq:pupdate}
\end{align}
Eq.~\ref{eq:pupdate}'s last form, precisions (inverse variances) summing
rather than variances averaging, is the exact reason a beacon fix does not
merely nudge the estimate but effectively overrides it once drift has run
long enough: if $P^{-}\gg R$ (accumulated uncertainty far exceeds the
beacon's own $2.5\times10^{-3}\,\text{m}^2$), Eq.~\ref{eq:gain} gives
$K\to 1$ and Eq.~\ref{eq:pupdate} gives $P^{+}\to R$, independent of how
large $P^{-}$ actually was. \textbf{The correction magnitude is therefore
not a tuned behaviour, it is what the arithmetic in Eq.~\ref{eq:xupdate} does
automatically once the beacon's precision dominates}: the ``jump'' the
supervisor's protocol asks the student to observe (\S\ref{sec:relocexp}) is
the innovation term $K(z-x^{-})$, which for $K\approx1$ is approximately the
\emph{entire} accumulated error at that instant, not a fraction of it.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\small, >=Latex]
  \draw[->] (0,0) -- (10.2,0) node[right, font=\scriptsize] {$t$};
  \draw[->] (0,0) -- (0,4.1) node[left, font=\scriptsize, align=center] {position\\error};
  % growing drift band before the fix, sqrt(P) envelope
  \draw[domain=0:5.6, samples=60, densely dashed, alertred]
       plot ({\x}, {0.5*sqrt(\x)});
  \draw[domain=0:5.6, samples=60, densely dashed, alertred]
       plot ({\x}, {-0.5*sqrt(\x)});
  \draw[alertred] (0,0) .. controls (2,0.9) and (4,1.6) .. (5.6,1.9);
  \node[alertred, font=\scriptsize, anchor=south] at (2.8,1.0) {actual drift, one realisation};
  \node[alertred, font=\scriptsize, anchor=north, align=center] at (4.6,-1.5)
       {$\pm\sqrt{P(t)}$ envelope\\(Eq.~\ref{eq:covgrowth}, grows $\propto\sqrt{t}$)};
  % correction instant
  \draw[thick, densely dotted] (5.6,-2.2) -- (5.6,3.9);
  \node[font=\scriptsize, anchor=south west] at (5.6,3.9) {beacon fix};
  \fill (5.6,1.9) circle (1.6pt);
  \draw[->, very thick, accentblue] (5.6,1.9) -- (5.6,0.15);
  \node[accentblue, font=\scriptsize, anchor=west, align=left] at (5.75,1.05)
       {$K(z-x^-)$\\(Eq.~\ref{eq:xupdate})};
  % post-fix, tight band around zero
  \draw[domain=5.6:10, samples=40, densely dashed, green!55!black]
       plot ({\x}, {0.13+0.05*sqrt(\x-5.6)});
  \draw[domain=5.6:10, samples=40, densely dashed, green!55!black]
       plot ({\x}, {-0.13-0.05*sqrt(\x-5.6)});
  \draw[green!55!black, very thick] (5.6,0.1) -- (10,0.1);
  \node[green!55!black, font=\scriptsize, anchor=north west] at (6.0,-0.35) {$P^+\to R$ (Eq.~\ref{eq:pupdate}), drift resumes from here};
\end{tikzpicture}
\caption[Relocalization correction, predicted]{The predicted shape of the
straight-line experiment (\S\ref{sec:relocexp}), derived, not measured: an
uncertainty envelope opening as $\sqrt t$ while no fix arrives (dashed red),
an abrupt correction sized by the innovation at the instant a fix lands
(blue arrow, Eq.~\ref{eq:xupdate}), and a tight, beacon-precision envelope
resuming immediately after (dashed green). No data from this rover appears
in this figure; it is what Eq.~\ref{eq:covgrowth}--\ref{eq:pupdate} require,
against which a real run can be checked.}
\label{fig:relocpredicted}
\end{figure}

\subsection{The straight-line experiment this predicts, not yet run}
\label{sec:relocexp}
The supervisor's protocol, precisely: record a rosbag through
\code{tools/record\_trajectories.sh} (\S\ref{ap:repro-comprun}) while
driving a long straight line with the beacon initially out of view, exposing
it partway through, either by manual driving or a scripted straight-line
primitive (the encoder-calibration scripts, \S\ref{ap:repro-wheels}, already
show the pattern: subscribe to odometry, integrate distance, gate a
\code{Twist} publisher on a threshold). \code{tools/plot\_relocalization.m}
(written this session, \S\ref{sec:aug12readiness} below) locates the
correction instant from the first timestamp in \code{*\_carolus.csv} and
reports the jump magnitude directly, the same $K(z-x^{-})$ quantity
Eq.~\ref{eq:xupdate} predicts.

\begin{trapbox}{Not yet run, on this platform or any other}
Everything in this section is a derivation from standard linear-Gaussian
estimation theory (Eq.~\ref{eq:covgrowth}--\ref{eq:pupdate}) applied to
\emph{this platform's own measured numbers} (\code{config\_vicon.yaml}'s
$R\approx2.5\times10^{-3}\,\text{m}^2$), not a report of anything observed.
It is gated on the same prerequisites already recorded in
\S\ref{sec:aug11mins} and \S\ref{sec:aug12readiness}: \code{vicon.enabled}
is \code{false}, the beacon's global coordinates are unsurveyed, and the
live UART checksum storm found the same night this theory was written
(\S\ref{sec:aug11mins}) is an open, unresolved threat to
\code{/firmware/wheel\_states}, one of MINS's three fused inputs, which
would corrupt exactly the propagation phase Eq.~\ref{eq:covgrowth} models.
Figure~\ref{fig:relocpredicted} is a prediction to test against, not a
result.
\end{trapbox}

\subsection{Code audit, same session: one real bug, one missing tool, one
name correction}
\label{sec:aug12readiness}
Before extending the theory above, the code it depends on was audited
directly rather than assumed current from earlier documentation.
\code{carolus\_vicon\_bridge.py} and \code{config\_vicon.yaml}
(\S\ref{ap:repro-mins-guide}) were re-read in full: both already met the
standard this audit checked for (explicit exception handling with no crash
path when the beacon is out of view; every configuration field commented),
no change made. Two real gaps were found and closed:
\begin{itemize}[leftmargin=1.4em]
\item \code{tools/plot\_relocalization.m}, referenced by name in
  \S\ref{sec:relocexp} and in earlier drafts of this guide, had only ever
  been drafted as chat text, never written to \code{tools/}. Written for
  real this session, with explicit error handling for every missing- or
  empty-CSV case (a bare file-not-found stack trace is not an acceptable
  failure mode for a script meant to run unattended after a field session);
  verified against a nonexistent path, confirmed to raise a named, readable
  error rather than a raw one.
\item \code{leo\_backend.py}'s AprilTag long-range fallback
  (\S\ref{sec:apriltagfallback}) carried its $0.7$\,s freshness threshold as
  a bare literal, duplicated in two call sites. Promoted to a named
  constant, \code{TAG\_FALLBACK\_MAX\_AGE\_S}, both sites updated,
  \code{py\_compile} clean.
\end{itemize}
A separate, independent cross-check (Table~\ref{tab:openitems}'s item~1
requirement, ``the TF2 node handling the \code{beacon\_link} anchor'')
found that the file matching this description in this project's own tree is
\code{carolus\_tf\_bridge.py}, not the generic example this guide's
Section~1 originally illustrated the mechanism with: the generic example
(modelled on the LIMO document, \S\ref{sec:limodoc}) republishes
\code{base\_link}$\to$\code{camera\_frame} as a static transform, which
this platform's own TF tree already carries (verified by listening to
\code{/tf} directly, \S\ref{ap:repro-launcharch}), so republishing it would
give \code{base\_link} two parents and break the tree. The real node's own
\code{\~{}publish\_body\_chain} guard (default \code{false}) exists
specifically to prevent this, with an explicit \code{rospy.logerr} if
enabled without verification first, a stronger and more accurate artefact
for this guide than the illustrative example it replaces.

\subsection{Foundation for Section 2: the existing Carolus/AprilTag
arbitration, re-verified}
\label{sec:apriltagfallback}
The motion-control appendix that follows (\S\ref{ap:posecontrol}) issues
commands through the same target-acquisition path already live in
\code{leo\_backend.py}, not a parallel one, so it is verified here rather
than assumed. \code{\_target\_center(det)} returns Carolus's own per-frame
centroid whenever present, and only consults the AprilTag long-range
fallback (\code{\_on\_tag\_detections}, reprojecting the nearest tag through
the already-Kalibr'd \code{cam0} intrinsics, $p_x=336.372\,(x/z)+310.200$)
on the frames where Carolus's own detection is absent, gated by
$\code{TAG\_FALLBACK\_MAX\_AGE\_S}=0.7$\,s. Strict priority, not arbitration
by comparison, is what prevents the two sources from ever competing: no code
path evaluates both and picks one, so no comparison logic exists to
oscillate. What is confirmed \emph{not} to exist is any hysteresis on the
handoff itself: near the boundary of Carolus's own reliable range
($\approx$0.5--4\,m), a frame-to-frame flicker in Carolus's own detection
would switch sources every such frame, and the two sources do not report
pixel-identical positions for the same physical beacon (Carolus: direct
detection; AprilTag: a computed reprojection). This is recorded as an
identified, currently unguarded risk, not a proven field problem, since no
live AUTO traverse has yet exercised this boundary
(\S\ref{sec:pidvalidation}).

\section{Standard versus mecanum wheels: two firmware kinematics, one of
them switched off}
\label{sec:wheelcompare}
\S\ref{sec:wheels} established, in July, that the wheels on this rover are
mecanum and not the rigid skid-steer set the configuration assumed, and
drew the estimator-side consequences (wheel yaw discounted,
$r_{\mathrm{eff}}=62.2$\,mm calibrated). What follows does not repeat that
discovery. It documents four things learned since, three of which come from
reading the running parameter server on 2026-08-12 rather than any
configuration file: that the firmware carries \emph{two} complete kinematic
profiles and runs the wrong one; what the wheel choice costs in metres,
written out; the full three-degree-of-freedom kinematics the hardware
supports and this configuration cannot express; and a cockpit control added
the same day so that recorded data finally states which wheels produced it.

\subsection{Two profiles, read live}
\label{sec:wheeltwoprofiles}
\code{rosparam} on the running robot returns two disjoint parameter trees
under \code{/firmware/}, plus a boolean selecting between them:
\begin{center}\small
\begin{tabular}{@{}lcc@{}}
\toprule
& \code{diff\_drive} \textbf{(active)} & \code{mecanum\_drive} (unused) \\
\midrule
\code{wheel\_radius}                & $0.0625$\,m & $0.0635$\,m \\
\code{wheel\_separation}            & $0.358$\,m  & $0.370$\,m \\
\code{wheel\_base}                  & ---         & $0.3052$\,m \\
\code{angular\_velocity\_multiplier}& $\mathbf{1.76}$ & $\mathbf{1.0}$ \\
\bottomrule
\end{tabular}
\end{center}
\noindent with \code{/firmware/mecanum\_wheels} $=$ \code{false} and, on the
robot's own \code{/etc/ros/setup.bash}, \code{export MECANUM\_WHEELS=false}
--- the single switch FictionLab's own integration guide names. The rover
therefore drives mecanum hardware through the differential model, and the
mecanum profile, geometry and all, sits unused beside it. Three separate
mismatches follow, and they are independent of one another:
\begin{enumerate}[leftmargin=1.4em]
\item the \textbf{radius} in use ($0.0625$) is the stock rubber-tyre figure,
  not this rover's measured $r_{\mathrm{eff}} = 0.0622$ (\S\ref{sec:wheelradius});
\item the \textbf{yaw multiplier} in use ($1.76$) is a scrub compensation
  derived for rubber tyres, applied to wheels whose rollers do not scrub
  (\S\ref{sec:wheelyaw});
\item the \textbf{model} in use has two degrees of freedom where the
  hardware has three (\S\ref{sec:wheelmecanum}).
\end{enumerate}

\subsection{Effective radius, and what a wrong one costs}
\label{sec:wheelradius}
A rubber tyre rolls on its outer surface, so its rolling radius is
essentially geometric. A mecanum wheel rolls on polyurethane rollers that
compress under load, so the distance covered per revolution corresponds to a
radius smaller than a calliper reading on the unloaded wheel:
\begin{equation}
\delta = r_{\mathrm{geo}} - r_{\mathrm{eff}} = 64.4 - 62.2 = 2.2\;\text{mm},
\qquad
\frac{\delta}{r_{\mathrm{geo}}} = \frac{2.2}{64.4} = 3.4\,\%.
\label{eq:rollercomp}
\end{equation}
...
\begin{tikzpicture}[font=\small, >=Latex, scale=0.9]
  % sol
  \draw[thick, black!55] (-2.6,0) -- (2.6,0);
  \foreach \x in {-2.4,-1.9,...,2.5} \draw[black!45] (\x,0) -- ++(-0.18,-0.2);
  % roue (rayon dessine 2.2 cm  <->  62.2 mm effectif)
  \draw[dashed, black!55] (0,2.2) circle (2.28);
  \fill[alertred!6] (0,2.2) circle (2.28);
  \fill (0,2.2) circle (1.4pt);
  % rayon geometrique, oblique
  \draw[black!60, thick] (0,2.2) -- ++(45:2.28);
  \node[font=\scriptsize, anchor=west, align=left] at (1.72,4.0)
       {$r_{\mathrm{geo}} = 64.4$\,mm\\[-1pt]\scriptsize($\varnothing128.8$, unloaded)};
  % rayon effectif, vertical
  \draw[alertred, very thick] (0,2.2) -- (0,0);
  \node[alertred, font=\scriptsize, anchor=west] at (0.16,1.5) {$r_{\mathrm{eff}} = 62.2$\,mm};
  % rouleaux stylises
  \foreach \a in {20,75,130,200,255,310}
    \draw[alertred!75, thick] ([shift=(\a:1.95)]0,2.2) -- ([shift=(\a:2.28)]0,2.2);
  % zoom
  \draw[accentblue] (0,0.12) circle (0.55);
  \draw[accentblue, dashed] (0.5,0.5) -- (4.3,1.75);
  \draw[accentblue, fill=accentblue!5] (4.3,0.75) rectangle (7.5,2.75);
  \node[accentblue, font=\scriptsize] at (5.9,2.5) {contact, $\times 10$};
  \draw[thick, black!55] (4.6,1.15) -- (7.2,1.15);
  \draw[dashed, black!55] (4.7,1.72) .. controls (5.9,2.12) .. (7.1,1.72);
  \draw[alertred, thick]  (4.7,1.15) .. controls (5.9,1.55) .. (7.1,1.15);
  \draw[alertred, thick, <->] (5.75,1.35) -- (5.75,1.92);
  \node[alertred, font=\scriptsize, anchor=west] at (5.95,1.63) {$\delta=2.2$\,mm};
  \node[black!55, font=\scriptsize] at (5.9,0.95) {roller compression};
\end{tikzpicture}
\caption[Geometric versus effective rolling radius]{The $2.2$\,mm of
Eq.~\ref{eq:rollercomp} drawn to scale on the wheel, where it is invisible,
and again at $\times 10$, where it is not. That ratio is the whole reason
this quantity is easy to leave wrong in a configuration file for months.}
\label{fig:rollercomp}
\end{figure}

Wheel odometry turns encoder angle into distance through a single
multiplication, so the radius is a \emph{pure scale factor}: an error in it
does not average out over a long run, it grows with the run.
\begin{align}
\hat d &= r_{\mathrm{used}} \textstyle\sum \Delta\theta_{\mathrm{wheel}},
\qquad
d_{\mathrm{true}} = r_{\mathrm{true}} \textstyle\sum \Delta\theta_{\mathrm{wheel}},
\label{eq:odoscale}\\
\varepsilon &= \frac{r_{\mathrm{used}} - r_{\mathrm{true}}}{r_{\mathrm{true}}}
 = \frac{0.0625 - 0.0622}{0.0622} = +0.48\,\%.
\label{eq:odoscaleval}
\end{align}
On the $23.35$\,m taped-rectangle lap of \S\ref{sec:aug05result} that is
$+0.11$\,m of pure over-report. \textbf{Stated honestly, this is not the
dominant error}: the loop-closure gap actually measured on that lap was
$0.449$\,m, four times larger. It is, however, the only part of that gap
available for free, by editing one number, where the remainder needs
estimator work.

\subsection{Why a pivot costs $1.76\times$ on rubber and $1.0\times$ on rollers}
\label{sec:wheelyaw}
A four-wheel skid-steer platform has no steering linkage. To rotate, its
tyres must slide sideways across the ground: the wheels roll forward while
the chassis turns, so all four contact patches scrub. That scrub absorbs
part of the commanded rotation, and the firmware compensates by
over-driving the wheel differential. Mecanum rollers, free to turn about
their own axes, convert the same lateral demand into rolling instead of
sliding, and need no such compensation --- which is exactly what the two
profiles of \S\ref{sec:wheeltwoprofiles} encode in one number each.

\begin{figure}[htbp]
\centering
\begin{tikzpicture}[font=\small, >=Latex, scale=0.82,
  chas/.style={draw=black!55, rounded corners=2pt, fill=black!3},
  whl/.style={draw=black!60, fill=black!12, rounded corners=1pt}]
  % --- panneau gauche : skid-steer ---
  \begin{scope}
    \node[font=\small\bfseries] at (1.1,4.5) {Standard --- skid-steer};
    \draw[chas] (0,0) rectangle (2.2,3.6);
    \foreach \y in {0.35,2.55} {
      \draw[whl] (-0.42,\y) rectangle (0,\y+0.7);
      \draw[whl] (2.2,\y) rectangle (2.62,\y+0.7);
    }
    % ripage lateral
    \foreach \y in {0.7,2.9} {
      \draw[alertred, very thick, ->] (-0.21,\y) -- ++(0,0.55);
      \draw[alertred, very thick, ->] (2.41,\y) -- ++(0,-0.55);
    }
    \draw[accentblue, very thick, ->] (0.5,1.8) arc (180:0:0.6);
    \node[alertred, font=\scriptsize, align=center] at (1.1,-0.75)
         {lateral scrub at\\all four patches};
    \node[font=\scriptsize\bfseries, black!70] at (1.1,-1.6) {multiplier $1.76$};
  \end{scope}
  % --- panneau droit : mecanum ---
  \begin{scope}[xshift=6.4cm]
    \node[font=\small\bfseries] at (1.1,4.5) {Mecanum --- rolling};
    \draw[chas] (0,0) rectangle (2.2,3.6);
    \foreach \y in {0.35,2.55} {
      \draw[whl] (-0.42,\y) rectangle (0,\y+0.7);
      \draw[whl] (2.2,\y) rectangle (2.62,\y+0.7);
    ...
    % rouleaux a 45 deg
    \foreach \y in {0.35,2.55} {
      \foreach \d in {0.1,0.32,0.54} {
        \draw[alertred!70] (-0.40,\y+\d) -- ++(0.22,0.22);
        \draw[alertred!70] (2.60,\y+\d) -- ++(-0.22,0.22);
      }
    }
    \foreach \y in {0.7,2.9} {
      \draw[green!55!black, very thick, ->] (-0.21,\y) -- ++(0,0.55);
      \draw[green!55!black, very thick, ->] (2.41,\y) -- ++(0,-0.55);
    }
    \draw[accentblue, very thick, ->] (0.5,1.8) arc (180:0:0.6);
    \node[green!55!black, font=\scriptsize, align=center] at (1.1,-0.75)
         {rollers turn freely,\\no sliding required};
    \node[font=\scriptsize\bfseries, black!70] at (1.1,-1.6) {multiplier $1.0$};
  \end{scope}
\end{tikzpicture}
\caption[Pivot: scrub versus rolling]{The same commanded pivot, produced two
different ways. Red arrows are enforced sliding; green arrows are free
rolling. The firmware states the difference as a single scalar per profile,
and both scalars were read from the live parameter server.}
\label{fig:pivotscrub}
\end{figure}

The differential model relates yaw to the wheel-speed difference through
the track $b$; the firmware inserts a gain $k$ on top:
\begin{equation}
\omega_z = \frac{r\,(\omega_R - \omega_L)}{b},
\qquad
\omega_{z,\mathrm{cmd}} = k\,\frac{r\,(\omega_R - \omega_L)}{b}.
\label{eq:diffyaw}
\end{equation}
A gain is indistinguishable from a change of track, so $k=1.76$ is exactly
the statement that the platform rotates as though its wheels were
\begin{equation}
b_{\mathrm{eff}} = k\,b = 1.76 \times 0.358 = 0.630\;\text{m}
\label{eq:beff}
\end{equation}
apart, against a geometric $0.358$\,m. Differentiating
Eq.~\ref{eq:diffyaw} gives the sensitivity that makes both geometric
parameters matter at once, and in opposite directions:
\begin{equation}
\frac{\delta\omega_z}{\omega_z}
  = \frac{\delta r}{r} - \frac{\delta b}{b}.
\label{eq:yawsens}
\end{equation}

\begin{trapbox}{The $1.76$ in use was derived for wheels this rover does not have}
$1.76$ is the stock FictionLab value, shipped with the rubber-tyre
configuration, and it compensates a scrub mechanism that mecanum rollers do
not produce. Running it against the fitted mecanum set is therefore a third
mismatch on top of the radius and the model, and it is consistent with what
\S\ref{sec:wheels} already measured the hard way in July: a sustained
$\Delta\omega \approx 3.8$\,rad/s of wheel differential that produced
$0.001$\,rad/s of real yaw. No scalar gain repairs that, because the
quantity being scaled is not merely mis-scaled but, in that regime,
fabricated --- which is why the estimator-side answer was to discount wheel
yaw (\code{noise\_w}: $0.2 \rightarrow 0.8$) rather than to retune $k$.
\end{trapbox}

\subsection{The third degree of freedom, written out}
\label{sec:wheelmecanum}
Each roller is set at $45^\circ$ to the wheel axis, so a mecanum wheel can
transmit ground force along one diagonal only. Mounted in the standard X
pattern the four diagonals span the plane, and the platform gains a genuine
third controllable degree of freedom. With this firmware's \emph{own}
mecanum numbers,
\begin{equation}
l_x = \tfrac{1}{2}(0.3052) = 0.1526\;\text{m},\quad
l_y = \tfrac{1}{2}(0.370) = 0.185\;\text{m},\quad
L = l_x + l_y = 0.3376\;\text{m},
\label{eq:mecgeom}
\end{equation}
the inverse kinematics mapping a body twist to the four wheel rates is
\begin{equation}
\begin{pmatrix}\omega_{FL}\\ \omega_{FR}\\ \omega_{RL}\\ \omega_{RR}\end{pmatrix}
=\frac{1}{r}
\begin{pmatrix}
 1 & -1 & -L\\
 1 & \phantom{-}1 & \phantom{-}L\\
 1 & \phantom{-}1 & -L\\
 1 & -1 & \phantom{-}L
\end{pmatrix}
\begin{pmatrix}v_x\\ v_y\\ \omega_z\end{pmatrix}.
\label{eq:mecik}
\end{equation}
Three degenerate cases make the structure legible. Pure advance
($v_y=\omega_z=0$) drives all four wheels alike, $\omega_i = v_x/r$. Pure
lateral translation ($v_x=\omega_z=0$) gives
$\omega_{FL}=\omega_{RR}=-v_y/r$ and $\omega_{FR}=\omega_{RL}=+v_y/r$: two
wheels forward, two backward, at equal magnitude, the forward components
cancelling and the lateral ones adding. Pure rotation ($v_x=v_y=0$) gives
$|\omega_i| = L|\omega_z|/r$ on all four, so that
\begin{equation}
\omega_z = \frac{r\,(\omega_R-\omega_L)}{2L},
\qquad 2L = 0.675\;\text{m},
\label{eq:mecyaw}
\end{equation}
i.e.\ mecanum yaw obeys the same shape as Eq.~\ref{eq:diffyaw} but with
$2L$ in place of $b$ --- an effective base $2L/b = 1.89$ times the
differential one. \textbf{The numerical proximity of $1.89$ to the
differential profile's $k=1.76$ is a coincidence and is flagged as such}:
the two arise from unrelated mechanisms (roller geometry against tyre
scrub), no derivation links them, and treating the match as evidence would
be exactly the kind of numerology this report avoids elsewhere.
...
\begin{tikzpicture}[font=\small, >=Latex, scale=0.82,
  ...
  % --- X configuration ---
  \begin{scope}
    \node[font=\small\bfseries] at (1.1,4.6) {X configuration (top view)};
    \draw[chas] (0,0) rectangle (2.2,3.6);
    \node[black!55, font=\scriptsize] at (1.1,1.8) {$x\uparrow$ fwd};
    \foreach \y in {0.35,2.55} {
      \draw[whl] (-0.42,\y) rectangle (0,\y+0.7);
      \draw[whl] (2.2,\y) rectangle (2.62,\y+0.7);
    }
    % FL et RR : "\" ; FR et RL : "/"
    \foreach \d in {0.12,0.34,0.56} {
      \draw[alertred!75] (-0.40,2.55+\d) -- ++(0.22,0.22);   % FL
      \draw[alertred!75] (2.60,2.55+\d) -- ++(-0.22,0.22);   % FR
      \draw[alertred!75] (-0.40,0.35+\d+0.12) -- ++(0.22,-0.22); % RL
      \draw[alertred!75] (2.60,0.35+\d+0.12) -- ++(-0.22,-0.22); % RR
    }
    \node[font=\scriptsize, black!60] at (-0.75,3.45) {FL};
    \node[font=\scriptsize, black!60] at (2.95,3.45) {FR};
    \node[font=\scriptsize, black!60] at (-0.75,0.25) {RL};
    \node[font=\scriptsize, black!60] at (2.95,0.25) {RR};
    \draw[accentblue, <->] (0,-0.55) -- (2.2,-0.55);
    \node[accentblue, font=\scriptsize] at (1.1,-0.95) {$2l_y = 0.370$\,m};
    \draw[accentblue, <->] (3.35,0.7) -- (3.35,2.9);
    \node[accentblue, font=\scriptsize, anchor=west, align=left] at (3.45,1.8)
         {$2l_x =$\\$0.305$\,m};
  \end{scope}
  % --- composition du strafe ---
  \begin{scope}[xshift=7.6cm]
    \node[font=\small\bfseries] at (1.1,4.6) {Pure lateral translation};
    \draw[chas] (0,0) rectangle (2.2,3.6);
    \foreach \y in {0.35,2.55} {
      \draw[whl] (-0.42,\y) rectangle (0,\y+0.7);
      \draw[whl] (2.2,\y) rectangle (2.62,\y+0.7);
    }
    % diagonales de force, toutes vers la gauche
    \draw[alertred, thick, ->] (-0.21,2.9) -- ++(-0.75,0.75);
    \draw[alertred, thick, ->] (2.41,2.9) -- ++(-0.75,0.75);
    \draw[alertred, thick, ->] (-0.21,0.7) -- ++(-0.75,-0.75);
    \draw[alertred, thick, ->] (2.41,0.7) -- ++(-0.75,-0.75);
    \draw[green!55!black, ultra thick, ->] (1.1,1.8) -- (-0.9,1.8);
    \node[green!55!black, font=\scriptsize, anchor=south] at (0.1,1.9) {$v_y$};
    \node[black!55, font=\scriptsize, align=center] at (1.1,-0.9)
         {forward components cancel,\\lateral components add};
  \end{scope}
\end{tikzpicture}
\caption[Mecanum X configuration]{Left: the roller diagonals in the X
pattern, with the two geometric lengths of Eq.~\ref{eq:mecgeom}. Right: the
second degenerate case of Eq.~\ref{eq:mecik} --- four diagonal wheel forces
whose longitudinal parts cancel in pairs, leaving a pure $v_y$.}
\label{fig:mecx}
\end{figure}

\begin{trapbox}{This rover cannot strafe, and the wheels are not the reason}
Every equation above describes hardware that is bolted to this rover today.
The configuration cannot reach it: with \code{MECANUM\_WHEELS=false} the
CORE2 firmware runs the two-degree-of-freedom differential model, and the
command path carries a \code{geometry\_msgs/Twist} whose \code{linear.y} is
discarded downstream --- there is no $v_y$ anywhere in the chain to strafe
with, whatever Eq.~\ref{eq:mecik} permits. Enabling it means the firmware
switch \emph{and} a mecanum-aware odometry model
(\S\ref{sec:wheels}'s July conclusion, unchanged), which is real work rather
than a checkbox. It stays listed as future work, not claimed as a
capability.
\end{trapbox}

\subsection{Four quantities, two sources each}
\label{sec:wheelgaps}
Collecting the disagreements rather than smoothing them over:
\begin{center}\small
\begin{tabular}{@{}L{2.6cm}L{3.2cm}L{3.3cm}L{3.4cm}@{}}
\toprule
Quantity & One source & The other & Gap, and status \\
\midrule
\code{wheel\_radius} & $0.0625$\,m, firmware, active &
  $0.0622$\,m, measured on this rover &
  $0.48\,\%$ --- registry item 1; Eq.~\ref{eq:odoscaleval} \\
track & $0.354$\,m, manufacturer & $0.358$\,m, firmware / as-built &
  $1.1\,\%$; neither tape-measured on this unit \\
mecanum radius & $0.0635$\,m, unused profile &
  $0.0622$\,m, measured effective &
  $2.1\,\%$; the unused profile assumes the geometric radius, ignoring
  Eq.~\ref{eq:rollercomp} \\
kinematic model & 4-wheel skid-steer, physically &
  2-wheel differential, in the estimator &
  deliberate approximation; online extrinsic refinement absorbs most of it \\
\bottomrule
\end{tabular}
\end{center}

\subsection{A cockpit control so the data says which wheels produced it}
\label{sec:wheelselector}
None of the above was recorded anywhere a dataset could carry: a bag
replayed in six months could not state which wheels were fitted when it was
taken, although that single fact conditions how every distance and heading
in it must be read. A two-position selector was therefore added to the
Operations panel, and it is \textbf{declarative on purpose}: it does not
write \code{/firmware/diff\_drive/*}. That tree is read-only in
\code{leo\_backend.py} by an existing, documented decision --- writing it
requires editing \code{/etc/ros/param.yaml} \emph{on the robot} and
restarting \code{leo.service}, which cuts the drive link, so it must stay a
deliberate human act rather than a side effect of a click.

What the selector does instead is publish the declared set into telemetry
(and from there into every export and bag), persist it across backend
restarts because it describes physical hardware, write a logbook entry on
change because a wheel swap invalidates comparability with everything
measured before, and --- the useful half --- continuously compare the
declaration against the radius the firmware is actually applying:
\begin{equation}
\text{mismatch} =
\begin{cases}
\text{undefined} & \text{if } r_{\mathrm{fw}} \text{ unreachable},\\[2pt]
\left| r_{\mathrm{fw}} - r_{\mathrm{decl}} \right| > \epsilon
  & \epsilon = 10^{-4}\,\text{m}.
\end{cases}
\label{eq:wheelmismatch}
\end{equation}
The three-state result matters: with the parameter server unreachable the
cockpit stays silent rather than asserting an agreement it has not checked.
Verified end to end on the running robot on 2026-08-12, the check fires
immediately and correctly, reporting
$r_{\mathrm{decl}} = 0.0622$ against $r_{\mathrm{fw}} = 0.0625$ --- which is
registry item 1, until now dormant in a configuration file and now visible
on every telemetry tick.

\section{What each sensor actually constrains: an observability audit of
this platform}
\label{sec:obsaudit}

The opening chapter recalled the four directions a monocular-inertial
system can never fix: global position, and rotation about gravity. That
result is about the \emph{trajectory}. Two failures in this campaign came
from a different and less-cited place --- the observability of the
\emph{calibration} states, and of the vertical direction on a wheeled
platform --- and both cost days before being named. This section derives
them, because in each case the fix is obvious once the algebra is written
and invisible until it is.

\subsection{Extrinsic calibration is unobservable at rest}
\label{sec:extunobs}

MINS carries the camera--IMU extrinsic $\mathbf{T}_{IC} = (\mathbf{R}_{IC},
\mathbf{p}_{IC})$ in its state vector and refines it online when
\code{do\_calib\_ext} is true. The question is what information a visual
measurement carries \emph{about that pair} when the platform does not move.

A feature $\mathbf{f}$ expressed in the global frame projects into the
camera as
\begin{equation}
\mathbf{z} = \boldsymbol{\pi}\!\left(
  \mathbf{R}_{IC}^{\top}\,\mathbf{R}_{GI}^{\top}
  \bigl(\mathbf{f} - \mathbf{p}_{GI}\bigr) - \mathbf{R}_{IC}^{\top}\,\mathbf{p}_{IC}
\right),
\label{eq:projext}
\end{equation}
with $\boldsymbol\pi$ the pinhole map and $(\mathbf{R}_{GI},
\mathbf{p}_{GI})$ the IMU pose. The filter learns $\mathbf{p}_{IC}$ only
through $\partial \mathbf{z} / \partial \mathbf{p}_{IC}$. Writing
$\mathbf{p}_C = \mathbf{R}_{IC}^{\top}\mathbf{R}_{GI}^{\top}(\mathbf{f} -
\mathbf{p}_{GI}) - \mathbf{R}_{IC}^{\top}\mathbf{p}_{IC}$ for the feature in
camera coordinates,
\begin{equation}
\frac{\partial \mathbf{z}}{\partial \mathbf{p}_{IC}}
  = \underbrace{\frac{\partial \boldsymbol\pi}{\partial \mathbf{p}_C}}_{\text{2}\times\text{3}}
    \bigl(-\mathbf{R}_{IC}^{\top}\bigr).
\label{eq:jacext}
\end{equation}

Equation~(\ref{eq:jacext}) is not zero: a single image does constrain the
lever arm. The difficulty is that it constrains it in exactly the same way
it constrains the position $\mathbf{p}_{GI}$, whose Jacobian is
\begin{equation}
\frac{\partial \mathbf{z}}{\partial \mathbf{p}_{GI}}
  = \frac{\partial \boldsymbol\pi}{\partial \mathbf{p}_C}
    \bigl(-\mathbf{R}_{IC}^{\top}\mathbf{R}_{GI}^{\top}\bigr).
\end{equation}
The two differ by the single rotation $\mathbf{R}_{GI}^{\top}$. If the
platform is stationary, $\mathbf{R}_{GI}$ is constant, and every measurement
in the window sees the \emph{same} pair of Jacobians related by the
\emph{same} constant rotation. Their columns then span a common subspace,
and the information matrix acquires a null direction along
\begin{equation}
\delta\mathbf{p}_{GI} = -\mathbf{R}_{GI}\,\delta\mathbf{p}_{IC},
\label{eq:nulldir}
\end{equation}
which leaves every predicted measurement unchanged. Moving the camera
one centimetre forward and declaring the rover one centimetre backward
produces an identical image. \textbf{Only a change in $\mathbf{R}_{GI}$
breaks the tie}, because it changes the relative rotation between the two
Jacobians; that is, only rotation of the platform makes the lever arm
observable.

\begin{eqblock}
\textbf{Consequence, and it is not a subtlety.} An estimator asked to
refine an unobservable state does not hold it still. Along the null
direction of Eq.~(\ref{eq:nulldir}) the measurements exert no restoring
force, so the estimate executes a random walk driven only by its own
process noise. Measured on a healthy instance, robot stationary, over
75\,s: \code{cam0} $x$ from $0.2113$ to $0.2096$, $y$ from $0.0373$ to
$0.0359$, $z$ from $0.0935$ to $0.0946$\,m --- monotone, ${\approx}1.4$\,mm
per minute, with no sign of convergence. Extrapolated, ${\approx}8$\,cm per
hour of standing still.
\end{eqblock}

\subsection{From a drifting lever arm to a rejected measurement}
\label{sec:levertochi2}

A wandering extrinsic would be harmless if it stayed in the calibration
block. It does not, and the coupling is quantitative. A lateral error
$\Delta$ in $\mathbf{p}_{IC}$ displaces the predicted projection of a
feature at depth $d$ by
\begin{equation}
\delta u \;\approx\; \frac{f\,\Delta}{d},
\label{eq:reproj}
\end{equation}
directly from the pinhole map $u = f\,x/z$. With this rover's calibrated
focal length $f \approx 337$\,px (Kalibr output, installed in \code{config\_camera.yaml}), features at a
typical indoor depth $d = 2$\,m and a decimetre of accumulated extrinsic
error,
\begin{equation}
\delta u \approx \frac{337 \times 0.10}{2.0} \approx 17\ \text{px}.
\end{equation}
The Mahalanobis gate accepts a residual $\mathbf{r}$ when $\mathbf{r}^{\top}
\mathbf{S}^{-1} \mathbf{r} < \gamma$, with $\mathbf{S}$ built from
\code{sigma\_px} $= 1$\,px and $\gamma$ scaled by \code{chi2\_mult} $= 1$.
A 17\,px residual against a 1\,px sigma is a $\chi^2$ contribution of order
$17^2 = 289$ for two degrees of freedom, against a threshold near $6$.
Every visual measurement is rejected, in silence, and the filter is left
with the wheels alone.

That is the entire failure chain, and each arrow in it was measured rather
than assumed:

\begin{center}
\begin{tikzpicture}[
  font=\small, >={Latex[length=2mm]},
  bloc/.style={draw, rounded corners=2pt, align=center, font=\scriptsize,
               minimum height=8mm, inner sep=3pt},
  mesure/.style={bloc, fill=blue!8},
  panne/.style={bloc, fill=alertred!12, draw=alertred!70},
  fl/.style={->, thick, draw=black!60}
]
  \node[mesure]                        (a) at (0,0)     {stationary\\platform};
  \node[mesure, right=7mm of a]        (b)              {$\mathbf{p}_{IC}$ unobservable\\Eq.~(\ref{eq:nulldir})};
  \node[mesure, right=7mm of b]        (c)              {random walk\\1.4\,mm/min};
  \node[panne,  right=7mm of c]        (d)              {17\,px reprojection\\Eq.~(\ref{eq:reproj})};
  \node[panne,  below=6mm of d]        (e)              {$\chi^2 \approx 289$\\vs threshold 6};
  \node[panne,  left=7mm of e]         (f)              {all visual\\updates rejected};
  \node[panne,  left=7mm of f]         (g)              {$X/Y$ anchor lost\\(wheels only)};
  \node[panne,  left=7mm of g]         (h)              {divergence,\\robot immobile};
  \foreach \x/\y in {a/b, b/c, c/d, d/e, e/f, f/g, g/h}
    \draw[fl] (\x) -- (\y);
\end{tikzpicture}
\end{center}

\noindent The fix follows from the first box, not the last: freeze
\code{do\_calib\_ext}. Estimating a quantity the data cannot resolve is not
a refinement, it is an unbounded input to the filter. The cost of freezing
is that the tape-measure seed becomes final --- which is precisely why the
outstanding Kalibr IMU--camera calibration was promoted from a precision
improvement to the highest-value remaining task (\S\ref{sec:openitems},
item~2).

\subsection{Why the wheels cannot save the vertical}
\label{sec:zunobs}

The platform runs \code{type: "Wheel2DAng"}, a differential-drive model
built from two wheel angular rates. Its measurement, expressed in the body
frame, is
\begin{equation}
\begin{bmatrix} v_x \\ \omega_z \end{bmatrix}
  = \begin{bmatrix} \dfrac{r}{2} & \dfrac{r}{2} \\[6pt]
                    \dfrac{r}{b} & -\dfrac{r}{b} \end{bmatrix}
    \begin{bmatrix} \omega_L \\ \omega_R \end{bmatrix},
\label{eq:wheel2d}
\end{equation}
with $r$ the effective radius and $b$ the track width. The model has two
outputs. It says nothing whatever about $v_y$, $v_z$, $\omega_x$ or
$\omega_y$ --- not that they are zero, but that this sensor does not
measure them. The measurement Jacobian with respect to the vertical
position is identically null:
\begin{equation}
\frac{\partial}{\partial p_z}
  \begin{bmatrix} v_x \\ \omega_z \end{bmatrix} = \mathbf{0}.
\label{eq:wheelnoz}
\end{equation}

Gravity does not rescue it either. The accelerometer observes the
\emph{direction} of $\mathbf{g}$, which fixes roll and pitch, but it enters
the propagation as
\begin{equation}
\dot{\mathbf{v}} = \mathbf{R}_{GI}\,(\mathbf{a}_m - \mathbf{b}_a) + \mathbf{g},
\end{equation}
where any residual bias $\mathbf{b}_a$ along the vertical is integrated
twice into $p_z$ with nothing to contradict it. On this rover, therefore,
\textbf{the camera is the sole sensor constraining $z$}, and the moment its
contribution weakens --- feature starvation, a $\chi^2$ gate closing on a
bad extrinsic, an initialisation taken during a disturbance --- the
vertical is left to integrate freely.

\begin{center}
\begin{tikzpicture}[font=\small,
  capteur/.style={draw, rounded corners=2pt, minimum width=24mm,
                  minimum height=7mm, font=\scriptsize, align=center},
  tete/.style={font=\scriptsize\bfseries},
  oui/.style={draw=green!45!black, fill=green!25, circle, inner sep=2pt},
  non/.style={draw=alertred!70, fill=alertred!15, circle, inner sep=2pt}
]
  % colonnes = degres de liberte
  \node[tete] at (3.2,1.15) {$x,\,y$};
  \node[tete] at (5.0,1.15) {yaw};
  \node[tete] at (6.8,1.15) {roll, pitch};
  \node[tete] at (8.6,1.15) {$z$};

  % lignes = capteurs
  \node[capteur] (w) at (0.8,0.6)  {wheels};
  \node[capteur] (i) at (0.8,0.0)  {IMU};
  \node[capteur] (c) at (0.8,-0.6) {camera};

  % ce que chaque capteur contraint reellement
  \node[oui] at (3.2, 0.6) {}; \node[oui] at (5.0, 0.6) {};
  \node[non] at (6.8, 0.6) {}; \node[non] at (8.6, 0.6) {};
  ...
  \draw[decorate, decoration={brace, amplitude=4pt}, draw=alertred!80, thick]
    (9.15,-0.85) -- (9.15,0.85);
  \node[font=\scriptsize, align=left, text=alertred!80, anchor=west]
    at (9.4,0.0) {only one\\sensor:\\no redundancy};
\end{tikzpicture}
\end{center}

\noindent Read the last column: three sensors, and only one of them says
anything about height. This is not a redundancy gap that can be closed by
tuning --- it is structural, and it is why the vertical is the first axis
to leave on this platform. Section~\ref{sec:zclamp} states what was and was
not done about it.

\subsection{Clamping the output is not constraining the filter}
\label{sec:zclamp}

Because $z$ is physically constant on a flat floor, it is tempting to force
it to zero and be done. That was implemented, and the distinction it forces
is worth stating precisely, because it is easy to get backwards.

\begin{center}
\begin{tikzpicture}[font=\small, >={Latex[length=2mm]},
  boite/.style={draw, rounded corners=2pt, align=center, font=\scriptsize,
                minimum height=9mm, inner sep=3pt},
  filtre/.style={boite, fill=blue!8},
  sortie/.style={boite, fill=green!12},
  diagn/.style={boite, fill=orange!14},
  fl/.style={->, thick, draw=black!60}
]
  \node[filtre] (imu)  at (0,0)               {\code{/imu/data\_clean}};
  \node[filtre, right=7mm of imu]  (mins)     {MINS\\\scriptsize $z$ free};
  \node[filtre, right=7mm of mins] (raw)      {\code{/mins/imu/odom}\\\scriptsize $z$ estimated};
  \node[sortie, right=7mm of raw]  (sel)      {\code{pose\_selector}\\\scriptsize $z \leftarrow 0$};
  \node[sortie, right=7mm of sel]  (fused)    {\code{/robot\_pose\_fused}\\\scriptsize $z \equiv 0$};
  \node[diagn, below=8mm of sel]   (diag)     {\code{pose\_z\_brut}\\\scriptsize $z$ preserved};
  ...
  \draw[fl] (sel)  -- (fused);
  \draw[fl] (sel)  -- (diag);
  \draw[->, thick, dashed, draw=alertred!70]
    (fused.north) to[bend right=25] (mins.north);
  \node[font=\scriptsize, text=alertred!80] at (7.2,1.35) {no such path};
\end{tikzpicture}
\end{center}

\noindent The dashed arrow is the point. \code{pose\_selector} is a pure
consumer: MINS subscribes to \code{/imu/data\_clean},
\code{/joint\_states\_mins} and the two image topics, and to nothing else
--- verified against the running node, not assumed from the launch file.
No feedback path exists, so clamping the output changes what is
\emph{displayed} and leaves the estimate untouched. In particular:

\begin{itemize}[leftmargin=1.6em]
  \item the filter's covariance, its bias states and its $X/Y$ accuracy are
        bit-for-bit what they were;
  \item the vertical error is still there, still coupled through the
        covariance to velocity and bias, so a clamped output can look
        healthier than the filter genuinely is;
  \item the symptom that has twice betrayed a locked initialisation --- a
        visibly wrong $z$ beside a healthy $X/Y$ --- is removed from the
        published pose.
\end{itemize}

The third point is why the raw value is republished on a separate topic and
why a warning fires past 25\,cm. A cosmetic improvement that deletes a
diagnostic is a net loss, and this one came within one design decision of
being exactly that.

\begin{trapbox}{Clamping the input of a transform is not clamping its output}
The first implementation forced $z=0$ on the pose handed to the TF
broadcaster. That pose is expressed in the \emph{IMU} frame; the broadcast
composes it with the static $\mathbf{T}_{B\!I}$ before publishing
$\text{odom} \rightarrow \text{base\_footprint}$, and that composition
re-introduces the sensor's mounting height:
\[
  \mathbf{T}_{OB} = \mathbf{T}_{OI}\,\mathbf{T}_{B\!I}^{-1}
  \quad\Longrightarrow\quad
  \bigl[\mathbf{T}_{OB}\bigr]_{z} \neq 0
  \ \text{ even when }\ \bigl[\mathbf{T}_{OI}\bigr]_{z} = 0 .
\]
Measured on the running system: the TF read $-0.159$\,m while
\code{/robot\_pose\_fused} announced $0$ for the \emph{same} frame. The map
and the pose would have disagreed with neither of them signalling it. The
clamp belongs on the transform that is actually broadcast.
\end{trapbox}

\subsection{A sampling trap on the replacement IMU}
\label{sec:dupsample}

The MinIMU-9 v5 driver polls an LSM6DS33 configured at $104$\,Hz. A
free-running loop at a nearby rate does not merely jitter; it aliases, and
the two failure modes are not symmetric.

Let the true angular rate be $\omega(t)$ and the propagation integrate
samples $\omega_k$ over intervals $\Delta t$. A \emph{missed} sample widens
one interval: the trapezoidal estimate over $2\Delta t$ carries the usual
$O(\Delta t^2)$ truncation error, and the integral remains unbiased. A
\emph{duplicated} sample is different in kind --- the same $\omega_k$ is
integrated twice, so
\begin{equation}
\widehat{\theta} - \theta
  \;=\; \omega_k\,\Delta t \;+\; O(\Delta t^2),
\label{eq:dupbias}
\end{equation}
a first-order term that does not vanish with the step and does not average
out, because duplicates occur preferentially while the beat between poll
rate and ODR is slow. Integrated once more into position, a systematic
first-order velocity error becomes a quadratic position error --- the same
mechanism, at smaller amplitude, as the accelerometer scale residual of
\S\ref{sec:july28accel}.

The remedy is to publish only on a fresh sample. \code{STATUS\_REG}
(\code{0x1E}) carries the layout
\[
  \bigl[\,-\ \ -\ \ -\ \ -\ \ \text{EV\_BOOT}\ \ \text{TDA}\ \ \text{GDA}\ \ \text{XLDA}\,\bigr],
\]
so gating on $\text{XLDA} \wedge \text{GDA}$ removes duplicates outright and
leaves only the benign failure mode. The poll rate is then chosen
\emph{above} the ODR --- $250$\,Hz against $104$\,Hz, roughly $2.4$ polls
per sample --- so that each sample is collected within a few milliseconds
of becoming available. Polling below the ODR, at the $100$\,Hz the driver
first used, would have fallen behind by about four samples per second.

\section{The two calibration inputs the filter cannot check}
\label{sec:calibinputs}

An estimator validates its \emph{measurements} continuously --- that is what
the $\chi^2$ gate is for. It never validates its \emph{calibration}. The
noise densities and the camera--IMU transform are read once at startup and
trusted for the rest of the run, so an error in either propagates into every
subsequent update with nothing to signal it. Both are quoted throughout this
report; neither has so far been derived. This section does that, because in
both cases the failure mode is a silent sign or unit error rather than
anything the software would notice.

\subsection{Inverting Kalibr's extrinsic, and why transposition is not enough}
\label{sec:seinv}

Kalibr reports \code{T\_cam\_imu}: the transform that takes a point
\emph{expressed in the IMU frame} and re-expresses it in the camera frame.
MINS's \code{config\_camera.yaml} wants the opposite convention,
\code{T\_imu\_cam}. The two are related by an SE(3) inverse, and the
temptation --- transposing the rotation and negating the translation
independently --- is wrong.

Write the Kalibr output in homogeneous form:
\begin{equation}
\mathbf{T}_{CI} =
\begin{bmatrix} \mathbf{R}_{CI} & \mathbf{p}_{CI} \\[2pt]
                \mathbf{0}^{\top} & 1 \end{bmatrix},
\qquad
\mathbf{p}_C = \mathbf{R}_{CI}\,\mathbf{p}_I + \mathbf{p}_{CI}.
\label{eq:tci}
\end{equation}
Solving Eq.~(\ref{eq:tci}) for $\mathbf{p}_I$ gives
$\mathbf{p}_I = \mathbf{R}_{CI}^{\top}\mathbf{p}_C -
\mathbf{R}_{CI}^{\top}\mathbf{p}_{CI}$, so
\begin{equation}
\boxed{\;
\mathbf{T}_{IC} = \mathbf{T}_{CI}^{-1} =
\begin{bmatrix} \mathbf{R}_{CI}^{\top} & -\,\mathbf{R}_{CI}^{\top}\mathbf{p}_{CI} \\[2pt]
                \mathbf{0}^{\top} & 1 \end{bmatrix}. \;}
\label{eq:seinv}
\end{equation}

The translation block is $-\mathbf{R}^{\top}\mathbf{p}$, \textbf{not}
$-\mathbf{p}$. The rotation must act on the translation before the sign is
applied, and skipping that step leaves an error whose magnitude is the full
lever arm whenever the two frames are not aligned. On this platform the
camera--IMU rotation is close to a $90^\circ$ composition
(\S\ref{sec:extrinsic}), so the naive form would misplace the camera by
essentially the whole $\lVert\mathbf{p}\rVert \approx 0.18$\,m --- an error
Eq.~(\ref{eq:reproj}) converts into ${\approx}30$\,px of reprojection at
2\,m, well past the gate.

\begin{center}
\begin{tikzpicture}[font=\small, >={Latex[length=2mm]},
  repere/.style={draw, rounded corners=2pt, minimum width=17mm,
                 minimum height=8mm, font=\scriptsize, fill=blue!8},
  fleche/.style={->, thick, draw=black!65}
]
  \node[repere] (i) at (0,0)   {IMU $\{I\}$};
  \node[repere] (c) at (4.2,0) {camera $\{C\}$};
  \draw[fleche] (i) to[bend left=32]
    node[midway, above, font=\scriptsize] {$\mathbf{T}_{CI}$ \;(Kalibr)} (c);
  \draw[fleche, draw=green!45!black] (c) to[bend left=32]
    node[midway, below, font=\scriptsize, text=green!35!black]
    {$\mathbf{T}_{IC} = \mathbf{T}_{CI}^{-1}$ \;(MINS)} (i);
  \node[font=\scriptsize, align=left, anchor=west] at (5.6,0)
    {$\mathbf{R} \mapsto \mathbf{R}^{\top}$\\
     $\mathbf{p} \mapsto -\mathbf{R}^{\top}\mathbf{p}$\\[2pt]
     \textcolor{alertred!80}{not $-\mathbf{p}$}};
\end{tikzpicture}
\end{center}

\noindent \code{fill\_mins\_camchain.py} applies Eq.~(\ref{eq:seinv}) and
also copies the estimated time offset, which Kalibr reports in the same
file and which is easy to leave behind. The rotation is the part that
cannot be recovered later: Eq.~(\ref{eq:nulldir}) showed the translation is
observable under rotation, but a $90^\circ$ seeding error in
$\mathbf{R}_{IC}$ sits outside any region the filter can walk back from ---
which is exactly the vertical divergence recorded on 2026-07-04.

\subsection{Where the noise densities come from, and the unit trap in them}
\label{sec:allan}

The four numbers in \code{config\_imu.yaml} are not datasheet values. They
were measured on this unit by Allan-variance analysis of a long stationary
recording, and their derivation matters because two of the four are
routinely quoted in the wrong units.

For a signal sampled at $f_s$, form averages over bins of duration $\tau$
and compute the Allan variance
\begin{equation}
\sigma^2(\tau) = \frac{1}{2\,(K-1)}\sum_{k=1}^{K-1}
  \bigl(\bar{y}_{k+1} - \bar{y}_k\bigr)^2 ,
\label{eq:allan}
\end{equation}
where $\bar{y}_k$ is the mean of the $k$-th bin. Plotted against $\tau$ on
log--log axes, an inertial sensor shows two asymptotes that correspond to
the two noise processes the filter models:
\begin{equation}
\sigma(\tau) \;\simeq\; \underbrace{\frac{N}{\sqrt{\tau}}}_{\text{white noise}}
  \;\oplus\; \underbrace{B\sqrt{\frac{\tau}{3}}}_{\text{bias random walk}} ,
\label{eq:allanslopes}
\end{equation}
a $-1/2$ slope at short averaging times and a $+1/2$ slope at long ones,
with a minimum between them. The white-noise density $N$ is read at
$\tau = 1$\,s on the descending branch; the random-walk coefficient $B$ is
read from the ascending branch at $\tau = 3$\,s, where the $\sqrt{\tau/3}$
form makes the value the ordinate directly.

\begin{center}
\begin{tikzpicture}[font=\small, scale=1.0]
  \draw[->, thick] (0,0) -- (7.4,0) node[right, font=\scriptsize] {$\log \tau$};
  \draw[->, thick] (0,0) -- (0,3.5) node[above, font=\scriptsize] {$\log \sigma(\tau)$};

  % white-noise branch, slope -1/2
  \draw[thick, blue!65, dashed] (0.5,3.1) -- (3.6,1.05)
    node[pos=0.25, above right, font=\scriptsize, text=blue!65]
    {slope $-1/2$: white noise $N$};
  % random-walk branch, slope +1/2
  \draw[thick, orange!80!black, dashed] (3.6,1.05) -- (6.9,3.0)
    node[pos=0.72, below right, font=\scriptsize, text=orange!70!black]
    {slope $+1/2$: bias random walk $B$};
  % measured curve
  \draw[very thick, black!75]
    (0.5,3.15) .. controls (2.0,1.9) and (2.9,1.15) .. (3.6,1.12)
               .. controls (4.4,1.12) and (5.6,2.2) .. (6.9,3.05);

  % read-off points
  \draw[dotted, gray] (1.6,0) -- (1.6,2.28);
  \node[font=\scriptsize, below] at (1.6,0) {$\tau=1$\,s};
  \fill[blue!65] (1.6,2.28) circle (1.7pt);
  \draw[dotted, gray] (5.3,0) -- (5.3,1.98);
  \node[font=\scriptsize, below] at (5.3,0) {$\tau=3$\,s};
  \fill[orange!80!black] (5.3,1.98) circle (1.7pt);
  \node[font=\scriptsize, below] at (3.6,0) {minimum};
\end{tikzpicture}
\end{center}

\begin{trapbox}{Continuous density against per-sample deviation}
The recurring error is not in Eq.~(\ref{eq:allan}) but in the units that
come out of it. A gyroscope white-noise density $N$ carries
$\text{rad}/\text{s}/\sqrt{\text{Hz}}$, i.e.\ $\text{rad}/\sqrt{\text{s}}$;
the per-sample standard deviation an implementer might measure directly is
$\sigma_{\text{sample}} = N\sqrt{f_s}$. At this platform's ${\approx}86$\,Hz
the two differ by $\sqrt{86} \approx 9.3$, close to an order of magnitude.
Substituting one for the other does not make the filter fail: it makes it
quietly over- or under-confident, which is far harder to notice than a
crash. The values below are continuous densities, as MINS expects.
\end{trapbox}

\noindent Measured on this unit and installed in \code{config\_imu.yaml}:
...
\midrule
$N_g$ & \code{gyro\_noise}  & $1.2637\times10^{-3}$ & rad/s/$\sqrt{\text{Hz}}$ \\
$B_g$ & \code{gyro\_bias}   & $3.3010\times10^{-5}$ & rad/s$^2$/$\sqrt{\text{Hz}}$ \\
$N_a$ & \code{accel\_noise} & $2.3315\times10^{-2}$ & m/s$^2$/$\sqrt{\text{Hz}}$ \\
$B_a$ & \code{accel\_bias}  & $8.9292\times10^{-4}$ & m/s$^3$/$\sqrt{\text{Hz}}$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Two remarks close the loop with the rest of this chapter. First,
these densities describe the \emph{CORE2} unit; the MinIMU-9 v5 that will
replace it (\S\ref{sec:openitems}) must be re-characterized before its
numbers are trusted, and inheriting these would be exactly the datasheet
fallacy this project has avoided elsewhere. Second, no Allan-variance figure
protects against a \emph{scale} error: the campaign's accelerometer read
$8.67$\,m/s$^2$ against a local gravity of $9.790$
(\S\ref{sec:july28accel}), an $11.4\,\%$ deficit (equivalently, the $1.129$ scale factor
applied by \code{imu\_sanitizer}) that is perfectly consistent
sample to sample and therefore invisible to Eq.~(\ref{eq:allan}). Noise
characterization and scale calibration answer different questions, and
passing one does not excuse the other.

\section{The re-anchoring correction, and the offset it can freeze}
\label{sec:se3corr}

The cockpit exposes a single pose topic and a button that changes which
estimator produces it. The two estimators keep independent origins, so a
raw switch would step the output by whatever separates them --- tens of
metres, in this campaign. \code{pose\_selector} therefore maintains a
rigid-body correction per source and applies it before publishing. The
mechanism is three lines of code and is described qualitatively in
Appendix~D; it is derived here because its one structural weakness produced
a 142\,m error on 2026-09-08 that looked exactly like an estimator failure
and was not one.

\subsection{Deriving the correction}
\label{sec:corrderiv}

Let $\mathbf{T}^{s}_{\text{raw}}(t) \in SE(3)$ be the pose published by
source $s$, and $\mathbf{T}_{\text{fused}}(t)$ the pose served downstream.
The selector stores one correction $\mathbf{C}_s$ per source and publishes
\begin{equation}
\mathbf{T}_{\text{fused}}(t) = \mathbf{C}_s\,\mathbf{T}^{s}_{\text{raw}}(t).
\label{eq:corrapply}
\end{equation}
The requirement at a switch from $a$ to $b$ at time $t^{\ast}$ is that the
served pose not jump:
\begin{equation}
\mathbf{C}_b\,\mathbf{T}^{b}_{\text{raw}}(t^{\ast})
  \;=\;
  \mathbf{C}_a\,\mathbf{T}^{a}_{\text{raw}}(t^{\ast})
  \;=\;
  \mathbf{T}_{\text{fused}}(t^{\ast\,-}),
\end{equation}
which determines $\mathbf{C}_b$ uniquely:
\begin{equation}
\boxed{\;
\mathbf{C}_b \;=\; \mathbf{T}_{\text{fused}}(t^{\ast\,-})\,
                   \bigl[\mathbf{T}^{b}_{\text{raw}}(t^{\ast})\bigr]^{-1}. \;}
\label{eq:corrdef}
\end{equation}
The inverse is the SE(3) inverse of Eq.~(\ref{eq:seinv}), with the same
$-\mathbf{R}^{\top}\mathbf{p}$ translation block. Each source is initialised
with $\mathbf{C}_s = \mathbf{I}$, so before any switch the selector is a
pass-through.

Two properties follow immediately, and the second is the whole problem.

\paragraph{The correction is constant between switches.}
Equation~(\ref{eq:corrdef}) is evaluated once, at $t^{\ast}$. For all
$t > t^{\ast}$ the same $\mathbf{C}_b$ multiplies every incoming pose.
Differentiating Eq.~(\ref{eq:corrapply}) with $\mathbf{C}_b$ constant, the
served motion is the raw motion rotated into the anchored frame:
\begin{equation}
\delta\mathbf{T}_{\text{fused}}
  = \mathbf{C}_b\,\delta\mathbf{T}^{b}_{\text{raw}},
\end{equation}
so \emph{relative} motion is preserved exactly. This is the desired
behaviour: re-anchoring must not distort the trajectory, only re-place its
origin.

\paragraph{The correction anchors to a pose, not to the world.}
$\mathbf{C}_b$ encodes the offset between the source's origin and the frame
the output happened to be in at $t^{\ast}$. It carries no information about
whether that frame was \emph{correct}. If $\mathbf{T}_{\text{fused}}
(t^{\ast\,-})$ was wrong --- because the previously active source had
diverged --- then $\mathbf{C}_b$ faithfully preserves that error, and keeps
preserving it indefinitely.

\subsection{The failure mode: a correct estimator behind a frozen offset}
\label{sec:frozenoffset}

Now let the upstream source reset. A respawned MINS re-initialises at a new
origin, so $\mathbf{T}^{b}_{\text{raw}}$ changes discontinuously at some
$t_r > t^{\ast}$:
\begin{equation}
\mathbf{T}^{b}_{\text{raw}}(t_r^{+}) \;\neq\; \mathbf{T}^{b}_{\text{raw}}(t_r^{-}),
  \qquad\text{while}\qquad
  \mathbf{C}_b(t_r^{+}) \;=\; \mathbf{C}_b(t_r^{-}).
\end{equation}
Nothing in Eq.~(\ref{eq:corrdef}) is re-evaluated, because no \emph{switch}
occurred. The served pose therefore becomes
\begin{equation}
\mathbf{T}_{\text{fused}} = \underbrace{\mathbf{C}_b}_{\text{stale}}
  \cdot \underbrace{\mathbf{T}^{b}_{\text{raw}}}_{\text{healthy}} ,
\label{eq:stalecorr}
\end{equation}
a constant rigid offset applied to a perfectly good estimate. The signature
is distinctive once known, and was measured directly on 2026-09-08:
...
\toprule
 & $x$ (m) & $y$ (m) & $z$ (m) \\
\midrule
\multicolumn{4}{@{}l}{\emph{absolute position, end of window}}\\
raw \code{/mins/imu/odom}         & $-0.0012$ & $+0.0016$ & $+0.0170$ \\
served \code{/robot\_pose\_fused} & $-0.4236$ & $-0.0846$ & $-142.0488$ \\
\midrule
\multicolumn{4}{@{}l}{\emph{displacement over the same 30\,s window}}\\
raw \code{/mins/imu/odom}         & $-0.0039$ & $+0.0012$ & $+0.0034$ \\
served \code{/robot\_pose\_fused} & $-0.0037$ & $+0.0018$ & $+0.0034$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent The \emph{deltas} agree to a tenth of a millimetre while the
\emph{absolute} values differ by 142\,m. That is Eq.~(\ref{eq:stalecorr})
written in measurements: relative motion tracked faithfully, absolute
position offset by a constant. An operator watching the map sees a diverged
robot; the estimator underneath is accurate to millimetres.

\begin{center}
\begin{tikzpicture}[font=\small, >={Latex[length=2mm]},
  etape/.style={draw, rounded corners=2pt, align=center, font=\scriptsize,
                minimum height=9mm, inner sep=3pt},
  sain/.style={etape, fill=green!12},
  malade/.style={etape, fill=alertred!12, draw=alertred!70},
  fig/.style={etape, fill=orange!14},
  fl/.style={->, thick, draw=black!60}
]
  \node[malade] (div) at (0,0)                {MINS diverged\\\scriptsize $z=-327$\,m};
  \node[fig, right=7mm of div]   (calc)       {$\mathbf{C}$ computed\\\scriptsize Eq.~(\ref{eq:corrdef})};
  \node[sain, right=7mm of calc] (resp)       {MINS respawned\\\scriptsize $z\approx 0$, healthy};
  \node[fig, right=7mm of resp]  (stale)      {$\mathbf{C}$ NOT\\re-evaluated};
  \node[malade, below=7mm of stale] (out)     {served pose\\\scriptsize offset $142$\,m};

  \draw[fl] (div) -- (calc);
  \draw[fl] (calc) -- (resp);
  \draw[fl] (resp) -- (stale);
  \draw[fl] (stale) -- (out);
  \draw[fl, draw=green!45!black] (resp.south) to[bend right=20]
    node[midway, below, font=\scriptsize, text=green!35!black]
    {estimate is fine} (out.west);
\end{tikzpicture}
\end{center}

\subsection{Why re-selecting the source does not clear it}

The instinctive remedy --- press the button again, or call
\code{set\_source\_by\_name} --- does not work, and Eq.~(\ref{eq:corrdef})
says why. Re-selecting source $b$ while $b$ is already active evaluates
\begin{equation}
\mathbf{C}_b^{\text{new}}
  = \mathbf{T}_{\text{fused}}(t^{-})\bigl[\mathbf{T}^{b}_{\text{raw}}(t)\bigr]^{-1}
  = \bigl(\mathbf{C}_b^{\text{old}}\mathbf{T}^{b}_{\text{raw}}(t)\bigr)
    \bigl[\mathbf{T}^{b}_{\text{raw}}(t)\bigr]^{-1}
  = \mathbf{C}_b^{\text{old}} ,
\label{eq:idempotent}
\end{equation}
because the served pose it anchors to is itself the product of the stale
correction. The operation is idempotent: it recomputes the same matrix. The
correction is preserved \emph{by construction}, which is precisely what one
asks of it at a genuine switch and precisely what one does not want here.

Restarting the node clears it, since $\mathbf{C}_s$ initialises to
$\mathbf{I}$. That is the procedure used on 2026-09-08 --- and it dictates
the ordering: the upstream estimator first, the selector second. Restarting
the selector before MINS would anchor $\mathbf{I}$ against a still-diverged
raw pose and serve the divergence directly.

\begin{trapbox}{A diagnosis that points at the wrong subsystem}
Equation~(\ref{eq:stalecorr}) is a factorisation, and both factors are
observable separately --- but only if one thinks to look. The served topic
alone cannot distinguish ``the estimator diverged'' from ``the estimator is
fine behind a stale correction'', because the two produce identical output.
The discriminator is the \emph{delta}: a diverging filter has drifting
deltas, a stale correction has deltas that match the raw source exactly.
Reading the absolute value of \code{/robot\_pose\_fused} and concluding that
MINS had failed would have sent the investigation into the filter, where
there was nothing to find.
\end{trapbox}

\subsection{The durable fix, and why it was not applied blind}

Equation~(\ref{eq:corrdef}) is evaluated on a switch. The defect is that a
source reset is also an event that invalidates $\mathbf{C}_s$, and nothing
signals it. A principled repair is to have the selector detect the
discontinuity itself --- a jump in $\mathbf{T}^{s}_{\text{raw}}$ far larger
than $v_{\max}\Delta t$ is not physical motion --- and reset $\mathbf{C}_s$
to $\mathbf{I}$ rather than preserve it.

The plausibility guard already computes exactly this quantity for a
different purpose: it freezes the output when a source jumps implausibly.
The two behaviours must not be conflated, and that is the design question
left open (\S\ref{sec:openitems}, item~48). A transient spike should be
\emph{held}, as it is now; a genuine re-initialisation should
\emph{re-anchor}. Distinguishing them from the pose stream alone is not
obviously possible, which is why the current code holds in both cases and
leaves the operator the respawn --- a documented procedure being preferable
to an automatic behaviour that guesses wrong half the time.

\section{The calibration that was half-harvested}
\label{sec:demicalib}

Appendix~D lists the Kalibr IMU--camera calibration as the highest-value
task still outstanding on this platform, and \S\ref{sec:extunobs} explains
why freezing \code{do\_calib\_ext} made it more urgent rather than less. Both
statements were written believing the calibration had never been run. It
had. The output has been sitting in \code{calib\_data/} since
2026-08-20, it is numerically good, and half of it is already deployed ---
which is a more awkward situation than either having it or not.

\subsection{What exists}

\code{imucam\_20260728\_175145-results-imucam.txt} reports a converged
solution against \code{/imu/data\_clean} --- the CORE2 unit, not the D455's
internal IMU, which the bag's topic list confirms:
...
Residual & value \\
\midrule
Reprojection, cam0 & $0.187$\,px (median $0.170$, $\sigma$ $0.112$) \\
Reprojection, cam1 & $0.201$\,px (median $0.161$, $\sigma$ $0.148$) \\
Gyroscope  & $2.64\times10^{-5}$\,rad/s \\
Accelerometer & $8.26\times10^{-4}$\,m/s$^2$ \\
Time offset, cam0 to imu0 & $-0.862$\,ms \\
\bottomrule
\end{tabular}
\end{center}

\noindent Sub-pixel reprojection on both cameras. The four noise densities
this run produced are, digit for digit, the four values in
\code{config\_imu.yaml} --- so this calibration was not ignored: its IMU
half was adopted and is what MINS has been running on ever since. Its
extrinsic half was not.

\subsection{Where the two disagree}

MINS still carries the tape-measure seed. Against Kalibr's measured
$\mathbf{T}_{ic}$:

\begin{center}
\begin{tabular}{@{}lrrr@{}}
\toprule
 & deployed & Kalibr & difference \\
\midrule
$x$ & $+0.1500$\,m & $+0.00330$\,m & $+0.1467$\,m \\
$y$ & $+0.0000$\,m & $+0.02631$\,m & $-0.0263$\,m \\
$z$ & $+0.1000$\,m & $+0.02051$\,m & $+0.0795$\,m \\
\midrule
\multicolumn{3}{@{}l}{\textbf{total}} & $\mathbf{0.169}$\,\textbf{m} \\
\bottomrule
\end{tabular}
\end{center}

\noindent The \emph{rotations} agree closely: the deployed matrix is the
exact $90^\circ$ idealisation of Kalibr's measured one, row by row
($[0,0,1]$ against $[-0.058,-0.076,0.995]$, and so on). That agreement is
worth noting for its own sake --- it confirms the frame convention used
throughout this report is the right one --- but it also isolates the
disagreement entirely in the translation, where it amounts to $16.9$\,cm.

Through Eq.~(\ref{eq:reproj}) that lever-arm error is $28.5$\,px of
reprojection at a $2$\,m feature depth, a $\chi^2$ contribution of order
$810$ against a gate near $6$. If the deployed value were simply wrong by
that much, no visual measurement would survive the gate at all.

\subsection{Why neither number can be installed as it stands}

The temptation is to run \code{fill\_mins\_camchain.py} and be done. The
calibration bag forbids it, and the reason is the null direction of
Eq.~(\ref{eq:nulldir}): the extrinsic translation is observable only under
rotation, so a calibration recording without vigorous excitation constrains
it weakly no matter how good its reprojection residual looks.

Measured on \code{imucam\_20260728\_175145.bag} itself:
...
\midrule
duration & $90.0$\,s & adequate \\
cumulative rotation & $611^\circ$ & modest \\
peak angular rate & $36.5^\circ$/s & gentle \\
accelerometer $\sigma$, per axis & $0.20/0.10/0.09$\,m/s$^2$ & \textbf{near-stationary} \\
\bottomrule
\end{tabular}
\end{center}

\noindent The last row is the disqualifying one. An accelerometer standard
deviation of a fifth of a metre per second squared is barely above what the
unit reads sitting still; the recording contains almost no translational
excitation. Kalibr's reprojection residual can be excellent under those
conditions and its translation still be poorly determined, because
reprojection is dominated by the rotation --- which \emph{is} well observed
here, and which is precisely the part that agrees with the deployed seed.

So the position is this. The rotation is confirmed twice over and can be
trusted. The translation has two candidate values, a $15$\,cm tape measure
and a $3.4$\,cm Kalibr estimate from a recording that could not observe it,
and \textbf{neither is established}. Installing the Kalibr number would
replace an unmeasured value with a badly measured one.

\begin{trapbox}{The consequence of the 2026-09-07 freeze, restated}
While \code{do\_calib\_ext} was true, this ambiguity was survivable: MINS
refined the translation away from whichever seed it started with, at least
during motion. The freeze of \S\ref{sec:extunobs} removed that path
deliberately, on the measured grounds that the same mechanism random-walks
at rest. That decision stands --- but it means the seed is now permanent,
and this section shows the seed is a tape measure that no calibration on
disk is good enough to replace.

The action is therefore not ``install the existing calibration'' but
``re-record it with proper excitation''. What was previously the
highest-value outstanding task is now also a blocking one, and it is
cheaper than it looks: the bag is ninety seconds of hand-held motion in
front of a target, and the pipeline that consumes it already exists and has
already run once.
\end{trapbox}
...
\label{sec:openitems}
\small
% 2026-09-08 : le \begin{center} qui entourait ce longtable est SUPPRIME.
% Un longtable ne doit pas etre place dans un environnement qui fabrique une
% boite (center, minipage, flottant) : c'est une regle du paquet, et le
% centrage etait de toute facon redondant puisque longtable se centre seul
% via \LTleft/\LTright. Gain mesure : une page.
%
% CE QUE CE RETRAIT NE FAIT PAS, et c'est important pour qui relira ceci :
% il ne supprime PAS les quatre avertissements « Overfull \vbox ... while
% \output is active » (296,7 / 11,1 / 602,7 / 255,9 pt) emis sur les pages
% de continuation de ce tableau. Verifie en le retirant seul et en
% recompilant : les quatre valeurs sont revenues IDENTIQUES au centieme.
% Le center n'en etait donc pas la cause.
%
% CES QUATRE AVERTISSEMENTS NE CORRESPONDENT A AUCUN DEFAUT VISIBLE, mesure
% et non suppose : sur les quatre pages concernees ET sur une page temoin
% sans avertissement, le texte s'arrete exactement a la meme hauteur
% (y = 753,60 pt sur 792). Rien ne mord sur la marge, le pied « continued on
% next page » est a sa place. C'est un artefact de l'interaction entre les
% blocs de longtable et la routine de sortie de TeX.
%
% NE PAS tenter de les faire taire avec \clearpage, \enlargethispage ou une
% coupure forcee : cela modifierait une mise en page correcte pour rendre un
% journal plus propre, au risque de laisser de vraies demi-pages blanches.
% Diagnostic complet, pour ne pas le refaire : \tracingpages montre un
% objectif de 628,59 pt et des accumulations a 938 / 652 / 1244 / 897 pt avec
% b=* (aucun point de coupure legal). \LTchunksize=1 reduit chaque
% debordement de 75,5 pt exactement (le coup de l'en-tete repete) mais
% produit 24 erreurs de compilation : piste fermee. Aucun \\*, \nopagebreak
% ni \kill dans le tableau, et aucune boite rigide dans les cellules.
...
\bottomrule
\endlastfoot
1 & \textbf{Wheel radius/track, and now the whole kinematic profile}
    (config frozen at stock values; 2026-07-21: leftward drift measured
    directly, $\approx 1.65\,\%$ right/left effective-radius bias
    (\S\ref{sec:yawtrim}); masked in software by a bisected yaw trim,
    $K\approx-0.087$\,rad/s per m/s, not a substitute for the measurement
    below). \textbf{Widened 2026-08-12} (\S\ref{sec:wheelcompare}): reading
    the live parameter server showed the firmware carries \emph{two}
    complete profiles and runs the differential one
    (\code{/firmware/mecanum\_wheels}\,$=$\,\code{false}), so three
    independent mismatches stack against the fitted mecanum set --- the
    radius ($0.0625$ stock against $0.0622$ measured, $+0.48\,\%$ on every
    wheel distance, Eq.~\ref{eq:odoscaleval}), the yaw multiplier ($1.76$,
    a rubber-tyre scrub compensation applied to rollers that do not
    scrub), and the model itself (2 DOF against 3). The cockpit now
    surfaces the radius half of this on every telemetry tick
    (\S\ref{sec:wheelselector}) &
    ...
    trim (\S\ref{sec:yawtrim}) once corrected. Separately, decide
    deliberately whether to enable \code{MECANUM\_WHEELS=true} with a
    matching odometry model or to keep the differential approximation ---
    what must not continue is running a rubber-tyre profile against
    mecanum hardware without that being a stated choice \\
...
11 & Unbounded ``still starting'' grace period in the web healer
    ...
    hours because idempotency has no time ceiling. \textbf{Recurred
    August 11} (\S\ref{sec:aug11stale}), same defect, on the sibling plain
    bridge (\code{:9090}) this time: found $\approx3$h$04$ old against a
    $28$-minute-old robot boot, masked for over three hours by the exact
    presence-only \code{pgrep} guard this item already named &
    replace the ``maximum grace duration'' idea (a threshold to tune) with
    the tuning-free test derived from the August 11 recurrence,
    $E_P(t) > U(t)$ (Eq.~\ref{eq:staleness}): any PC-side ROS process whose
    own elapsed time exceeds the robot's current uptime is provably stale
    and should be killed and relaunched by the same watchdog cycle that
    already reads both quantities \\
...
36 & \textbf{Colour-camera geometry: four stacked defects, fixed and
    validated against a tape measure 2026-08-18, CLOSED}
    (\S\ref{sec:july30intrinsics}). The recorded defect stands: intrinsics
    from a $1280\times720$ calibration used on a $640\times480$ stream,
    principal point \emph{outside} the image. The launch audit found three
    more, all silent: \code{fov=true} selected the Astrobee FOV lens model
    (reading only \code{distortion[0]} as $\omega$) although the D455
    declares \code{plumb\_bob}; and \code{distortion} \emph{and}
    \code{known\_points} were both \code{<param>} \emph{strings}, so each
    \code{getParam} into a \code{vector<double>} failed silently and fell
    back to hard-coded defaults --- neither had \emph{ever} been applied.
    Fixed: factory intrinsics for this exact profile, \code{fov=false},
    and real \code{<rosparam>} lists. \textbf{Validated}: beacon at
    $23''$ ($0.5842$\,m) to the tip of the LED protruding $55.5$\,mm ahead
    of the cross plane; expected range to the beacon origin
    $0.6397$\,m, measured $0.6393$\,m over $40$ samples
    ($\sigma = 0.05$\,mm) --- \textbf{$0.4$\,mm, $0.06\,\%$} &
    closed. One caution kept on the record: validated at a single range
    ($\approx0.64$\,m) and a single measurement. A residual scale error
    grows linearly with distance, so a second check near $2$\,m would be far
    more sensitive and is the cheapest way to confirm this holds. Items 37
    and 5 are ungated \\
...
38 & \textbf{Thermal envelope is three degrees wide} --- \textbf{active
    cooling installed August 11, matched-load A/B run August 13, CLOSED}
    (\S\ref{sec:july30thermal}, \S\ref{sec:aug11thermal}, \S\ref{sec:aug13ab}).
    The July 28 scheduling fix holds the serial link at $86\,^\circ$C under
    full colour load and does not hold it at $89.1\,^\circ$C, where the
    chain collapses entirely ($676\,751$ UART overruns, load $8.85$, all
    firmware topics at $0$\,Hz). Two GPIO fans hold the ARM clock at
    $1500$\,MHz under every load tested, $89.1\,^\circ$C never approached.
    The controlled comparison this item specified --- overrun rate,
    July 30's load (colour nodelet, both estimators), cooling installed,
    two independent $180$\,s windows --- gives $2.074\,\mathrm{s}^{-1}$
    pooled against the July 30 reference of $3.07\,\mathrm{s}^{-1}$: a
    $32.4\,\%$ reduction, not elimination &
    closed as specified; the $67.6\,\%$ of the original overrun rate that
    survives zero throttling is not this item's question any more, it is
    item 40's --- see its updated entry below \\
...
40 & \textbf{A live UART checksum storm --- root mechanism caught directly
    by \code{ftrace}} (\S\ref{sec:aug11mins}, \S\ref{sec:aug13ab},
    \S\ref{sec:aug13irqsoff}). Motion excitation and the kernel's RT budget
    both refuted by controlled measurement; temperature ruled insufficient
    (item 38: two thirds of the July 30 rate survives zero throttling).
    \code{irqsoff} tracing on IRQ~$14$ then found the mechanism directly:
    two independent captures, $2829$ and $3064\,\mu$s of interrupts
    disabled, both $>\!2.2\times$ the $1.28$\,ms FIFO deadline
    (Eq.~\ref{eq:fifodeadline}), both a \code{swiotlb}-bound USB (xHCI)
    descriptor drain, one preceded by SD-card interrupt handling. Capture 2
    brackets the UART directly: serviced at $t{=}10\,\mu$s, not again until
    $t{=}2889\,\mu$s (Eq.~\ref{eq:pl011gap}) --- a gap wide enough for
    $\sim\!40$ bytes to overflow the FIFO in one occurrence. Two of three
    mitigation candidates checked the same evening: DMA mask ruled out
    directly ($64$-bit device, $16$\,GiB platform window, both confirmed
    by reading the hardware/device-tree, not inferred); IRQ affinity for
    xHCI refused outright by the kernel (\code{I/O error}, this MSI
    controller does not support runtime re-steering), MMC's move gave an
    inconclusive result against a load that drifted on its own across the
    test window, so re-run with a proper B--A--B--A matched protocol
    ($4\times120$\,s): mean $2.180\,\mathrm{s}^{-1}$ (CPU$1$) against
    $2.504\,\mathrm{s}^{-1}$ (original) --- a $12.9\,\%$ nominal gap
    smaller than the spread within the CPU$1$ samples themselves
    ($0.872\,\mathrm{s}^{-1}$), not a confirmed effect either way. Third
    candidate closed: \code{bcm2835\_mmc} exposes only two module
    parameters, both debug-logging flags, no timing control &
    a kernel patch to \code{bcm2835\_mmc} is the only route left to the
    \code{udelay} busy-waits, not attempted (this Pi's SD card is also
    its boot device); the MMC-affinity question needs more statistical
    power than four $120$\,s windows gave it --- longer or more numerous
    runs, not a different protocol \\
41 & \textbf{$\sqrt{\text{VINS}}$ on the Pi: root cause of the ZUPT
    $\chi^2$ defect found and patched, the patch verified, the platform
    still diverges} (\S\ref{sec:sqrtvins}, \S\ref{sec:sqrtvins-patch}).
    Builds and runs at $30$--$70\,\mathrm{Hz}$ on live sensor data; on
    one recorded lab lap, MINS $1.02$\,m loop closure, PC openVINS
    $541.7$\,m, $\sqrt{\text{VINS}}$ unbounded. Reading
    \code{UpdaterZeroVelocity.cpp} found the actual bug behind both
    ZUPT sub-mode failures: an unchecked \code{LLT().solve()} plus a
    reject condition the disparity-only path bypasses unconditionally
    when stationary. A minimal additive patch (numerical-validity guard,
    64-line diff, \code{docs/sqrtvins\_zupt\_numerical\_guard.patch})
    was built and run live for $60$\,s: $588$ correct rejections, $241$
    correct acceptances, zero \code{inf}/garbage $\chi^2$ accepted ---
    the reported bug no longer reproduces. Same run, patch active: the
    filter still diverges anyway, smoothly, $10^{15}\to3.4{\times}10^{17}$\,m
    over $60$\,s, proving a second, separate numerical issue survives
    outside the ZUPT updater &
    the RPNG bug report is updated with the patch and this finding,
    ready to submit, not yet submitted; the second divergence source
    (propagation or the MSCKF update, not yet diagnosed) is the only
    path left on this platform --- reading \code{Propagator.cpp} and
    \code{UpdaterMSCKF.cpp} with the same discipline, not attempted;
    no driven test follows until that source is found, per this
    item's own at-rest-first condition \\
42 & \textbf{MINS ROS1 entry point was an empty stub: the binary loaded its
    configuration and exited without subscribing to anything} (patched
    locally 2026-08-31; independently corroborated 2026-09-04).
    Upstream \code{mins/src/run\_subscribe.cpp} implements the ROS2 path in
    full (\code{rclcpp::init}, executor, spin) but leaves the
    \code{\#elif ROS\_AVAILABLE == 1} branch empty, a gap dating from the
    ROS2 port two years earlier: every other file and directory under
    \code{mins/src/} carries a recent upstream commit, that one file does
    not. On ROS1 the node printed \code{op->load\_print()}'s summary and
    returned, with no crash and no error message, so the failure presents
    as a successful run. Patched here by mirroring the ROS2 flow
    (\code{ros::init}, a \code{NodeHandle}, \code{ROSPublisher} and
    \code{ROSSubscriber}, \code{ros::spin()}). M. Crane reached the same
    defect independently and patched it as a parallel file
    (\code{run\_subscribe\_MWC.cpp}), which establishes one dormant
    upstream bug rather than two divergent MINS versions &
    document the patch in the body text, not only in the source comment;
    adopt the defensive configuration-path read from Crane's version
    (\code{nh->param<std::string>} with \code{argv[1]} retained as the
    fallback default), which tolerates a launch file passing the path
    through \code{<param>} instead of \code{args=}, the exact failure
    already documented in \S\ref{ap:repro-argsparam} \\
43 & \textbf{MINS stays on the ground station: the embedded option is closed
    on measurement, not on a failed attempt} (architecture decision,
    \S\ref{ap:mins-embedded}). MINS was never executed on the Pi; the
    verdict rests on an extrapolation whose inputs were each measured.
    MINS costs $85\,\%$ of one i7-8700 core with the robot stationary and
    dynamic cloning at its $9$\,Hz floor, a best case; the camera path is
    $90\,\%$ of that cost. A single-core floating-point benchmark put the
    workstation at $4.05\times$ the Pi 4B ($436$ against $108$
    operations/s), which places MINS near $3.4$ Pi cores against roughly
    $2.3$ free. The thermal margin closes the question rather than the
    arithmetic: the Pi idles at $70.6\,^{\circ}$C with the throttle flag
    already armed, and the $80$--$85\,^{\circ}$C threshold drops the clock
    to $600$\,MHz. Neither that threshold nor the $600$\,MHz state has been
    observed under MINS load, because no such load was ever applied. A
    \code{leo\_pi} profile exists (mono, \code{downsample: true}, online
    calibration off) and may well run, but it is unmeasured and it costs
    stereo on a platform where the camera is the only sensor constraining
    $z$ &
    keep the estimator on the ground station and treat the embedded path
    as reopened only by different hardware (Jetson class), not by further
    tuning; if the \code{leo\_pi} profile is ever to be claimed as viable,
    measure it on the target rather than extrapolating a second time \\
44 & \textbf{Battery below the project's own threshold, coincident with a
    silent motor-command failure; the link is plausible and undemonstrated}
    (2026-09-02). During an investigation into the rover not moving,
    \code{/firmware/battery} read a stable $9.33$--$9.41$\,V across ten
    samples, against the $10.3$\,V threshold this project sets in
    \code{leo\_backend.py} and against the $10.3$\,V that had already
    produced motor torque sag on 2026-07-10
    (\S\ref{sec:precision-campaign}). Concurrently
    \code{pwm\_duty\_cycle} read $[0,0,0,0]$, \code{/cmd\_vel} reported
    \code{Subscribers: None} (the CORE2 rosserial subscription was not
    registered at all), and $261$ checksum errors were logged in two
    minutes, while telemetry continued normally at $20.1$\,Hz on wheel
    states and $87$\,Hz on the IMU. Healthy telemetry is therefore not
    evidence of a healthy power rail, since telemetry draws almost no
    current. The chain brownout $\to$ failed rosserial negotiation $\to$ no
    subscriber $\to$ no motion is consistent with every observation but has
    not been demonstrated &
    recharge and re-read \code{/cmd\_vel}: if \code{Subscribers: None}
    persists at full voltage the fault is separate and must be diagnosed on
    its own terms. Independently of that outcome, surface battery voltage
    as an operator-visible alarm below $10.3$\,V rather than leaving it a
    value one has to think to query, and read it first in any future motor
    or actuation investigation \\
45 & \textbf{Dual-IMU evaluation requested: MINS propagates from exactly one
    IMU, so this is an engineering effort and not a configuration change}
    (H. Gutierrez, 2026-09-04; scoping done, nothing implemented).
    \code{State} holds a single \code{shared\_ptr<IMU>}, there is one
    \code{feed\_measurement\_imu()} entry point, and the five updater types
    present (\code{cam}, \code{wheel}, \code{vicon}, \code{gps},
    \code{lidar}) include no IMU updater, because in this architecture the
    IMU drives propagation rather than correcting it. On this rover the
    D455's onboard IMU is additionally not active anywhere: every
    IMU-derived topic traces back to the CORE2 body unit. \textbf{Phase A
    was executed on 2026-09-06 and returned a blocking negative result.}
    The motion module is confirmed present at four independent levels (USB
    descriptor exposing a \code{HID} interface; kernel modules
    \code{hid\_sensor\_accel\_3d} and \code{hid\_sensor\_gyro\_3d} loaded;
    IIO nodes \code{accel\_3d} and \code{gyro\_3d}; and the driver's own
    \code{Motion Module was found}), and it was disabled only by omission:
    \code{/etc/ros/robot.launch} never overrode the upstream defaults
    \code{enable\_gyro: false} and \code{enable\_accel: false}. Enabling it
    publishes \code{/camera/imu} (\code{sensor\_msgs/Imu}, gyro 200\,Hz,
    accelerometer 100\,Hz) --- but the RealSense node then never reaches
    \code{RealSense Node Is Up}, restarts in a loop, and \emph{all three
    image streams stop}. Both publication modes fail identically
    (\code{linear\_interpolation} and \code{none}), so the interpolation
    thread is not the cause. Two candidate explanations were eliminated by
    measurement: USB bandwidth (the link negotiates 3.20 at
    5000\,Mbit/s) and the libusb \code{control\_transfer} warning storm,
    which is present with the IMU disabled as well. The configuration was
    reverted and the chain reverified. Separately, the driver reports
    \code{IMU Calibration is not available}, so this unit carries no
    factory IMU calibration. Kalibr as installed ships no IMU-to-IMU
    extrinsic tool; \code{tools/compose\_imu\_extrinsics.py} now chains two
    camera-IMU calibrations through the shared camera, validated against
    Kalibr's independently computed stereo baseline to
    $4.6\times10^{-15}$\,mm &
    The driver stall is now the critical path: nothing in any dual-IMU
    strategy is reachable until it is understood. Treat it as a timeboxed
    spike (librealsense version, kernel HID binding, camera firmware, in
    that order) and re-plan afterwards. Phase B, only if the IMU can be
    brought up without losing the cameras: characterise it by Allan
    variance rather than assuming the existing densities apply, then use it
    as the sole propagating IMU to establish whether it beats the CORE2
    unit at all, before implementing an IMU updater on the pattern of the
    existing \code{wheel} updater. Joint dual-IMU propagation is a third,
    far larger effort touching \code{State}, \code{StateHelper} and the
    propagator, and is not recommended as a continuation of either phase.
    A standing reservation applies to all of them: the dominant inertial
    error here is a \emph{scale} error on the CORE2 accelerometer that
    drifts over weeks, and least-squares averaging with a second,
    uncalibrated IMU does not remove a systematic scale error --- the
    defensible gain is fault \emph{detection}, obtainable from a
    consistency monitor far cheaper than a fusion node \\
46 & \textbf{MINS cannot ingest vicon measurements in live mode: the
    ground-frame anchor rests on a subscription that does not exist}
    (found 2026-09-06 by reading the source, while tracing the sensor
    inputs for a handoff document). \code{ROSSubscriber} registers exactly
    five subscriptions --- IMU, camera, wheel, GPS, lidar
    (\code{ROSSubscriber.cpp} lines 52, 69/79, 87, 94, 102) --- and there
    is \emph{no} vicon subscription, in the ROS1 or the ROS2 subscriber.
    \code{SystemManager::feed\_measurement\_vicon()} is reached only from
    \code{run\_simulation.cpp:135} and \code{run\_bag.cpp:188}, and the
    converter \code{ROSHelper::PoseStamped2Data} likewise only from bag
    playback. Setting \code{config\_vicon.yaml} \code{enabled: true} would
    allocate vicon extrinsic state and begin publishing
    \code{/mins/vicon0/pose} while \emph{no measurement ever reaches the
    filter} --- a failure that presents as ``the anchor has no effect'',
    with no error raised anywhere. This is the same shape of defect as the
    ROS1 entry point (item~42): a path implemented for bags and simulation
    but never wired into \code{run\_subscribe}. It matters because the
    Carolus/AprilTag beacon fix is the platform's only route to a surveyed
    ground frame: MINS's \code{global} origin is the pose at which the
    filter initialised, gravity-aligned through
    \code{imu\_gravity\_aligned: true} but with arbitrary yaw and position,
    and the downstream \code{pose\_selector} SE(3) correction only keeps
    the fused output continuous across a source switch --- it registers
    nothing to the world &
    Implement a vicon callback in \code{ROSSubscriber}, mirroring the four
    that already exist; the conversion helper
    \code{ROSHelper::PoseStamped2Data} is already written and can be reused
    unchanged, so the work is subscription plumbing rather than new
    estimation code. Until then, treat any plan that begins ``enable the
    beacon anchor'' as blocked, and do not schedule beacon-placement
    sessions against it: a beacon physically in the camera's field of view
    changes nothing while the measurement has no path into the filter.
    Verify the fix by the arrival of measurements, not by the appearance of
    \code{/mins/vicon0/pose}, which publishes either way \\

47 & \textbf{MINS diverges spontaneously with every input healthy, and a
    respawn is the only known remedy} (2026-09-08: two distinct failures
    within one hour on the same process). The first was a \emph{locked
    initialisation}: $x,y$ correct at $(0.74, 0.30)$\,m but $z = -327.08$\,m,
    and \emph{stationary} --- $3.6$\,mm of movement over 30\,s, amplitude
    $6.7$\,mm, $-0.01$\,m/min. The filter was tracking well, simply anchored
    327\,m too low, having initialised during the network instability that
    had made the robot unreachable the previous evening. The second, thirty
    minutes after the repair, was an \emph{active runaway} at 562\,m/min with
    the robot immobile ($z$ from $+1325$ to $+1138$\,m in 20\,s). Inputs were
    measured healthy \emph{during} the runaway: IMU at 85\,Hz with
    $|a| = 9.7894$\,m/s$^2$, both infrared streams at $7.5$\,Hz, wheels at
    $18.7$\,Hz --- so the fault is internal to the filter, not a sensor
    dropout &
    Do not treat the respawn as a fix: it restores $6.3$\,mm accuracy
    (drift $0.9/0.8/0.4$\,mm over 30\,s) but the cause remains unidentified,
    and the failure recurred within the hour. Instrument the filter's own
    covariance and chi-squared acceptance rates over time rather than only
    its output pose, since both failures were invisible in the pose until
    they were catastrophic. Note that the two signatures differ --- one
    frozen, one runaway --- so a single mechanism is not yet established \\
48 & \textbf{\code{pose\_selector} keeps a stale SE(3) correction that
    survives an estimator reset, masking a healthy filter}
    (2026-09-08, measured). The node holds one correction matrix per source
    so the fused output does not jump when the operator switches estimator.
    Computed while MINS sat at $z=-327$\,m, it \emph{survived} the MINS
    respawn: the selector then tracked the repaired filter faithfully ---
    deltas identical to a tenth of a millimetre --- while adding a constant
    $142.07$\,m offset. The cockpit therefore showed a diverged robot while
    the estimator underneath was accurate to millimetres &
    Re-selecting the source does \emph{not} clear it: the correction is
    recomputed to preserve continuity with the (wrong) fused pose, which
    reproduces the offset. Restarting the node is required, since
    \code{\_correction} initialises to identity. Enforce the ordering
    sanitiser $\rightarrow$ MINS $\rightarrow$ \code{pose\_selector} after
    any estimator reset. A durable fix would invalidate the cached
    correction whenever the upstream source reports a reset, rather than
    relying on the operator to remember the sequence \\
49 & \textbf{An IMU--camera calibration exists, is half-installed, and
    cannot be finished with the recording on disk} (2026-08-20 output,
    found 2026-09-09, \S\ref{sec:demicalib}). Kalibr converged against
    \code{/imu/data\_clean} with $0.187$/$0.201$\,px reprojection, and its
    four noise densities are already deployed digit-for-digit in
    \code{config\_imu.yaml}. Its extrinsic was never installed: MINS still
    carries the tape-measure seed, $16.9$\,cm away in translation, worth
    $28.5$\,px of reprojection at 2\,m. The rotations agree closely, so the
    disagreement is purely the lever arm &
    Do \emph{not} install the existing value. The calibration bag carries
    $611^\circ$ of cumulative rotation, a $36.5^\circ$/s peak and an
    accelerometer standard deviation of $0.20/0.10/0.09$\,m/s$^2$ --- near
    stationary, so the translation is weakly observable by
    Eq.~(\ref{eq:nulldir}) whatever the reprojection residual says.
    Re-record with vigorous six-axis excitation and re-run the same
    pipeline, which already exists and has already run once. Since
    \code{do\_calib\_ext} was frozen on 2026-09-07 the seed is permanent,
    so this is now blocking rather than merely valuable \\
50 & \textbf{No estimator recording exists after either divergence
    patch} (\S\ref{sec:datationpreuves}). The sqrtVINS ZUPT guards are
    dated 2026-08-24 and the openVINS covariance reset 2026-08-28; the most
    recent recording in \code{data/} is from 2026-08-24, and it carries no
    covariance columns. Every consistency result in this report therefore
    describes software that has since been changed &
    Record one stationary run and one drive on the current build, exported
    with the \code{Ppose} and \code{Ptwist} columns, and re-run the NEES
    procedure of \S\ref{sec:nees}. About twenty minutes of robot time. It
    would date the covariance audit to what is actually running and close
    the before-and-after on both patches, neither of which has a measured
    effect today. \textbf{Partiellement clos le 2026-09-09} : une course
    statique de 242\,s sur le build courant date desormais l'audit MINS
    (\S\ref{sec:mesureactuelle}) ; les deux VIO n'ont rien publie, ne
    s'initialisant pas a l'arret, de sorte que leur datation demande une
    course commencant par une excitation puis s'immobilisant, sans
