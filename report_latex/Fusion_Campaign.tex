...
36 & \textbf{Colour-camera geometry: three stacked defects, all fixed
    2026-08-18, item still open pending validation}
    ...
    principal point \emph{outside} the image. Auditing the launch found
    \textbf{two more nobody had noticed}: \code{fov=true} selected the
    Astrobee FOV lens model (which reads only \code{distortion[0]} as its
    $\omega$) although the D455 declares \code{plumb\_bob}; and
    \code{distortion} was a \code{<param>} \emph{string}, so
    \code{getParam} into a \code{vector<double>} failed silently and the
    code fell back to its hard-coded $\{0.993591,0,0,0\}$ --- those
    coefficients had \emph{never} been applied. Now factory intrinsics for
    this exact profile, \code{fov=false}, and a real \code{<rosparam>} list,
    all verified on the parameter server &
    geometry is now \emph{plausible} where it was impossible, but this is
    factory calibration, not a dedicated one, and no detection has been
    validated against a measured distance. \textbf{Still no dimensional
    metric from this camera} until a beacon at known range is checked.
    Items 37 and 5 stay gated \\
