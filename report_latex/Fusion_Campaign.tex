...
38 & \textbf{Thermal envelope is three degrees wide} --- \textbf{active
    cooling installed August 11, partially re-measured}
    (\S\ref{sec:july30thermal}, \S\ref{sec:aug11thermal}). The July 28
    scheduling fix holds the serial link at $86\,^\circ$C under full colour
    load ($19.7$\,Hz wheel odometry, $82.8$\,Hz IMU) and does not hold it at
    $89.1\,^\circ$C, where the chain collapses entirely ($676\,751$ UART
    overruns, load $8.85$, all firmware topics at $0$\,Hz). Two GPIO fans
    now hold the ARM clock at $1500$\,MHz (no throttling, $89.1\,^\circ$C
    never approached) at both a light load ($58$--$59\,^\circ$C) and the
    heaviest load observed to date, a $31$-node stack restart
    ($72.5\,^\circ$C, margin $M_{89.1}=16.6\,^\circ$C by
    Eq.~\ref{eq:thermalmargin}) &
    the matched-load A/B this item originally specified, overrun rate at
    the exact July 30 profile ($185\,\%$ colour nodelet, sustained,
    cooling installed), has still not been run; tonight's heavy-load figure
    is a different, heavier-but-not-equivalent profile and is suggestive,
...
40 & \textbf{A live UART checksum storm, unresolved, two more hypotheses
    now eliminated by measurement} (\S\ref{sec:aug11mins}).
    \code{tools/restart\_stack.sh} ended \code{TIMEOUT init MINS}; a
    re-check found \code{mins\_subscribe} registered but
    \code{/mins/imu/odom} and \code{/firmware/wheel\_states} both silent
    over a $10$\,s window, \code{pose\_selector} still reporting
    \code{pose\_source: "MINS"} regardless, and
    \code{/serial\_node} filling \code{journalctl} with sustained
    \code{wrong checksum for topic id and msg} entries, the same
    signature as Eq.~\ref{eq:missprob} (July 30) and
    \S\ref{sec:july22network} (July 22), while the ROS graph wiring itself
    (unlike the July 22 case) was confirmed correct. Both earlier
    occurrences were thermal; the ARM clock stayed at $1500$\,MHz
    throughout, ruling that specific mechanism out.
    \textbf{Follow-up, same night:} the storm was confirmed still active
    with the rover completely stationary (\code{/firmware/wheel\_states}
    velocity all zero), directly refuting the motion-excitation hypothesis
    this item previously proposed. A second hypothesis, that the kernel's
    global real-time budget (\code{sched\_rt\_runtime\_us}, found still at
    its stock default of $950\,000\,\mu$s$/1\,000\,000\,\mu$s, i.e.\ a
    mandatory $5\,\%$ idle window every second across \emph{all} SCHED\_FIFO
    tasks regardless of individual priority) was starving
    \code{serial\_node} despite its correct \code{rtprio}$=25$, evidenced by
    one \code{dmesg} line, \code{sched: RT throttling activated}, $271$\,s
    after boot, was tested directly: a controlled before/after A/B
    (\code{sysctl -w kernel.sched\_rt\_runtime\_us=-1}, four 15\,s samples
    against a 15\,s baseline of $38$ checksum errors) gave
    $28,41,36,26$, well within the baseline's own sample-to-sample noise,
    not a reduction. The setting was reverted immediately
    (\code{950\,000}, confirmed). A structural fact was found in passing
    that narrows the remaining search: IRQ~$14$ (\code{uart-pl011}, the
    CORE2 link) is a traditional, non-threaded hard interrupt pinned
    entirely to CPU0 ($\approx550$/s), not a schedulable kernel thread, so
    neither \code{serial\_node}'s process priority nor the kernel's RT
    budget setting is the layer where a multi-millisecond gap could
    actually be introduced; both were plausible and both are now ruled out
    by measurement, not assumption &
    instrument interrupt-handling latency directly (e.g.\ the kernel's own
    \code{irqsoff}/\code{preemptirqsoff} ftrace tracers, or a
    \code{cyclictest} run against \code{/dev/ttyAMA0}'s IRQ) rather than
    continuing to test process-level scheduling hypotheses, since two of
    those have now been eliminated by direct, controlled measurement \\
\end{longtable}
\end{center}
