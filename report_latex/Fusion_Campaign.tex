...
\paragraph{Reading the result (to be completed once both drives exist).}
This subsection is written ahead of the two drives it analyses, deliberately
scaffolded with the exact numbers and judgement calls the closing analysis
will need, so that filling it in is a matter of driving the loop twice and
transcribing the script's own printed output rather than re-deriving the
